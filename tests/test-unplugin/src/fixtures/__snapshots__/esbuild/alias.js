    ...
    const __errors = (() => {
      ...
      const __pick = (chunks) => {
        ...
        for (const chunk of chunks) {
          ...
          if (best.length === 0 || chunk.length < best.length) best = chunk;
        }
        return best;
      };
      const __prop = (path, key) => /^[$A-Z_a-z][$0-9A-Z_a-z]*$/.test(key) ? path + "." + key : path + "[" + JSON.stringify(key) + "]";
      const __diag_0 = (v, p) => (() => {
