...
namespace plain {
  ...
  function validateClassify<T>(
    input: Classifiable<T>,
  ): IValidation<ClassifyResult<T>>; // + validate
}
```
...
Four case converters: `camel`, `pascal`, `snake`, `kebab`. Each one walks the object and rewrites property names to that case at type _and_ runtime. Same four-variant family as everywhere else (`name`, `assertName`, `isName`, `validateName`, plus `create*` factories).
...
A query string has no spelling for an empty array, so an absent key decodes a required array to `[]`, a nullable one to `null`, and an optional one to `undefined`. An absent required non-array property makes `http.query` throw `Error("missing <key>")`; `assertQuery`, `isQuery`, and `validateQuery` report it as a type mismatch instead.

A blank numeric value decodes to `undefined` rather than `0`. In a string-typed property, the text `null` decodes to `null` only where the type admits `null`; elsewhere it stays the string `"null"`.
...
`http.headers` decodes an Express/Fastify-style headers object. Numbers and bigints are coerced as in `query`, booleans only from `true` and `false`, with a stricter set of restrictions specific to HTTP:

1. `T` must be an object type
2. No dynamic keys
3. **Property keys are case-insensitive**: typia reads each header by its lowercase name, so two keys that differ only by case are rejected
...
An absent optional array header, or one given as an empty array, is omitted from the decoded object, while an absent required one decodes to `[]`. A string value of any other array header is split on `, ` (`; ` for `cookie`), but `set-cookie` never is, because cookie attributes such as `Expires` contain `, `: the values of a `string[]` `set-cookie` are kept as given, and a single string becomes a one-element array. A blank numeric header decodes to `undefined` rather than `0`.
...
namespace http {
  function parameter<T extends boolean | bigint | number | string | null>(
    input: string,
  ): Resolved<T>;
}
```
...
A blank segment is not a number, and a string-typed parameter keeps the text `null` unless `T` admits `null`: `http.parameter<string>("null")` returns the string `"null"`.
...
An absent array field decodes like a query array: `[]` when required, `null` when nullable. A blank numeric field decodes to `undefined` rather than `0`, and a string-typed field keeps the text `null` unless its type admits `null`.
...
An `IMetadataSchema` is a _union of buckets_ with top-level flags:
...
`T` may hold only literal types and the `boolean` atomic, which stands for `true | false`. An argument carrying anything else — `string`, `number`, a template literal type — is a compile error, so `reflect.literals<"a" | number>()` never compiles into a partial array.

An argument naming no literal at all is a compile error too, including `never` and the `Exclude` that filters every member away. There is no list to hand back, and an empty array would read as a real answer.

A `bigint` literal comes back as a `bigint`, exact at any magnitude — `reflect.literals<9007199254740993n>()` is `[9007199254740993n]`, not the value a `number` would have rounded it to.
