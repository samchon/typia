...
func TestMetadataReferencesToJSONPreserveTags(t *testing.T) {
  ...
  expectedTagNames := []string{"ArrayTag", "TupleTag", "ObjectTag", "AliasTag"}
  for i, ref := range cases {
    if ref.Name == "" || len(ref.Tags) != 1 || len(ref.Tags[0]) != 1 {
      t.Fatalf("reference JSON lost name or tag: %#v", ref)
    }
    if ref.Tags[0][0].Name != expectedTagNames[i] {
      t.Fatalf("reference %d tag name mismatch: expected %q got %q", i, expectedTagNames[i], ref.Tags[0][0].Name)
    }
  }
}
