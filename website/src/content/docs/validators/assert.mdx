...
Same rules as [`is`](/docs/validators/is#what-it-doesnt-check): user-defined classes are walked by property (no `instanceof`), and function-typed properties are skipped unless you turn on the `functional` transform flag.
