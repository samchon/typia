...
Plain-data operations that reshape a value to its declared type `T`: `clone` deep-copies it, `prune` strips properties that aren't on `T`. Each has the same `assert` / `is` / `validate` family and `create*` factories as the validators.
