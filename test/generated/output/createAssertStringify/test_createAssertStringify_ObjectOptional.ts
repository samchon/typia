...
export const test_createAssertStringify_ObjectOptional = _test_assertStringify("ObjectOptional", ObjectOptional.generate, (input: any): string => { const assert = (input: any): ObjectOptional => {
    ...
    return $so0(input);
}; return stringify(assert(input)); }, ObjectOptional.SPOILERS);
