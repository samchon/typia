...
export function _test_isStringify<T>(name: string, generator: () => T, converter: (input: T) => string | null, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        const data: T = generator();
        const optimized: string | null = converter(data);
        if (optimized === null || predicate(data, optimized) === false) {
            throw new Error(`Bug on typia.isStringify(): failed to understand the ${name} type.`);
        }
        for (const spoil of spoilers ?? []) {
            const elem: T = generator();
            spoil(elem);
            if (converter(elem) !== null)
                throw new Error(`Bug on typia.isStringify(): failed to detect error on the ${name} type.`);
        }
    };
}
