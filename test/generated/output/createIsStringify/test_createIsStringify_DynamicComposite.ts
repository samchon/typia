...
export const test_createIsStringify_DynamicComposite = _test_isStringify("DynamicComposite", DynamicComposite.generate, (input: DynamicComposite): string | null => { const is = (input: any): input is DynamicComposite => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, DynamicComposite.SPOILERS);
