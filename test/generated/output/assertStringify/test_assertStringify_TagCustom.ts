...
export const test_assertStringify_TagCustom = _test_assertStringify("TagCustom", TagCustom.generate, (input) => ((input: any): string => { const assert = (input: any): TagCustom => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), TagCustom.SPOILERS);
