...
Jev's own wire format, shared by TypeSafe's API and OpenRouter's Decisions API, spells the boolean question type `"noul"`. [`@typia/jev`](/docs/utilization/jev) converts the questions, evaluates through either endpoint, and validates the answers; the raw answers keep the probabilities and confidence that the decision type has no room for.
