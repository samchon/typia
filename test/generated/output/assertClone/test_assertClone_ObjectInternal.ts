...
export const test_assertClone_ObjectInternal = _test_assertClone("ObjectInternal", ObjectInternal.generate, (input) => ((input: any): typia.Primitive<ObjectInternal> => { const assert = (input: any): ObjectInternal => {
...
}; assert(input); const output = clone(input); return output; })(input), ObjectInternal.SPOILERS);
