...
export const test_assertParse_TagFormat = _test_assertParse("TagFormat", TagFormat.generate, (input) => ((input: string): typia.Primitive<TagFormat> => { const assert = (input: any): TagFormat => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagFormat.SPOILERS);
