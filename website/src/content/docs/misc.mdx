...
Plain `classify` trusts the input; `assertClassify` and `validateClassify` validate it first — the same pairing as the validators. There is no `is` variant. The input type [`Classifiable<T>`](https://github.com/samchon/typia/blob/master/packages/interface/src/typings/Classifiable.ts) is the plain seed the chosen strategy accepts; the output is always the instance `T`.
