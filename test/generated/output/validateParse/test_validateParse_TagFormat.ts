...
export const test_validateParse_TagFormat = _test_validateParse("TagFormat", TagFormat.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TagFormat>> => { const validate = (input: any): typia.IValidation<TagFormat> => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), TagFormat.SPOILERS);
