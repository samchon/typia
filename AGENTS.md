...
`typia` is a TypeScript transformer and runtime library that turns pure TypeScript types into inline validators, serializers, schemas, decoders, and random-data generators. It ships the `typia`, `@typia/interface`, `@typia/utils`, `@typia/langchain`, `@typia/mcp`, `@typia/vercel`, and `@typia/jev` packages.
