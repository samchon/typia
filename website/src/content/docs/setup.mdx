...
```bash filename="Terminal" copy showLineNumbers
npm install --save typia
npm install --save-dev @typescript/native-preview @typia/ttsc
```
  </Tabs.Tab>
  <Tabs.Tab>
```bash filename="Terminal" copy showLineNumbers
pnpm install --save typia
pnpm install --save-dev @typescript/native-preview @typia/ttsc
```
  </Tabs.Tab>
  <Tabs.Tab>
```bash filename="Terminal" copy showLineNumbers
# YARN BERRY IS NOT SUPPORTED
yarn add typia
yarn add -D @typescript/native-preview @typia/ttsc
```
  </Tabs.Tab>
  <Tabs.Tab>
```bash filename="Terminal" copy showLineNumbers
bun add typia
bun add -d @typescript/native-preview @typia/ttsc
```
  </Tabs.Tab>
</Tabs>
...
Firstly install `typia` as a dependency. And then, install `@typescript/native-preview` and `@typia/ttsc` as `devDependencies`.
...
After installing `typia` like above, and ensuring `@typescript/native-preview` and `@typia/ttsc` are available, modify the `tsconfig.lib.json` on each `@nx/js` package to be similar to the below.
