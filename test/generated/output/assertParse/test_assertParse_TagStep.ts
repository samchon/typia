...
export const test_assertParse_TagStep = _test_assertParse("TagStep", TagStep.generate, (input) => ((input: string): typia.Primitive<TagStep> => { const assert = (input: any): TagStep => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagStep.SPOILERS);
