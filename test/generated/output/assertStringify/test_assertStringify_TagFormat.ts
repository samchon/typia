...
export const test_assertStringify_TagFormat = _test_assertStringify("TagFormat", TagFormat.generate, (input) => ((input: any): string => { const assert = (input: any): TagFormat => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), TagFormat.SPOILERS);
