...
export const test_validateParse_TagStep = _test_validateParse("TagStep", TagStep.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TagStep>> => { const validate = (input: any): typia.IValidation<TagStep> => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), TagStep.SPOILERS);
