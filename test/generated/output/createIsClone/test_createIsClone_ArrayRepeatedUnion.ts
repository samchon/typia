...
export const test_createIsClone_ArrayRepeatedUnion = _test_isClone("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: any): typia.Primitive<ArrayRepeatedUnion> | null => { const is = (input: any): input is ArrayRepeatedUnion => {
    const $ip0 = (input: any) => {
        const array = input;
        const top = input[0];
        if (0 === input.length)
    ...
    return null; const output = clone(input); return output; }, ArrayRepeatedUnion.SPOILERS);
