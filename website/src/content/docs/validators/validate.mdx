...
| Scenario                                                               | Pick                                                   |
| ---------------------------------------------------------------------- | ------------------------------------------------------ |
| Need to render every field error to the user (form, request body)      | **`validate`** _(you are here)_                        |
...
typia declares `StandardSchemaV1` itself rather than depending on a package for it, the same way `zod` and `valibot` do, so `import type { StandardSchemaV1 } from "typia"` is all you need to write the factory's type out by hand.
...
Same rules as the rest of the validator family — see [`is` § What it doesn't check](/docs/validators/is#what-it-doesnt-check). User-defined classes are walked by property, function types are skipped unless `functional` is on, native classes (`Date`, `Set`, `Map`, `Uint8Array`) are inspected end-to-end.
...
`validate` does the most work — it walks the entire structure even after finding errors, and collects each one with path and expected-type strings. It's still up to 20,000× faster than the JS-ecosystem alternatives on union-heavy types, and still the only one that handles arbitrary unions. The full structural feature matrix is on [`is` § Performance](/docs/validators/is#performance) — same coverage, since `validate` extends `is` with error collection.
