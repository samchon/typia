...
The same boundary applies to `tags.Format<"byte">`. Typia keeps `format: "byte"` in its emended schema, OpenAPI 3.0 output retains that spelling, and raw OpenAPI 3.1 output uses `contentEncoding: "base64"`. OpenAPI 3.0 preserves an explicitly supplied content media type as `x-jsonschema-contentMediaType` and a non-byte content encoding as `x-jsonschema-contentEncoding`; OpenAPI 3.1 uses the standard `contentMediaType` and `contentEncoding` keywords. Typia does not invent either annotation when none was declared.

### Swagger 2 document conversion

For Swagger 2 documents, `OpenApiConverter.upgradeDocument` composes each server URL from `schemes`, `host`, and `basePath` and rejects endpoint fields containing queries or fragments. Equivalent host casing and root URL spellings with and without a trailing slash normalize to one Swagger endpoint. An operation's `schemes`, `consumes`, and `produces` override the document defaults; when neither media list is present, the converter uses `application/json` for compatibility. An explicit empty media list clears the inherited default, but it cannot accompany a body or response payload because the emended content model has no media key under which to retain that schema or example. Swagger `formData` parameters become one object-shaped request body; scalar enums, nullable scalar or array fields, and recursively nested arrays of those scalar shapes remain simple form fields. File-field validation metadata and `x-*` extensions remain on the binary property schema, and response examples remain associated with their MIME types.

`OpenApiConverter.downgradeDocument(document, "2.0")` keeps media types on each operation instead of guessing shared document defaults. Schema constants become typed Swagger enums, and supported annotations on individual constant branches, including nullable branches and their `default: null`, survive a downgrade and re-upgrade. An empty response `content` map normalizes to a response without a payload. Swagger 2 can express only one host and base path and one schema shared by a response's media types, so the converter throws a `TypeError` when server descriptions, variables, credentials, unsupported schemes, different authorities or paths, streaming item schemas, encrypted bodies or responses, request bodies without at least one defined media type and schema, non-empty response media without a schema or example, mixed form and non-form request media, forms without a defined field, aggregate form-object constraints, non-simple form fields, mismatched form-body and field requiredness, media-specific schemas, or named examples would otherwise be lost. File form fields support both `multipart/form-data` and `application/x-www-form-urlencoded`, as defined by Swagger 2. Response examples may still differ by MIME type because Swagger represents them as a keyed map.
