...
export const _test_functional_validateEqualsFunction =
  (name: string) =>
    ...
    const task = (replacer: string) => (callback: (input: T) => [T, T]) => {
      // SUCCESS
      {
        ...
        if (validate(() => y)(x).success === false)
          throw new Error(
            `Bug on typia.functional.validateEqualsFunction(): failed to understand the ${name} type.`,
          );
      ...
      if (
        ...
        console.log(actual);
        throw new Error(
          `Bug on typia.functional.validateEqualsFunction(): failed to detect surplus property on the ${name} type.`,
        );
      }
    };
