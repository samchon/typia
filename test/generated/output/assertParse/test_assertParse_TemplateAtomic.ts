...
export const test_assertParse_TemplateAtomic = _test_assertParse("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: string): typia.Primitive<TemplateAtomic> => { const assert = (input: any): TemplateAtomic => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TemplateAtomic.SPOILERS);
