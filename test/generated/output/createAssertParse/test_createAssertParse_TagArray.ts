...
export const test_createAssertParse_TagArray = _test_assertParse("TagArray", TagArray.generate, (input: string): typia.Primitive<TagArray> => { const assert = (input: any): TagArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TagArray.SPOILERS);
