# Audit 9 — Cycle 3 Phase 0 Critical Self-Review (2026-04-18)

> 목적: master plan v5(cycle 2 반영 후) 작성된 `08/17-phase0-kickoff.md`의 결함을 공격적으로 찾아내 개정 대상을 명확히 함.

## 결함 목록 (20개, 우선순위 표기)

### 🔴 Critical (Phase 0 실행 직전 반드시 해결)
...
- **개정**: Week 0 (사전 준비) + 6주 스파이크 또는 Phase 0 작업 컷다운
...
- Option B: Phase 2부터 내부 dogfooding (risk 큼, 버그 빨리 발견)
...
- **개정**: 17번에 "setup v2" 초안 스펙 섹션 추가 (Phase 1~2 상세화)

---

### 🟡 Important (Phase 0 전/중 해결)
...
- typia: MIT → 호환 OK
- **개정**: Phase 0 Week 1에 "외부 참조 LICENSE 확인" 작업 추가
...
#### D16. Phase 0 samchon 혼자 4주 예산 현실성
- 총 작업: 공식 입장문 (2주 draft) + scaffolding + submodule + gen_shims + metadata Go + end-to-end + 2 경로 prototype + benchmarks + 2 blog + Go 조력자 모집 + Phase 1 plan
- 혼자 주 25시간 × 4주 = 100시간 예산으로 실행 불가능
- **개정**:
  - 작업 중 일부를 Phase 0.5 (후속 2주)로 분할
  - 또는 병행 작업(blog, 입장문 게시)을 Phase 1 Week 1~2로 이월
  - 6주 스파이크로 현실화
...
- **개정**: Phase 0 Week 0 (사전 준비)에 이름 가용성·상표 체크 추가
...
#### D13. 입장문 발행 시점 재조정
- Week 1 draft → Week 4 발행 (Phase 0 결과와 함께)
...
- Phase 0 동안 typescript-go가 release할 가능성
...
1. **Phase 0에서 Go 조력자 확보 실패 시** — 현재 계획에는 "Phase 2 범위 축소"뿐. samchon 혼자 3년은 번아웃 R8 확실 트리거. **대안**: AutoBE 수익 우선 검증 후 Phase 1 착수
2. **typescript-go의 Go 1.27 handshake 불응** — 현재 "협상" 추상. **대안**: effect-tsgo의 patch 모델을 fallback으로 Phase 0 Week 4에 동시 검증
3. **tsgonest 2027 Q2 NestJS 점유 50%+** — typia v13 preview가 타이밍 놓치면 방어 불가. **대안**: Phase 2 validators 집중 (json/llm 후순위)
4. **Standard Schema가 기대만큼 효과 없을 가능성** — Zod v4가 여전히 디팩토 유지 → typia 어댑터 출시해도 유입 저조 가능. **대안**: MCP/AI SDK별 직접 통합 패키지 (@typia/mcp 등) 동시 강화

## 한 줄

> **Phase 0 계획은 방향은 맞으나 주차별 패킹·자기 빌드·UX·측정 기준·중단 기준에 공백이 있다. 17번을 6주 스파이크로 재구성하고 10개 critical 결함을 반영하면 실행 가능한 무결 계획이 된다.**
