...
| `V & tags.Probability<N>` on a literal union member | that member's acceptance minimum; in an `Array<...>` set, its inclusion threshold |
...
A member's own requirement wins over the property's. A choice or score member without either has no minimum, and a set member without either uses the `0.5` threshold, since each set member is its own yes/no decision. Gating only the risky options is the intended use:
