...
export const test_isParse_ArrayRecursive = _test_isParse("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: any): typia.Primitive<ArrayRecursive> => { const is = (input: any): input is ArrayRecursive => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ArrayRecursive.SPOILERS);
