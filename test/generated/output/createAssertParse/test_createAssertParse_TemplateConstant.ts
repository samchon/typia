...
export const test_createAssertParse_TemplateConstant = _test_assertParse("TemplateConstant", TemplateConstant.generate, (input: string): typia.Primitive<TemplateConstant> => { const assert = (input: any): TemplateConstant => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TemplateConstant.SPOILERS);
