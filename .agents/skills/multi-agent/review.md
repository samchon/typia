...
7. Record every lead and critic disposition in the campaign knowledge base. If any meaningful candidate survives, finish adjudicating the whole round, then replace the review team and repeat from step 2 against the same frozen commit before publishing or implementing anything. A replaced team holds no inventory, so it re-derives one rather than inheriting the previous round's. Repeat until one round leaves no surviving candidate; discovery owns this loop and never hands a partial candidate pool to publication or implementation. A prior finding is not permission to narrow the next round to its owner or neighbors.
