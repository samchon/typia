...
export const test_misc_createPrune_ObjectHierarchical = _test_misc_prune(
  "ObjectHierarchical",
)<ObjectHierarchical>(ObjectHierarchical)((input: ObjectHierarchical): void => {
  ...
  const $po4 = (input: any): any => {
    if ("object" === typeof input.created_at && null !== input.created_at)
      $po2(input.created_at);
    for (const key of Object.keys(input)) {
      if ("id" === key || "code" === key || "created_at" === key) continue;
      delete input[key];
    }
  };
