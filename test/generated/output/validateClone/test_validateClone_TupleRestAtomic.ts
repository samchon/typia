...
export const test_validateClone_TupleRestAtomic = _test_validateClone("TupleRestAtomic", TupleRestAtomic.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<[boolean, number, ...string[]]>> => { const validate = (input: any): typia.IValidation<[boolean, number, ...string[]]> => {
    ...
    return Array.isArray(input) && ("boolean" === typeof input[0] && "number" === typeof input[1] && (Array.isArray(input.slice(2)) && input.slice(2).every((elem: any) => "string" === typeof elem))) ? [
        input[0] as any,
        input[1] as any,
        ...Array.isArray(input.slice(2)) ? input.slice(2).map((elem: any) => elem as any) : input.slice(2) as any
    ] as any : input as any;
}; const output = validate(input) as any; if (output.success)
    output.data = clone(input); return output; })(input), TupleRestAtomic.SPOILERS);
