...
export const test_assertEquals_TemplateUnion = _test_assertEquals("TemplateUnion", TemplateUnion.generate, (input) => ((input: any): Array<TemplateUnion.Type> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is Array<TemplateUnion.Type> => {
            const $ao0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => ("string" === typeof input.prefix && (RegExp(/^prefix_(.*)/).test(input.prefix) || RegExp(/^prefix_-?\d+\.?\d*$/).test(input.prefix)) || $guard(_exceptionable, {
                ...
                expected: "(\"the_A_value\" | \"the_B_value\" | __type | `the_${number}_value` | boolean | number)",
                value: input.mixed
            })) && $ao1(input.mixed, _path + ".mixed", true && _exceptionable))) && (4 === Object.keys(input).length || (false === _exceptionable || Object.keys(input).every(key => {
                if (["prefix", "postfix", "middle", "mixed"].some(prop => key === prop))
                    return true;
                const value = input[key];
                if (undefined === value)
