...
export const test_createAssertParse_ArrayAtomicAlias = _test_assertParse("ArrayAtomicAlias", ArrayAtomicAlias.generate, (input: string): typia.Primitive<ArrayAtomicAlias> => { const assert = (input: any): ArrayAtomicAlias => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ArrayAtomicAlias.SPOILERS);
