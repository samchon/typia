	...
	for _, node := range metadata_node_declarations(symbol) {
		for _, jsdoc := range node.JSDoc(nil) {
			...
			for _, tag := range doc.Tags.Nodes {
				...
				} else if text := metadata_js_doc_type_expression_text(tag); text != "" {
					texts = append(texts, schemametadata.IJsDocTagInfo_IText{
						Kind: "text",
						Text: text,
					})
...
func metadata_js_doc_type_expression_text(tag *nativeast.Node) (out string) {
	if tag == nil {
		return ""
	}
	defer func() {
	...
	if typed := expr.AsJSDocTypeExpression(); typed != nil && typed.Type != nil {
		expr = typed.Type
	}
	sourceFile := nativeast.GetSourceFileOfNode(expr)
	if sourceFile == nil {
		return strings.TrimSpace(expr.Text())
	}
	text := nativescanner.GetSourceTextOfNodeFromSourceFile(sourceFile, expr, false)
