...
export const test_createValidateClone_ArrayRepeatedUnion = _test_validateClone("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: any): typia.IValidation<typia.Primitive<ArrayRepeatedUnion>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedUnion> => {
    const errors = [] as any[];
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is ArrayRepeatedUnion => {
...
}; const clone = (input: ArrayRepeatedUnion): typia.Primitive<ArrayRepeatedUnion> => {
    ...
    output.data = clone(input); return output; }, ArrayRepeatedUnion.SPOILERS);
