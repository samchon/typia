...
export const test_notation_createValidatePascal_DynamicNever =
  ...
  >({
    convert: (
      input: any,
    ): typia.IValidation<typia.PascalCase<DynamicNever>> => {
      ...
      const general = (input: DynamicNever): typia.PascalCase<DynamicNever> => {
    ...
    assert: (input: any): typia.PascalCase<DynamicNever> => {
      const __is = (input: any): input is typia.PascalCase<DynamicNever> => {
        const $io0 = (input: any): boolean =>
          Object.keys(input).every((key: any) => {
            const value = input[key];
            if (undefined === value) return true;
