...
export const test_assertStringify_NativeSimple = _test_assertStringify("NativeSimple", NativeSimple.generate, (input) => ((input: NativeSimple): string => { const assert = (input: any) => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), NativeSimple.SPOILERS);
