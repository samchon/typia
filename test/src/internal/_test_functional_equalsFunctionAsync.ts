    ...
    const task = async (callback: (input: T) => [T, T]) => {
      {
        ...
        if ((await validate(async () => y)(x)) === null)
          throw new Error(
            `Bug on await typia.functional.equalsFunction(): failed to understand the ${name} type.`,
          );
      }
      ...
      if ((await validate(async () => y)(x)) !== null)
        throw new Error(
          `Bug on await typia.functional.equalsFunction(): failed to detect surplus property on the ${name} type.`,
        );
    };
