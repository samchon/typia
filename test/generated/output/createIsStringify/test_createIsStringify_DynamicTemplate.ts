...
export const test_createIsStringify_DynamicTemplate = _test_isStringify("DynamicTemplate", DynamicTemplate.generate, (input: DynamicTemplate): string | null => { const is = (input: any): input is DynamicTemplate => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, DynamicTemplate.SPOILERS);
