...
export const test_isParse_DefinitionArrayRecursiveNullable = _test_isParse("DefinitionArrayRecursiveNullable", DefinitionArrayRecursiveNullable.generate, (input) => ((input: any): typia.Primitive<DefinitionArrayRecursiveNullable> => { const is = (input: any): input is DefinitionArrayRecursiveNullable => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), DefinitionArrayRecursiveNullable.SPOILERS);
