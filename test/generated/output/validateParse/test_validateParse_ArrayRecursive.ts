...
export const test_validateParse_ArrayRecursive = _test_validateParse("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ICategory>> => { const validate = (input: any): typia.IValidation<ICategory> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ArrayRecursive.SPOILERS);
