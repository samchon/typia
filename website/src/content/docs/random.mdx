...
`MultipleOf` is the one constraint where a generated `number` can fail its own validator. The generator divides exact decimals, while `tags.MultipleOf<N>` validates with `value % N === 0`, which divides the binary doubles actually stored — so the two part company wherever a value or a divisor has no exact binary representation. `typia.random<number & tags.MultipleOf<0.1>>()` can return `0.3`, and `0.3 % 0.1` is `0.09999999999999998`; past `Number.MAX_SAFE_INTEGER` even an integer divisor can diverge. A `bigint` divisor is exact and always agrees.
