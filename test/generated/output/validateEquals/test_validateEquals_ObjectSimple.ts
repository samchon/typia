...
export const test_validateEquals_ObjectSimple = _test_validateEquals("ObjectSimple", ObjectSimple.generate, (input) => ((input: any): typia.IValidation<ObjectSimple.IBox3D> => {
    const __is = (input: any, _exceptionable: boolean = true): input is ObjectSimple.IBox3D => {
        ...
        const $io1 = (input: any, _exceptionable: boolean = true): boolean => "number" === typeof input.x && Number.isFinite(input.x) && ("number" === typeof input.y && Number.isFinite(input.y)) && ("number" === typeof input.z && Number.isFinite(input.z)) && (3 === Object.keys(input).length || Object.keys(input).every(key => {
