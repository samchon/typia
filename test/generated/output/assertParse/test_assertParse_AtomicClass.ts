...
export const test_assertParse_AtomicClass = _test_assertParse("AtomicClass", AtomicClass.generate, (input) => ((input: string): typia.Primitive<AtomicClass> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), AtomicClass.SPOILERS);
