...
export const test_assertStringify_DynamicUndefined = _test_assertStringify("DynamicUndefined", DynamicUndefined.generate, (input) => ((input: any): string => { const assert = (input: any): DynamicUndefined => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), DynamicUndefined.SPOILERS);
