...
export const test_createAssertParse_TagType = _test_assertParse("TagType", TagType.generate, (input: string): typia.Primitive<TagType> => { const assert = (input: any): TagType => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TagType.SPOILERS);
