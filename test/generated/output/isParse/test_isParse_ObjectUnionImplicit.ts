...
export const test_isParse_ObjectUnionImplicit = _test_isParse("ObjectUnionImplicit", ObjectUnionImplicit.generate, (input) => ((input: any): typia.Primitive<ObjectUnionImplicit> => { const is = (input: any): input is ObjectUnionImplicit => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ObjectUnionImplicit.SPOILERS);
