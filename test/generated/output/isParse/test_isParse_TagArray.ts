...
export const test_isParse_TagArray = _test_isParse("TagArray", TagArray.generate, (input) => ((input: any): typia.Primitive<TagArray> => { const is = (input: any): input is TagArray => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), TagArray.SPOILERS);
