...
export const test_createAssertParse_ArrayMatrix = _test_assertParse("ArrayMatrix", ArrayMatrix.generate, (input: string): typia.Primitive<ArrayMatrix> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ArrayMatrix.SPOILERS);
