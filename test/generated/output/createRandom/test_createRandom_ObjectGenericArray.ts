...
export const test_createRandom_ObjectGenericArray = _test_random("ObjectGenericArray", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<ObjectGenericArray> => {
    const $generator = (typia.createRandom as any).generator;
    const $ro0 = (_recursive: boolean = false, _depth: number = 0): any => ({
        pagination: $ro1(_recursive, _recursive ? 1 + _depth : _depth),
        data: (generator?.array ?? $generator.array)(() => $ro2(_recursive, _recursive ? 1 + _depth : _depth))
    });
    const $ro1 = (_recursive: boolean = false, _depth: number = 0): any => ({
        page: (generator?.customs ?? $generator.customs)?.number?.([]) ?? (generator?.number ?? $generator.number)(0, 100),
        limit: (generator?.customs ?? $generator.customs)?.number?.([]) ?? (generator?.number ?? $generator.number)(0, 100),
        total_count: (generator?.customs ?? $generator.customs)?.number?.([]) ?? (generator?.number ?? $generator.number)(0, 100),
        total_pages: (generator?.customs ?? $generator.customs)?.number?.([]) ?? (generator?.number ?? $generator.number)(0, 100)
    });
    ...
    const __is = (input: any): input is typia.Primitive<ObjectGenericArray> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<ObjectGenericArray> => {
