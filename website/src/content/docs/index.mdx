...
> The right-hand "Compiled JavaScript" tab is produced by typia's compile-time transform — **stock `tsc` will not produce it**. To enable the transform, install one of the build paths in [Setup](/docs/setup) (`ttsc` for TypeScript-Go, `ts-patch` for the stable TypeScript v5/v6 release). Without setup, your `typia.is<T>()` call sites throw at runtime instead.
