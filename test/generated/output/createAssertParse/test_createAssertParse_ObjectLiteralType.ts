...
export const test_createAssertParse_ObjectLiteralType = _test_assertParse("ObjectLiteralType", ObjectLiteralType.generate, (input: string): typia.Primitive<ObjectLiteralType> => { const assert = (input: any): ObjectLiteralType => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectLiteralType.SPOILERS);
