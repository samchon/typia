...
export const test_validateClone_NativeUnion = _test_validateClone("NativeUnion", NativeUnion.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<Array<NativeUnion.Union>>> => { const validate = (input: any): typia.IValidation<Array<NativeUnion.Union>> => {
    const errors = [] as any[];
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is Array<NativeUnion.Union> => {
            const $vo0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => [null === input.date || input.date instanceof Date || $report(_exceptionable, {
                    path: _path + ".date",
                    expected: "(Date | null)",
                    value: input.date
                    ...
                    value: input.float
                }), input.buffer instanceof ArrayBuffer || input.buffer instanceof SharedArrayBuffer || input.buffer instanceof DataView || input.buffer instanceof Buffer || $report(_exceptionable, {
                    path: _path + ".buffer",
...
}; const clone = (input: Array<NativeUnion.Union>): typia.Primitive<Array<NativeUnion.Union>> => {
    const $io1 = (input: any): boolean => "Buffer" === input.type && (Array.isArray(input.data) && input.data.every((elem: any) => "number" === typeof elem));
    ...
    const $co0 = (input: any): any => ({
        ...
        unsigned: input.unsigned instanceof Uint8Array ? {} : input.unsigned instanceof Uint8ClampedArray ? {} : input.unsigned instanceof Uint16Array ? {} : input.unsigned instanceof Uint32Array ? {} : input.unsigned instanceof BigUint64Array ? {} : input.unsigned as any,
        signed: input.signed instanceof Int8Array ? {} : input.signed instanceof Int16Array ? {} : input.signed instanceof Int32Array ? {} : input.signed instanceof BigInt64Array ? {} : input.signed as any,
        float: input.float instanceof Float32Array ? {} : input.float instanceof Float64Array ? {} : input.float as any,
        buffer: "object" === typeof input.buffer && null !== input.buffer && "function" === typeof input.buffer.toJSON ? input.buffer.toJSON() as any : input.buffer instanceof ArrayBuffer ? {} : input.buffer instanceof SharedArrayBuffer ? {} : input.buffer instanceof DataView ? {} : "object" === typeof input.buffer && null !== input.buffer ? $co1(input.buffer) : input.buffer as any,
        weak: input.weak instanceof WeakSet ? {} : input.weak instanceof WeakMap ? {} : input.weak as any
    });
    const $co1 = (input: any): any => ({
        type: input.type as any,
        data: Array.isArray(input.data) ? $cp1(input.data) : input.data as any
    });
    return Array.isArray(input) ? $cp0(input) : input as any;
}; const output = validate(input) as any; if (output.success)
    output.data = clone(input); return output; })(input), NativeUnion.SPOILERS);
