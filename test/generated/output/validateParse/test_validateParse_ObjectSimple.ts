...
export const test_validateParse_ObjectSimple = _test_validateParse("ObjectSimple", ObjectSimple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<IBox3D>> => { const validate = (input: any): typia.IValidation<IBox3D> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ObjectSimple.SPOILERS);
