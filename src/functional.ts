...
----------------------------------------------------------- */
/**
 * Wraps functions with runtime validation that checks parameters and return values.
 *
 * Creates a wrapper around your function that validates both input parameters 
 * and return values at runtime. If validation fails, throws detailed error 
 * information. Perfect for creating validated API endpoints, ensuring function 
 * contracts, or adding runtime safety to critical functions.
 *
 * The validation errors show exactly which parameter failed and why, making 
 * debugging much easier than generic "invalid input" messages.
 *
 * @example
 * ```typescript
 ...
 * 
 * const safeCreateUser = typia.functional.assertFunction(createUser);
 * // Validates both input parameters and return value
 * ```
 *
 * @template T Function type to wrap with validation
 * @param func Function to wrap with validation
 * @param errorFactory Custom error generator (optional)
 * @returns Wrapped function with parameter and return value validation
 */
...
/**
 * Wraps functions with validation that only checks input parameters.
 *
 * Creates a wrapper that validates function parameters at runtime but lets 
 * return values pass through unchecked. Useful when you trust your function 
 * implementation but want to ensure callers provide valid inputs.
 *
 * Lighter weight than full function validation when you only care about 
 * input validation and don't need return value checking.
 *
 * @example
 * ```typescript
 * function processUser(user: User, options: ProcessOptions): any {
 *   // Complex processing that you trust
 *   return processedResult;
 * }
 * 
 * const safeProcess = typia.functional.assertParameters(processUser);
 * // Only validates input parameters, not return value
 * ```
 *
 * @template T Function type to wrap with parameter validation
 * @param func Function to wrap with parameter validation
 * @param errorFactory Custom error generator (optional)  
 * @returns Wrapped function with parameter validation only
 */
