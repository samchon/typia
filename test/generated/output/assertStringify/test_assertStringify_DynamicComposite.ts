...
export const test_assertStringify_DynamicComposite = _test_assertStringify("DynamicComposite", DynamicComposite.generate, (input) => ((input: any): string => { const assert = (input: any): DynamicComposite => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), DynamicComposite.SPOILERS);
