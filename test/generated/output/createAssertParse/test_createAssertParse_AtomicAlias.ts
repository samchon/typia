...
export const test_createAssertParse_AtomicAlias = _test_assertParse("AtomicAlias", AtomicAlias.generate, (input: string): typia.Primitive<AtomicAlias> => { const assert = (input: any): AtomicAlias => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, AtomicAlias.SPOILERS);
