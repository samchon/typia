...
export const test_createAssertParse_DynamicTree = _test_assertParse("DynamicTree", DynamicTree.generate, (input: string): typia.Primitive<DynamicTree> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, DynamicTree.SPOILERS);
