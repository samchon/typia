...
export const test_assertParse_ObjectNullable = _test_assertParse("ObjectNullable", ObjectNullable.generate, (input) => ((input: string): typia.Primitive<ObjectNullable> => { const assert = (input: any): ObjectNullable => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectNullable.SPOILERS);
