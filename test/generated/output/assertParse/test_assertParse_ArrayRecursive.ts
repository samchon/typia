...
export const test_assertParse_ArrayRecursive = _test_assertParse("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: string): typia.Primitive<ICategory> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ArrayRecursive.SPOILERS);
