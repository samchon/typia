...
export const test_validateParse_DynamicTree = _test_validateParse("DynamicTree", DynamicTree.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicTree>> => { const validate = (input: any): typia.IValidation<DynamicTree> => {
    const errors = [] as any[];
    ...
    ((input: any, _path: string, _exceptionable: boolean): input is DynamicTree => {
        ...
        const $vo1 = (input: any, _path: string, _exceptionable: boolean) => [false === _exceptionable || Object.keys(input).map(key => {
    ...
    } as typia.IValidation<DynamicTree>;
}; input = JSON.parse(input); const output = validate(input); return output; })(input), DynamicTree.SPOILERS);
