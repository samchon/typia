...
export const test_assertClone_SetUnion = _test_assertClone("SetUnion", SetUnion.generate, (input) => ((input: any): typia.Primitive<Array<SetUnion.Union>> => { const assert = (input: any): Array<SetUnion.Union> => {
...
}; assert(input); const output = clone(input); return output; })(input), SetUnion.SPOILERS);
