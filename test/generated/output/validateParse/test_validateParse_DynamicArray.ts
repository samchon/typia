...
export const test_validateParse_DynamicArray = _test_validateParse("DynamicArray", DynamicArray.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicArray>> => { const validate = (input: any): typia.IValidation<DynamicArray> => {
    const errors = [] as any[];
    ...
    } as typia.IValidation<DynamicArray>;
}; input = JSON.parse(input); const output = validate(input); return output; })(input), DynamicArray.SPOILERS);
