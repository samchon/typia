...
export const test_assertClone_AtomicUnion = _test_assertClone("AtomicUnion", AtomicUnion.generate, (input) => ((input: any): typia.Primitive<Array<AtomicUnion.Union>> => { const assert = (input: any): Array<AtomicUnion.Union> => {
...
}; assert(input); const output = clone(input); return output; })(input), AtomicUnion.SPOILERS);
