...
export const test_createAssertParse_ArrayAny = _test_assertParse("ArrayAny", ArrayAny.generate, (input: string): typia.Primitive<ArrayAny> => { const assert = (input: any): ArrayAny => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ArrayAny.SPOILERS);
