...
export const test_validateParse_ObjectHierarchical = _test_validateParse("ObjectHierarchical", ObjectHierarchical.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ICustomer>> => { const validate = (input: any): typia.IValidation<ICustomer> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ObjectHierarchical.SPOILERS);
