...
A fractional divisor is the one case where the generated value can fail its own validator. The generator divides exact decimals, while `tags.MultipleOf<N>` validates with `value % N === 0`, and binary floating point only reaches a zero remainder for a power-of-two quotient — so `typia.random<number & tags.MultipleOf<0.1>>()` can return `0.3`, which `typia.is` rejects. Integer and `bigint` divisors are unaffected.
