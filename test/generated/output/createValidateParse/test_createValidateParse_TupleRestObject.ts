...
export const test_createValidateParse_TupleRestObject = _test_validateParse("TupleRestObject", TupleRestObject.generate, (input: string): typia.IValidation<typia.Primitive<TupleRestObject>> => { const validate = (input: any): typia.IValidation<TupleRestObject> => {
    const errors = [] as any[];
    ...
    } as typia.IValidation<TupleRestObject>;
}; input = JSON.parse(input); const output = validate(input); return output; }, TupleRestObject.SPOILERS);
