...
> 사용자는 `typia.is<T>()` 같은 기존 API를 그대로 호출하고, `typia/lib/transform` 이 Go native analyzer/emitter를 통해 호출 위치를 rewrite한다.
...
| 제거된 표면  | `@typia/core`, `@typia/transform`, `TypiaProgrammer`                       |
