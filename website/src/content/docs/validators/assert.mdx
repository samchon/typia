...
| Scenario                                                 | Pick                                    |
| -------------------------------------------------------- | --------------------------------------- |
| One field is wrong → throw an exception                  | **`assert`** _(you are here)_           |
...
That's convenient but it makes you assign to a new variable. If you'd rather **narrow the original variable in place**, use `typia.assertGuard<T>`. It returns nothing — but its return type is `asserts input is T`, which is TypeScript's _assertion guard_ syntax: when a function with that signature returns without throwing, the compiler treats the argument as narrowed to `T` on every subsequent line. So you keep using the same variable instead of binding a new one:
...
Same idea as [`createIs`](/docs/validators/is#reusable-checkers) — when you call `assert<T>` from many places, hoist the checker once:
...
export const guardUserEquals: AssertionGuard<User> =
  typia.createAssertGuardEquals<User>();
```
...
>
> `AssertionGuard<T>` is one parameter wide, so it also hides the per-call `errorFactory` described below. Annotate with `(input: unknown, errorFactory?: (props: TypeGuardError.IProps) => Error) => asserts input is T` when you need that override.
...
`createAssert*` factories accept an optional `errorFactory` that applies to every call. The function they return takes that same factory as an optional **second parameter**, which overrides the factory for that one call:

```typescript copy
const assertUser = typia.createAssert<User>(
  (info) => new HttpException(`Invalid ${info.path}`, 400),
);

assertUser(input); // throws HttpException
assertUser(input, (info) => new Error(`audit: ${info.path}`)); // throws Error
```

> [!WARNING]
>
> **Do not hand a created function straight to `.map()`.**
>
> `Array#map`, `forEach`, and `filter` pass the element index as their second argument, which lands in `errorFactory`. TypeScript rejects `rows.map(assertUser)` for that reason; write `rows.map((row) => assertUser(row))` instead.
>
> Code compiled against typia 13.2.0 or earlier still type-checks the direct form, because those releases declared the returned function with one parameter. Run it on this release and the index is ignored: the failing element raises a `TypeGuardError` instead of the configured error. Run it on 13.2.0 and it raises `TypeError: factory is not a function` for every element but the first.
...
Same rules as [`is`](/docs/validators/is#what-it-doesnt-check): user-defined classes are walked by property (no `instanceof`), and function-typed properties are skipped unless you turn on the [`functional` transform flag](/docs/validators/is#what-it-doesnt-check) in `tsconfig.json`.
...
Same emission strategy as [`is`](/docs/validators/is#performance), plus the per-field error-throwing branches. Still up to 20,000× faster than `class-validator` on hard-to-validate union structures, and still the only listed library that handles arbitrary unions. The structural feature matrix (recursive unions, template literals, rest tuples, etc.) lives on [`is` § Performance](/docs/validators/is#performance) — same coverage, since `assert` is `is` plus error paths.
