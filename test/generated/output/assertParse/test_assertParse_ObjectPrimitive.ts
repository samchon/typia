...
export const test_assertParse_ObjectPrimitive = _test_assertParse("ObjectPrimitive", ObjectPrimitive.generate, (input) => ((input: string): typia.Primitive<ObjectPrimitive> => { const assert = (input: any): ObjectPrimitive => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectPrimitive.SPOILERS);
