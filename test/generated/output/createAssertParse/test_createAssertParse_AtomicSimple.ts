...
export const test_createAssertParse_AtomicSimple = _test_assertParse("AtomicSimple", AtomicSimple.generate, (input: string): typia.Primitive<AtomicSimple> => { const assert = (input: any): AtomicSimple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, AtomicSimple.SPOILERS);
