...
export const test_notation_createValidateCamel_DynamicUnion =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.CamelCase<DynamicUnion>> => {
      ...
      const general = (input: DynamicUnion): typia.CamelCase<DynamicUnion> => {
    ...
    assert: (input: any): typia.CamelCase<DynamicUnion> => {
      const __is = (input: any): input is typia.CamelCase<DynamicUnion> => {
        const $io0 = (input: any): boolean =>
          Object.keys(input).every((key: any) => {
            const value = input[key];
            if (undefined === value) return true;
