...
export const test_assertParse_TagCustom = _test_assertParse("TagCustom", TagCustom.generate, (input) => ((input: string): typia.Primitive<TagCustom> => { const assert = (input: any): TagCustom => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagCustom.SPOILERS);
