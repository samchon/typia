...
export const test_validateStringify_NativeAlias = _test_validateStringify("NativeAlias", NativeAlias.generate, (input) => ((input: NativeAlias): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<NativeAlias> => {
    const errors = [] as any[];
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is NativeAlias => {
