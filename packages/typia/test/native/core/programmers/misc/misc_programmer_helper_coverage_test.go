...
func TestMiscProgrammerHelperCoverage(t *testing.T) {
	...
	if miscPruneProgrammer_to_statement(input) == nil ||
		miscPruneProgrammer_top_level_body(input) == nil ||
	...
	name := "Named"
	if miscCloneProgrammer_internal(nativecontext.ITypiaContext{Importer: miscProgrammerImporter{}}, "helper").Text() != "helper" ||
		miscCloneProgrammer_import_type(nativecontext.ITypiaContext{Importer: miscProgrammerImporter{}}, nativeprogrammers.ImportProgrammer_TypeProps{Name: "Resolved"}) == nil ||
		miscCloneProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: "Resolved"}) == nil ||
		miscCloneProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: input}) != input ||
...
type miscProgrammerImporter struct{}

func (miscProgrammerImporter) Internal(name string) *shimast.Node {
	return shimast.NewNodeFactory(shimast.NodeFactoryHooks{}).NewIdentifier(name)
}

func (miscProgrammerImporter) Type(props nativeprogrammers.ImportProgrammer_TypeProps) *shimast.Node {
	if str, ok := props.Name.(string); ok {
		return shimast.NewNodeFactory(shimast.NodeFactoryHooks{}).NewIdentifier(str)
	}
