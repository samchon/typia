...
export const test_isStringify_MapAlias = _test_isStringify("MapAlias", MapAlias.generate, (input) => ((input: MapSimple): string | null => { const is = (input: any): input is MapSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), MapAlias.SPOILERS);
