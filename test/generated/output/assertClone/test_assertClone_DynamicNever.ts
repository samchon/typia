...
export const test_assertClone_DynamicNever = _test_assertClone("DynamicNever", DynamicNever.generate, (input) => ((input: any): typia.Primitive<DynamicNever> => { const assert = (input: any): DynamicNever => {
...
}; assert(input); const output = clone(input); return output; })(input), DynamicNever.SPOILERS);
