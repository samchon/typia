...
export const test_assertParse_ObjectLiteralType = _test_assertParse("ObjectLiteralType", ObjectLiteralType.generate, (input) => ((input: string): typia.Primitive<{ id: string; name: string; age: number; }> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectLiteralType.SPOILERS);
