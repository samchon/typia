...
export const test_assertParse_ObjectOptional = _test_assertParse("ObjectOptional", ObjectOptional.generate, (input) => ((input: string): typia.Primitive<ObjectOptional> => { const assert = (input: any): ObjectOptional => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectOptional.SPOILERS);
