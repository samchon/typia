...
| `tags.MultipleOf<N>`                                                                                                       | `value / N` has an integer mathematical quotient                                                                                                   |
...
For `number`, decimal divisors use exact finite-decimal arithmetic rather than JavaScript remainder rounding. For example, `0.03` satisfies `tags.MultipleOf<0.01>`, while `0.031` does not.

#### `string`

| Tag                         | Notes                                                                          |
| --------------------------- | ------------------------------------------------------------------------------ |
| `tags.MinLength<N>`         | Unicode code-point count is at least `N`                                       |
| `tags.MaxLength<N>`         | Unicode code-point count is at most `N`                                        |
...
String length follows JSON Schema character semantics: an astral character such as `😀` counts as one code point, while a base character and a combining mark count as two. It does not count grapheme clusters.
