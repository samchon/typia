...
export const test_assertParse_ObjectUnionComposite = _test_assertParse("ObjectUnionComposite", ObjectUnionComposite.generate, (input) => ((input: string): typia.Primitive<ObjectUnionComposite> => { const assert = (input: any): ObjectUnionComposite => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectUnionComposite.SPOILERS);
