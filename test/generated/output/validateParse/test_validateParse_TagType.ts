...
export const test_validateParse_TagType = _test_validateParse("TagType", TagType.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TagType>> => { const validate = (input: any): typia.IValidation<TagType> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), TagType.SPOILERS);
