...
export const test_createAssertPrune_ConstantAtomicUnion = _test_assertPrune("ConstantAtomicUnion", ConstantAtomicUnion.generate, (input: any): ConstantAtomicUnion => { const assert = (input: any): ConstantAtomicUnion => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, ConstantAtomicUnion.SPOILERS);
