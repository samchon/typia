...
export const test_assertClone_DynamicTree = _test_assertClone("DynamicTree", DynamicTree.generate, (input) => ((input: any): typia.Primitive<DynamicTree> => { const assert = (input: any): DynamicTree => {
...
}; assert(input); const output = clone(input); return output; })(input), DynamicTree.SPOILERS);
