...
export const test_stringify_DynamicTree = _test_stringify("DynamicTree", DynamicTree.generate, (input) => ((input: DynamicTree): string => {
    ...
    const $so0 = (input: any) => `{"id":${$string(input.id)},"sequence":${$number(input.sequence)},"children":${$so1(input.children)}}`;
