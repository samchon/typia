...
export const test_createAssertParse_DynamicComposite = _test_assertParse("DynamicComposite", DynamicComposite.generate, (input: string): typia.Primitive<DynamicComposite> => { const assert = (input: any): DynamicComposite => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, DynamicComposite.SPOILERS);
