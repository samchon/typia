...
export const test_assertClone_AtomicAlias = _test_assertClone("AtomicAlias", AtomicAlias.generate, (input) => ((input: any): typia.Primitive<[boolean, number, string]> => { const assert = (input: any): [boolean, number, string] => {
...
}; assert(input); const output = clone(input); return output; })(input), AtomicAlias.SPOILERS);
