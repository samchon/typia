...
func EmitHttpQueryObjectArrowFunction(schema *metadata.Schema) (string, error) {
	...
	b.WriteString("(input) => { if (typeof input === \"string\") input = new URLSearchParams(input); const __get = (k) => { if (!input) return undefined; if (typeof input.get === \"function\") return input.get(k); return input[k]; }; const __getAll = (k) => { if (!input) return []; if (typeof input.getAll === \"function\") return input.getAll(k); const v = input[k]; if (Array.isArray(v)) return v; return v === undefined ? [] : [v]; }; const out = {};")
...
func httpQueryProperty(s *metadata.Schema, name string) (string, error) {
	...
	if len(s.Arrays) == 1 && s.Arrays[0].Type != nil && s.Arrays[0].Type.Value != nil {
		...
		if kind, ok := httpConstantAtomicKind(elemSchema); ok {
			return fmt.Sprintf(` out[%s] = __getAll(%s).map((v) => %s);`, quoted, quoted, coerceAtomic("v", kind)), nil
		}
		if httpTemplateLike(elemSchema) {
			return fmt.Sprintf(` out[%s] = __getAll(%s).map((v) => String(v));`, quoted, quoted), nil
		}
		if nativeExpr, ok := httpNativePassthroughExpr("v", elemSchema); ok {
			return fmt.Sprintf(` out[%s] = __getAll(%s).map((v) => %s);`, quoted, quoted, nativeExpr), nil
		}
	...
	if httpTemplateLike(s) {
		if s.IsRequired() {
			return fmt.Sprintf(
				` { const v = __get(%s); if (v == null) throw new Error("typia.http.query: missing %s"); out[%s] = String(v); }`,
				quoted, jsEscape(name), quoted,
			), nil
		}
		return fmt.Sprintf(
			` { const v = __get(%s); if (v != null) out[%s] = String(v); }`,
			quoted, quoted,
		), nil
	}
	if nativeExpr, ok := httpNativePassthroughExpr("v", s); ok {
		if s.IsRequired() {
			return fmt.Sprintf(
				` { const v = __get(%s); if (v == null) throw new Error("typia.http.query: missing %s"); out[%s] = %s; }`,
				quoted, jsEscape(name), quoted, nativeExpr,
			), nil
		}
		return fmt.Sprintf(
			` { const v = __get(%s); if (v != null) out[%s] = %s; }`,
			quoted, quoted, nativeExpr,
		), nil
	}
