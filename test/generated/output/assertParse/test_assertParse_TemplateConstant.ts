...
export const test_assertParse_TemplateConstant = _test_assertParse("TemplateConstant", TemplateConstant.generate, (input) => ((input: string): typia.Primitive<TemplateConstant> => { const assert = (input: any): TemplateConstant => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TemplateConstant.SPOILERS);
