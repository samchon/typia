...
export function _test_validatePrune<T>(name: string, generator: () => T, pruner: (input: T) => IValidation<T>, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        ...
        if (pruner(input).success === false)
            throw new Error(`Bug on typia.validatePrune(): failed to prune the ${name} type.`);
        ...
        for (const spoil of spoilers ?? []) {
            ...
            if (valid.success === true)
                throw new Error(`Bug on typia.validatePrune(): failed to detect error on the ${name} type.`);
            assert(valid);
            expected.sort();
