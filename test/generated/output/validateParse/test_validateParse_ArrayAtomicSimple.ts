...
export const test_validateParse_ArrayAtomicSimple = _test_validateParse("ArrayAtomicSimple", ArrayAtomicSimple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArrayAtomicSimple>> => { const validate = (input: any): typia.IValidation<ArrayAtomicSimple> => {
    const errors = [] as any[];
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is ArrayAtomicSimple => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArrayAtomicSimple.SPOILERS);
