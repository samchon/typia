...
Close each issue from the commit that earns it. End the commit message with one `Close #n: <issue title>` line per resolved issue, and submit a formal GitHub pull-request review with the `COMMENT` event naming what that commit landed. A revert inside the batch must not carry the closing keyword forward, and the merge gate reconciles the closing keywords against what survives at `HEAD`; the [solo campaign development procedure](../issue-campaign/development.md#implement-and-write-tests) owns both rules.
