...
export const test_createAssertParse_ConstantAtomicWrapper = _test_assertParse("ConstantAtomicWrapper", ConstantAtomicWrapper.generate, (input: string): typia.Primitive<ConstantAtomicWrapper> => { const assert = (input: any): ConstantAtomicWrapper => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ConstantAtomicWrapper.SPOILERS);
