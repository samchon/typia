...
export interface IMcpServerOptions {
  textFallback?: boolean; // default true
}
```
...
The opt-out only suppresses the duplicate copy. A result with no structured representation — a `void` method's `"Success"`, a validation failure, a runtime error — keeps its text content regardless.
