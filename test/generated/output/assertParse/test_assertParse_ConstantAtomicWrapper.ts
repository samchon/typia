...
export const test_assertParse_ConstantAtomicWrapper = _test_assertParse("ConstantAtomicWrapper", ConstantAtomicWrapper.generate, (input) => ((input: string): typia.Primitive<ConstantAtomicWrapper> => { const assert = (input: any): ConstantAtomicWrapper => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ConstantAtomicWrapper.SPOILERS);
