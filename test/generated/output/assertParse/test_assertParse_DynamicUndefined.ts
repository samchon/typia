...
export const test_assertParse_DynamicUndefined = _test_assertParse("DynamicUndefined", DynamicUndefined.generate, (input) => ((input: string): typia.Primitive<DynamicUndefined> => { const assert = (input: any): DynamicUndefined => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicUndefined.SPOILERS);
