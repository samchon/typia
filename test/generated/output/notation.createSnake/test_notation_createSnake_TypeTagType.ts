...
export const test_notation_createValidateSnake_TypeTagType =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.SnakeCase<TypeTagType>> => {
      ...
      const general = (input: TypeTagType): typia.SnakeCase<TypeTagType> => {
        const $io1 = (input: any): boolean =>
          "number" === typeof input.int &&
          Math.floor(input.int) === input.int &&
          -2147483648 <= input.int &&
    ...
    assert: (input: any): typia.SnakeCase<TypeTagType> => {
      const __is = (input: any): input is typia.SnakeCase<TypeTagType> => {
        const $io0 = (input: any): boolean =>
          Array.isArray(input.value) &&
          input.value.every(
            (elem: any) =>
