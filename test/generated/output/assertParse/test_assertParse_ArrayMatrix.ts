...
export const test_assertParse_ArrayMatrix = _test_assertParse("ArrayMatrix", ArrayMatrix.generate, (input) => ((input: string): typia.Primitive<ArrayMatrix> => { const assert = (input: any): ArrayMatrix => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayMatrix.SPOILERS);
