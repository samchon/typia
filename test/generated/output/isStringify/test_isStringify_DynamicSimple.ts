...
export const test_isStringify_DynamicSimple = _test_isStringify("DynamicSimple", DynamicSimple.generate, (input) => ((input: DynamicSimple): string | null => { const is = (input: any): input is DynamicSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), DynamicSimple.SPOILERS);
