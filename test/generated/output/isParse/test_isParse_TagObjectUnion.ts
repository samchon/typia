...
export const test_isParse_TagObjectUnion = _test_isParse("TagObjectUnion", TagObjectUnion.generate, (input) => ((input: any): typia.Primitive<TagObjectUnion> => { const is = (input: any): input is TagObjectUnion => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), TagObjectUnion.SPOILERS);
