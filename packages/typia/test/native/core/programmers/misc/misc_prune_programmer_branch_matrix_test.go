...
func TestMiscPruneProgrammerBranchMatrix(t *testing.T) {
	...
	}{Config: config, Functor: functor, Collection: collection})) != 1 {
	...
	if miscPruneProgrammer_to_statement(nil) != nil ||
		miscPruneProgrammer_to_statement(factory.NewBlock(factory.NewNodeList(nil), false)).Kind != shimast.KindBlock ||
		miscPruneProgrammer_to_statement(factory.NewReturnStatement(input)).Kind != shimast.KindReturnStatement {
