...
export const test_validateParse_ArrayRepeatedUnionWithTuple = _test_validateParse("ArrayRepeatedUnionWithTuple", ArrayRepeatedUnionWithTuple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArrayRepeatedUnionWithTuple>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedUnionWithTuple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArrayRepeatedUnionWithTuple.SPOILERS);
