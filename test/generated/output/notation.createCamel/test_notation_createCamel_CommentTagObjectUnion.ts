...
export const test_notation_createValidateCamel_CommentTagObjectUnion =
    ...
    >({
        convert: (
            input: any,
        ): typia.IValidation<typia.CamelCase<CommentTagObjectUnion>> => {
            const validate = (
                input: any,
            ): typia.IValidation<CommentTagObjectUnion> => {
                const errors = [] as any[];
            ...
            const general = (
                ...
                const $io1 = (input: any): boolean =>
                    ...
                    3 <= input.value.length &&
                    input.value.length <= 7;
                const $throws = (typia.notations.createValidateCamel as any)
                    .throws;
                const $cp0 = (input: any) =>
                    input.map((elem: any) =>
        ...
        assert: (input: any): typia.CamelCase<CommentTagObjectUnion> => {
            const __is = (
                input: any,
            ): input is typia.CamelCase<CommentTagObjectUnion> => {
                const $io0 = (input: any): boolean =>
                    "number" === typeof input.value &&
                    Number.isFinite(input.value) &&
                    3 <= input.value;
