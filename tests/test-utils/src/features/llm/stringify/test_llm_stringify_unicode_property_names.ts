...
export const test_llm_stringify_unicode_property_names = (): void => {
  ...
  TestEquality.equals("korean-success", result1.success, false);
  if (!result1.success) {
    const output1: string = LlmJson.stringify(result1);
    TestEquality.equals("korean-code-block", output1.includes("```json"), true);
    TestEquality.equals("korean-error-marker", output1.includes("// ❌"), true);
    TestEquality.equals("korean-key", output1.includes("이름"), true);
  }
  ...
  TestEquality.equals("japanese-success", result2.success, false);
  if (!result2.success) {
    ...
    TestEquality.equals("japanese-key", output2.includes("名前"), true);
  }
  ...
  TestEquality.equals("value-success", result3.success, false);
  if (!result3.success) {
    const output3: string = LlmJson.stringify(result3);
    TestEquality.equals("value-code-block", output3.includes("```json"), true);
    TestEquality.equals("value-error-marker", output3.includes("// ❌"), true);
  }
};
