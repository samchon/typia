...
export const test_assertParse_ArrayAtomicAlias = _test_assertParse("ArrayAtomicAlias", ArrayAtomicAlias.generate, (input) => ((input: string): typia.Primitive<ArrayAtomicAlias> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ArrayAtomicAlias.SPOILERS);
