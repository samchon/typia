...
export const test_notation_createValidateSnake_ArrayRepeatedOptional =
  ...
  >({
    convert: (
      input: any,
    ): typia.IValidation<typia.SnakeCase<ArrayRepeatedOptional>> => {
      const validate = (
        input: any,
    ...
    assert: (input: any): typia.SnakeCase<ArrayRepeatedOptional> => {
      const __is = (
        input: any,
      ): input is typia.SnakeCase<ArrayRepeatedOptional> => {
        const $ia0 = (input: any): any =>
          input.every(
            (elem: any) =>
              null !== elem &&
