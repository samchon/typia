...
<Tabs items={[
  ...
  <Tab>
```typescript filename="typia" showLineNumbers {2-14}
...
`typia.llm.schema<T, Model>()` is a function generating type schema which is used in the LLM (Large Language Model) function calling application schema, especially in the [`typia.llm.parameters<Parameters, Model>()`](./parameters) and [`typia.llm.application<App, Model>()`](./application) functions. 
