...
export const test_validateParse_TupleRestArray = _test_validateParse("TupleRestArray", TupleRestArray.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TupleRestArray>> => { const validate = (input: any): typia.IValidation<TupleRestArray> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), TupleRestArray.SPOILERS);
