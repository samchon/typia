...
export const test_assertParse_DynamicUnion = _test_assertParse("DynamicUnion", DynamicUnion.generate, (input) => ((input: string): typia.Primitive<DynamicUnion> => { const assert = (input: any): DynamicUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicUnion.SPOILERS);
