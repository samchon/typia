...
export const test_llm_json_structuredOutput_basic = (): void => {
  ...
  // Test parse
  const json = JSON.stringify({ name: "John", age: 30, email: "john@test.com" });
  ...
  const corrupted = { name: "Jane", age: "25" as unknown as number, email: "jane@test.com" };
  const coerced = output.coerce(corrupted as IMember);
  TestValidator.equals("coerce.age", coerced.age, 25);

  // Test validate (valid input)
  const validResult = output.validate({ name: "Bob", age: 40, email: "bob@test.com" });
  TestValidator.equals("validate.success", validResult.success, true);

  // Test validate (invalid input)
  const invalidResult = output.validate({ name: "Invalid", age: "not-a-number", email: "test@test.com" });
