...
`criteria` tells the model how to distinguish the available answers. For a choice it maps each string value to its enum member's JSDoc description. For a score it is an array of numeric enum member descriptions, sorted by numeric value. The score returned by the provider is an index into that array; `decode()` maps the index back to the enum's numeric value. A missing choice description becomes `null`, while a missing score description becomes the numeric value written as text.
...
// Set these only to the precision guaranteed by your Jev endpoint.
