...
export const test_isParse_ObjectGenericUnion = _test_isParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input) => ((input: any): typia.Primitive<ISaleEntireArticle> => { const is = (input: any): input is ISaleEntireArticle => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ObjectGenericUnion.SPOILERS);
