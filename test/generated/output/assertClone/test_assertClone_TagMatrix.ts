...
export const test_assertClone_TagMatrix = _test_assertClone("TagMatrix", TagMatrix.generate, (input) => ((input: any): typia.Primitive<TagMatrix> => { const assert = (input: any): TagMatrix => {
...
}; assert(input); const output = clone(input); return output; })(input), TagMatrix.SPOILERS);
