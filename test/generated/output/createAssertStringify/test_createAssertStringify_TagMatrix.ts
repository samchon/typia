...
export const test_createAssertStringify_TagMatrix = _test_assertStringify("TagMatrix", TagMatrix.generate, (input: any): string => { const assert = (input: any): TagMatrix => {
    ...
    return $so0(input);
}; return stringify(assert(input)); }, TagMatrix.SPOILERS);
