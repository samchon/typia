...
func TestNotationGeneralHelpers(t *testing.T) {
	...
	importerContext := nativecontext.ITypiaContext{Importer: nativecontext.NewImportProgrammer()}
	if notationGeneralProgrammer_internal(nativecontext.ITypiaContext{}, "helper") == nil ||
		notationGeneralProgrammer_internal(importerContext, "helper") == nil {
		t.Fatal("internal fallback returned nil")
	}
	if notationGeneralProgrammer_import_type(nativecontext.ITypiaContext{}, nativecontext.ImportProgrammer_TypeProps{Name: "Alias"}) == nil {
		t.Fatal("string import type fallback returned nil")
	}
	if notationGeneralProgrammer_import_type(nativecontext.ITypiaContext{}, nativecontext.ImportProgrammer_TypeProps{Name: factory.NewIdentifier("AliasNode")}) == nil {
		t.Fatal("node import type fallback returned nil")
	}
	if notationGeneralProgrammer_import_type(importerContext, nativecontext.ImportProgrammer_TypeProps{Name: "Imported"}) == nil {
	...
	}{Config: notationConfig, Context: nativecontext.ITypiaContext{}, Functor: functor, Collection: collection})) == 0 {
