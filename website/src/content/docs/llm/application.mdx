...
Because it describes the entire collection, agent frameworks surface it as a system instruction: [`@typia/mcp`](/docs/utilization/mcp)'s `createMcpServer` ships this `description` verbatim as the MCP server's handshake `instructions`, so the class JSDoc _is_ the agent's usage contract. Other consumers may treat it as a plain description. The property is omitted when the source type carries no JSDoc.
