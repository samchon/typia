...
export const test_createValidateParse_ObjectLiteralType = _test_validateParse("ObjectLiteralType", ObjectLiteralType.generate, (input: string): typia.IValidation<typia.Primitive<ObjectLiteralType>> => { const validate = (input: any): typia.IValidation<ObjectLiteralType> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectLiteralType.SPOILERS);
