...
export const test_createValidateParse_ArrayRepeatedUnion = _test_validateParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: string): typia.IValidation<typia.Primitive<ArrayRepeatedUnion>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedUnion> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayRepeatedUnion.SPOILERS);
