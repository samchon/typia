...
export const test_assertClone_TagAtomicUnion = _test_assertClone("TagAtomicUnion", TagAtomicUnion.generate, (input) => ((input: any): typia.Primitive<Array<TagAtomicUnion.Type>> => { const assert = (input: any): Array<TagAtomicUnion.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagAtomicUnion.SPOILERS);
