...
export const test_assertParse_TemplateUnion = _test_assertParse("TemplateUnion", TemplateUnion.generate, (input) => ((input: string): typia.Primitive<TemplateUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TemplateUnion.SPOILERS);
