...
export const test_stringify_ArrayRecursiveUnionImplicit = _test_stringify("ArrayRecursiveUnionImplicit", ArrayRecursiveUnionImplicit.generate, (input) => ((input: ArrayRecursiveUnionImplicit): string => {
    ...
    const $so0 = (input: any) => `{"id":${$number(input.id)},"name":${$string(input.name)},"path":${$string(input.path)},"children":${`[${input.children.map((elem: any) => $su0(elem)).join(",")}]`}}`;
    ...
    const $so2 = (input: any) => `{"id":${$number(input.id)},"name":${$string(input.name)},"path":${$string(input.path)},"width":${$number(input.width)},"height":${$number(input.height)},"url":${$string(input.url)},"size":${$number(input.size)}}`;
    const $so3 = (input: any) => `{"id":${$number(input.id)},"name":${$string(input.name)},"path":${$string(input.path)},"size":${$number(input.size)},"content":${$string(input.content)}}`;
    const $so4 = (input: any) => `{"id":${$number(input.id)},"name":${$string(input.name)},"path":${$string(input.path)},"size":${$number(input.size)},"count":${$number(input.count)}}`;
    const $so5 = (input: any) => `{"id":${$number(input.id)},"name":${$string(input.name)},"path":${$string(input.path)},"target":${$su0(input.target)}}`;
    const $su0 = (input: any) => (() => {
        if (undefined !== input.access)
            return $so1(input);
        if (undefined !== input.width)
