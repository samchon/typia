...
export const test_createValidateParse_DynamicComposite = _test_validateParse("DynamicComposite", DynamicComposite.generate, (input: string): typia.IValidation<typia.Primitive<DynamicComposite>> => { const validate = (input: any): typia.IValidation<DynamicComposite> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, DynamicComposite.SPOILERS);
