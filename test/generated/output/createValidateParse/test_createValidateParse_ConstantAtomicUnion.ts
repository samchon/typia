...
export const test_createValidateParse_ConstantAtomicUnion = _test_validateParse("ConstantAtomicUnion", ConstantAtomicUnion.generate, (input: string): typia.IValidation<typia.Primitive<ConstantAtomicUnion>> => { const validate = (input: any): typia.IValidation<ConstantAtomicUnion> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ConstantAtomicUnion.SPOILERS);
