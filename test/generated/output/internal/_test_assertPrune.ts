...
export function _test_assertPrune<T>(name: string, generator: () => T, pruner: (input: T) => T, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        ...
        for (const spoil of spoilers ?? []) {
            ...
            throw new Error(`Bug on typia.assertPrune(): failed to detect error on the ${name} type.`);
        }
    };
}
