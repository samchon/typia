...
export const test_isParse_TemplateUnion = _test_isParse("TemplateUnion", TemplateUnion.generate, (input) => ((input: any): typia.Primitive<TemplateUnion> => { const is = (input: any): input is TemplateUnion => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), TemplateUnion.SPOILERS);
