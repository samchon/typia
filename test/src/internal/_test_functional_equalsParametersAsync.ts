...
export const _test_functional_equalsParametersAsync =
  (name: string) =>
    ...
    const task = async (callback: (input: T) => [T, T]) => {
      {
        ...
        if ((await validate(async () => y)(x)) === null)
          throw new Error(
            `Bug on await typia.functional.equalsParameters(): failed to understand the ${name} type.`,
          );
      }
      ...
      if ((await validate(async () => y)(x)) !== null)
        throw new Error(
          `Bug on await typia.functional.equalsParameters(): failed to detect surplus property on the ${name} type.`,
        );
    };
