...
export const test_assertParse_ObjectUnionDouble = _test_assertParse("ObjectUnionDouble", ObjectUnionDouble.generate, (input) => ((input: string): typia.Primitive<ObjectUnionDouble> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectUnionDouble.SPOILERS);
