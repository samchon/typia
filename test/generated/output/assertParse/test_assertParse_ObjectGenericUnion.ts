...
export const test_assertParse_ObjectGenericUnion = _test_assertParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input) => ((input: string): typia.Primitive<ISaleEntireArticle> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectGenericUnion.SPOILERS);
