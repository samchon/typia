...
export const test_assertStringify_DynamicConstant = _test_assertStringify("DynamicConstant", DynamicConstant.generate, (input) => ((input: any): string => { const assert = (input: any): {    a: number;    b: number;    c: number;    d: number;} => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), DynamicConstant.SPOILERS);
