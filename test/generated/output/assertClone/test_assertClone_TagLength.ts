...
export const test_assertClone_TagLength = _test_assertClone("TagLength", TagLength.generate, (input) => ((input: any): typia.Primitive<Array<TagLength.Type>> => { const assert = (input: any): Array<TagLength.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagLength.SPOILERS);
