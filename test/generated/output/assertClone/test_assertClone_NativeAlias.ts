...
export const test_assertClone_NativeAlias = _test_assertClone("NativeAlias", NativeAlias.generate, (input) => ((input: any): typia.Primitive<NativeAlias> => { const assert = (input: any): NativeAlias => {
...
}; assert(input); const output = clone(input); return output; })(input), NativeAlias.SPOILERS);
