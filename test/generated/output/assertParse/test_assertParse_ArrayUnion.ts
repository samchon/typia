...
export const test_assertParse_ArrayUnion = _test_assertParse("ArrayUnion", ArrayUnion.generate, (input) => ((input: string): typia.Primitive<ArrayUnion> => { const assert = (input: any): ArrayUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayUnion.SPOILERS);
