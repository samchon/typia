...
각 mega-cycle 은 다음 두 층의 산출을 냅니다.

1. **회의록 (minutes/)** — 6 역할이 서로 발언하는 **대화 로그**. session 별로 분리. 이것이 토의의 실체.
2. **최종 규약 (01-*.md ~ 06-*.md)** — 회의 합의를 반영한 각 역할의 규약 문서. 회의록의 결론이 응축된 형태.

그 밖에 **cycles/** 폴더에는 각 역할이 회의 **전후로 제출**한 서면 자료 (포지션 / 비판서 / 개정안) 가 보관됩니다 — reference 용.

## 진행 상태

- [x] **Mega-cycle 1** — 초안 시스템 구축 (9 세션 완료)
  - [x] Sub-1 초안 6편 (6,275 줄)
  - [x] Sub-2 교차 리뷰 (2,429 줄)
  - [x] Sub-3 개정 (6,971 줄)
  - [x] Sub-4 통합 감수 (session-08)
  - [x] Sub-5 최종 확정 (session-09 + 01~06-*.md)
- [ ] **Mega-cycle 2** — 허점 공격
...
| v2 | 6 mega-cycle × 5 sub-cycle 체제 확장 — 끝장 연속 토론 |
| **v3 (현재)** | 산출물을 **회의록 (minutes/, dialog 형태)** 중심으로 재편. cycles/ 는 reference 서면으로 격하. Mega-1 완료. |
