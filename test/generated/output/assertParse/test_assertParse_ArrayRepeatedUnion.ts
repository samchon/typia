...
export const test_assertParse_ArrayRepeatedUnion = _test_assertParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input) => ((input: string): typia.Primitive<ArrayRepeatedUnion> => { const assert = (input: any): ArrayRepeatedUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayRepeatedUnion.SPOILERS);
