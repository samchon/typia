...
export function _test_assertStringify<T>(name: string, generator: () => T, converter: (input: T) => string, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        const data: T = generator();
        const optimized: string = converter(data);
        if (predicate(data, optimized) === false) {
            throw new Error(`Bug on typia.assertStringify(): failed to understand the ${name} type.`);
        }
        for (const spoil of spoilers ?? []) {
            ...
            throw new Error(`Bug on typia.assertStringify(): failed to detect error on the ${name} type.`);
        }
    };
}
