...
export const test_functional_validateFunction_SetAlias =
  _test_functional_validateFunction("SetAlias")(SetAlias)(
    (p: (input: SetAlias) => SetAlias) =>
      (input: SetAlias): import("typia").IValidation<SetAlias> => {
        const paramResults = [
        ...
        const result = ((input: any): typia.IValidation<SetAlias> => {
          const errors = [] as any[];
          const __is = (input: any): input is SetAlias => {
            const $io0 = (input: any): boolean =>
              input.booleans instanceof Set &&
