...
export function _test_equals<T extends object>(name: string, generator: () => T, validator: (input: T) => boolean, repeat: number = 1): () => void {
    return () => {
        if (validator(generator()) === false)
            throw new Error(`Bug on typia.equals(): failed to understand the ${name} type.`);
        while (repeat-- > 0) {
            const elem: T = generator();
            if (spoil(elem) && validator(elem))
                throw new Error(`Bug on typia.equals(): failed to detect error on the ${name} type.`);
        }
    };
}
function spoil(input: any): boolean {
    if (Array.isArray(input))
        return spoil_array(input);
