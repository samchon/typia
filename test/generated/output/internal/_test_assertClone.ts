...
export function _test_assertClone<T>(name: string, generator: () => T, cloner: (input: T) => Primitive<T>, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        ...
        if (primitive_equal_to(replica, cloned) === false) {
            throw new Error(`Bug on TSON.assertClone(): failed to understand the ${name} type.`);
        }
        for (const spoil of spoilers || []) {
            ...
            throw new Error(`Bug on TSON.assertClone(): failed to detect error on the ${name} type.`);
        }
    };
}
