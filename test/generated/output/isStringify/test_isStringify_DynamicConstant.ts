...
export const test_isStringify_DynamicConstant = _test_isStringify("DynamicConstant", DynamicConstant.generate, (input) => ((input: {    a: number;    b: number;    c: number;    d: number;}): string | null => { const is = (input: any): input is {    a: number;    b: number;    c: number;    d: number;} => {
