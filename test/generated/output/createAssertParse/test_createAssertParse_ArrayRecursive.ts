...
export const test_createAssertParse_ArrayRecursive = _test_assertParse("ArrayRecursive", ArrayRecursive.generate, (input: string): typia.Primitive<ArrayRecursive> => { const assert = (input: any): ArrayRecursive => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ArrayRecursive.SPOILERS);
