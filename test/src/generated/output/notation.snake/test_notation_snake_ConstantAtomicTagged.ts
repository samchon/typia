...
export const test_notation_validateSnake_ConstantAtomicTagged =
  _test_notation_validateGeneral("ConstantAtomicTagged")<ConstantAtomicTagged>(
    ...
    convert: (input) =>
      ((
        input: any,
      ): typia.IValidation<typia.SnakeCase<ConstantAtomicTagged>> => {
        const validate = (
          input: any,
    ...
    assert: (input: any): typia.SnakeCase<ConstantAtomicTagged> => {
      const __is = (
        input: any,
      ): input is typia.SnakeCase<ConstantAtomicTagged> => {
        const $io0 = (input: any): boolean =>
          ("latest" === input.id ||
            ("string" === typeof input.id &&
              /^(?:urn:uuid:)?[0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12}$/i.test(
