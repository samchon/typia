...
export const test_createAssertParse_ObjectUnionComposite = _test_assertParse("ObjectUnionComposite", ObjectUnionComposite.generate, (input: string): typia.Primitive<ObjectUnionComposite> => { const assert = (input: any): ObjectUnionComposite => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectUnionComposite.SPOILERS);
