...
export const test_createAssertParse_TupleRestArray = _test_assertParse("TupleRestArray", TupleRestArray.generate, (input: string): typia.Primitive<TupleRestArray> => { const assert = (input: any): TupleRestArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TupleRestArray.SPOILERS);
