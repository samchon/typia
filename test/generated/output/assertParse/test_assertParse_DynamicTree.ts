...
export const test_assertParse_DynamicTree = _test_assertParse("DynamicTree", DynamicTree.generate, (input) => ((input: string): typia.Primitive<DynamicTree> => { const assert = (input: any): DynamicTree => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicTree.SPOILERS);
