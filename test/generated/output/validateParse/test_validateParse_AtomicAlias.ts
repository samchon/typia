...
export const test_validateParse_AtomicAlias = _test_validateParse("AtomicAlias", AtomicAlias.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<AtomicAlias>> => { const validate = (input: any): typia.IValidation<AtomicAlias> => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), AtomicAlias.SPOILERS);
