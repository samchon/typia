...
export const test_assertClone_ClassMethod = _test_assertClone("ClassMethod", ClassMethod.generate, (input) => ((input: any): typia.Primitive<ClassMethod.Animal> => { const assert = (input: any): ClassMethod.Animal => {
...
}; assert(input); const output = clone(input); return output; })(input), ClassMethod.SPOILERS);
