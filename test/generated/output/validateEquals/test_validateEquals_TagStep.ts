...
export const test_validateEquals_TagStep = _test_validateEquals("TagStep", TagStep.generate, (input) => ((input: any): typia.IValidation<Array<TagStep.Type>> => {
    const __is = (input: any, _exceptionable: boolean = true): input is Array<TagStep.Type> => {
        const $io0 = (input: any, _exceptionable: boolean = true): boolean => "number" === typeof input.exclusiveMinimum && 0 === input.exclusiveMinimum % 5 - 3 && 3 < input.exclusiveMinimum && ("number" === typeof input.minimum && 0 === input.minimum % 5 - 3 && 3 <= input.minimum) && ("number" === typeof input.range && 0 === input.range % 5 - 0 && 0 < input.range && 100 > input.range) && ("number" === typeof input.multipleOf && 0 === input.multipleOf % 5 && 3 <= input.multipleOf && 99 >= input.multipleOf) && (4 === Object.keys(input).length || Object.keys(input).every(key => {
            if (["exclusiveMinimum", "minimum", "range", "multipleOf"].some(prop => key === prop))
                return true;
