...
export const test_createRandom_ObjectLiteralProperty = _test_random("ObjectLiteralProperty", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<ObjectLiteralProperty> => {
    const $generator = (typia.createRandom as any).generator;
    const $ro0 = (_recursive: boolean = false, _depth: number = 0): any => ({
        "something-interesting-do-you-want?": (generator?.customs ?? $generator.customs)?.string?.([]) ?? (generator?.string ?? $generator.string)(),
        "or-something-crazy-do-you-want?": (generator?.customs ?? $generator.customs)?.string?.([]) ?? (generator?.string ?? $generator.string)()
    });
    return $ro0();
}, (input: any): typia.Primitive<ObjectLiteralProperty> => {
    const $guard = (typia.createAssert as any).guard;
    const __is = (input: any): input is typia.Primitive<ObjectLiteralProperty> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<ObjectLiteralProperty> => {
