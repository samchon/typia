...
export const test_assertClone_DynamicArray = _test_assertClone("DynamicArray", DynamicArray.generate, (input) => ((input: any): typia.Primitive<DynamicArray> => { const assert = (input: any): DynamicArray => {
...
}; assert(input); const output = clone(input); return output; })(input), DynamicArray.SPOILERS);
