...
export const test_notation_createValidatePascal_AtomicAlias =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.PascalCase<AtomicAlias>> => {
      ...
      const general = (input: AtomicAlias): typia.PascalCase<AtomicAlias> => {
    ...
    assert: (input: any): typia.PascalCase<AtomicAlias> => {
      const __is = (input: any): input is typia.PascalCase<AtomicAlias> => {
        return (
          Array.isArray(input) &&
          input.length === 3 &&
          "boolean" === typeof input[0] &&
