...
export const test_assertStringify_ClassGetter = _test_assertStringify("ClassGetter", ClassGetter.generate, (input) => ((input: any): string => { const assert = (input: any): ClassGetter.Person => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), ClassGetter.SPOILERS);
