...
export const test_createValidateParse_AtomicAlias = _test_validateParse("AtomicAlias", AtomicAlias.generate, (input: string): typia.IValidation<typia.Primitive<AtomicAlias>> => { const validate = (input: any): typia.IValidation<AtomicAlias> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, AtomicAlias.SPOILERS);
