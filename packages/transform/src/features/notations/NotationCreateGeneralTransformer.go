package notations

import (
	corenotations "github.com/samchon/typia/packages/core/src/programmers/notations"
	internal "github.com/samchon/typia/packages/transform/src/internal"
)

var NotationCreateGeneralTransformer = notationCreateGeneralTransformerNamespace{}

type notationCreateGeneralTransformerNamespace struct{}

func (notationCreateGeneralTransformerNamespace) Transform(rename string) internal.Task {
	return func(call internal.ITypiaCallExpression) string {
		return corenotations.NotationGeneralProgrammer.Write(rename, call.TypeText)
	}
}
