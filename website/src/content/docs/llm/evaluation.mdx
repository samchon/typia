...
Put `@probability` on the decision property or enum member, not on a `type`, `interface`, or enum declaration. The compiler rejects declaration-level comments because they cannot unambiguously select a decision threshold.
