...
`byte` validates one complete padded base64 value, `iri-reference` accepts both absolute IRIs and relative references, and `date` / `date-time` reject impossible Gregorian calendar dates.
