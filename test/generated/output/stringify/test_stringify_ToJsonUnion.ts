...
export const test_stringify_ToJsonUnion = _test_stringify("ToJsonUnion", ToJsonUnion.generate, (input) => ((input: ToJsonUnion): string => {
    ...
    const $so0 = (input: any) => `{"id":${$number(input.id)},"mobile":${$string(input.mobile)},"name":${$string(input.name)}}`;
