...
export const test_createValidateParse_DynamicArray = _test_validateParse("DynamicArray", DynamicArray.generate, (input: string): typia.IValidation<typia.Primitive<DynamicArray>> => { const validate = (input: any): typia.IValidation<DynamicArray> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; }, DynamicArray.SPOILERS);
