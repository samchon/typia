...
export const test_validateParse_ObjectUnionNonPredictable = _test_validateParse("ObjectUnionNonPredictable", ObjectUnionNonPredictable.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ObjectUnionNonPredictable>> => { const validate = (input: any): typia.IValidation<ObjectUnionNonPredictable> => {
    const errors = [] as any[];
    ...
    ((input: any, _path: string, _exceptionable: boolean): input is ObjectUnionNonPredictable => {
        ...
        const $vo5 = (input: any, _path: string, _exceptionable: boolean) => ["number" === typeof input.value || $report(_exceptionable, {
                path: _path + ".value",
                expected: "number",
                value: input.value
            })].every((flag: boolean) => flag);
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ObjectUnionNonPredictable.SPOILERS);
