...
>
> **답: 그렇다.** 이 문서는 현행 9개 패키지와 신규 전환 드라이버 `@typia/ttsc`의 Go/TS 결정과 근거를 정리한다.
...
**왜 Go**: 빌드 타임에 typescript-go **내부**에서 실행되어야 한다. Node 프로세스 spawn은 파일당 IPC 수십~수백 ms 오버헤드. typescript-go가 Go면 같은 프로세스의 Go 코드가 접근해야 한다.
