...
export const test_isStringify_TagMatrix = _test_isStringify("TagMatrix", TagMatrix.generate, (input) => ((input: TagMatrix): string | null => { const is = (input: any): input is TagMatrix => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), TagMatrix.SPOILERS);
