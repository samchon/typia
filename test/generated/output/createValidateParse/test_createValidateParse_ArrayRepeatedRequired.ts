...
export const test_createValidateParse_ArrayRepeatedRequired = _test_validateParse("ArrayRepeatedRequired", ArrayRepeatedRequired.generate, (input: string): typia.IValidation<typia.Primitive<ArrayRepeatedRequired>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedRequired> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayRepeatedRequired.SPOILERS);
