...
export const test_validateParse_ObjectLiteralProperty = _test_validateParse("ObjectLiteralProperty", ObjectLiteralProperty.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ISomething>> => { const validate = (input: any): typia.IValidation<ISomething> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ObjectLiteralProperty.SPOILERS);
