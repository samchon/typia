...
export const test_assertParse_DefinitionJsonValue = _test_assertParse("DefinitionJsonValue", DefinitionJsonValue.generate, (input) => ((input: string): typia.Primitive<DefinitionJsonValue> => { const assert = (input: any): DefinitionJsonValue => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DefinitionJsonValue.SPOILERS);
