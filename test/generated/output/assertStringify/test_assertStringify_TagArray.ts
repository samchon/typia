...
export const test_assertStringify_TagArray = _test_assertStringify("TagArray", TagArray.generate, (input) => ((input: TagArray): string => { const assert = (input: any) => {
    ...
    ((input: any, _path: string, _exceptionable: boolean): input is TagArray => {
        ...
        })) && input.minItems.every((elem: any, _index3: number) => "number" === typeof elem && !Number.isNaN(elem) && 3 <= elem || $guard(_exceptionable, {
        ...
        })) && input.maxItems.every((elem: any, _index4: number) => "string" === typeof elem && 7 >= elem.length || "number" === typeof elem && !Number.isNaN(elem) && 7 >= elem || $guard(_exceptionable, {
...
}; return stringify(assert(input)); })(input), TagArray.SPOILERS);
