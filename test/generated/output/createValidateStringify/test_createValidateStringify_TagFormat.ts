...
export const test_createValidateStringify_TagFormat = _test_validateStringify("TagFormat", TagFormat.generate, (input: TagFormat): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<TagFormat> => {
    ...
    const $is_datetime = (typia.createValidateStringify as any).is_datetime;
    if (false === __is(input))
    ...
    output.data = stringify(input); return output; }, TagFormat.SPOILERS);
