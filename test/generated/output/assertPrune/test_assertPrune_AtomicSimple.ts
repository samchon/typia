...
export const test_assertPrune_AtomicSimple = _test_assertPrune("AtomicSimple", AtomicSimple.generate, (input) => ((input: any): [boolean, number, string] => { const assert = (input: any): [boolean, number, string] => {
...
}; assert(input); prune(input); return input; })(input), AtomicSimple.SPOILERS);
