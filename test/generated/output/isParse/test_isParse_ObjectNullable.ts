...
export const test_isParse_ObjectNullable = _test_isParse("ObjectNullable", ObjectNullable.generate, (input) => ((input: any): typia.Primitive<ObjectNullable> => { const is = (input: any): input is ObjectNullable => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ObjectNullable.SPOILERS);
