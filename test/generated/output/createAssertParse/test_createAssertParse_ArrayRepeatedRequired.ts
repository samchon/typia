...
export const test_createAssertParse_ArrayRepeatedRequired = _test_assertParse("ArrayRepeatedRequired", ArrayRepeatedRequired.generate, (input: string): typia.Primitive<ArrayRepeatedRequired> => { const assert = (input: any): ArrayRepeatedRequired => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ArrayRepeatedRequired.SPOILERS);
