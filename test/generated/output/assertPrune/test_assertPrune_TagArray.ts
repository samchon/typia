...
export const test_assertPrune_TagArray = _test_assertPrune("TagArray", TagArray.generate, (input) => ((input: any): Array<TagArray.Type> => { const assert = (input: any): Array<TagArray.Type> => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; })(input), TagArray.SPOILERS);
