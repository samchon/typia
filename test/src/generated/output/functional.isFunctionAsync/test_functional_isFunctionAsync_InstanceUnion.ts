...
export const test_functional_isFunctionAsync_InstanceUnion =
  _test_functional_isFunctionAsync("InstanceUnion")(InstanceUnion)(
    (p: (input: InstanceUnion) => Promise<InstanceUnion>) =>
      async (input: InstanceUnion): Promise<InstanceUnion | null> => {
        if (
          false ===
        ...
        return ((input: any): input is InstanceUnion => {
          const $ip0 = (input: any) => {
            const array = input;
            const tuplePredicators = [
              [
