...
export const test_assertClone_SetAlias = _test_assertClone("SetAlias", SetAlias.generate, (input) => ((input: any): typia.Primitive<SetAlias> => { const assert = (input: any): SetAlias => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is SetAlias => {
...
}; assert(input); const output = clone(input); return output; })(input), SetAlias.SPOILERS);
