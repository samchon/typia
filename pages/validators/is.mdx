...
If the instance is following type `T`, `true` value would be returned. Otherwise, `false` would be returned.

For reference, if you'd used other competitive validator libraries like `zod` or `class-validator`, you may found that `typia` does not require extra schema definition. Yeah, `typia` needs only pure TypeScript type. `typia` can analyze TypeScript type in the compiiation level, and enable to write the optimal validation code like below:
...
When target instance is following the type `T`, therefore `true` value be returned, `typia.is<T>()` function automatically casts the instance to the type `T`. Threfore, you can utilize the `typia.is<T>()` function for safe type casting tool like below:
...
Comparing `typia.is<T>()` function with competitive libraries, maximum 20,000x faster than others. 

Furthremore, only `typia` can validate complicate union types.
