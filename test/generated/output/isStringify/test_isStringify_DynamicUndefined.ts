...
export const test_isStringify_DynamicUndefined = _test_isStringify("DynamicUndefined", DynamicUndefined.generate, (input) => ((input: DynamicUndefined): string | null => { const is = (input: any): input is DynamicUndefined => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), DynamicUndefined.SPOILERS);
