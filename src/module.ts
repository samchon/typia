...
export function createStringifier<T>(
    ...
    if (!closure) halt("createStringifier");
...
export function createApplication(
    ...
    if (!closure) halt("createApplication");
    return closure();
}
...
function halt(name: string): never {
    throw new Error(
        `Error on TSON.${name}(): no transform has been configured. Configure the "tsconfig.json" file following the README - https://github.com/samchon/typescript-json`,
    );
}
