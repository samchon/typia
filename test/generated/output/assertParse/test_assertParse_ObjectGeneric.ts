...
export const test_assertParse_ObjectGeneric = _test_assertParse("ObjectGeneric", ObjectGeneric.generate, (input) => ((input: string): typia.Primitive<ObjectGeneric> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectGeneric.SPOILERS);
