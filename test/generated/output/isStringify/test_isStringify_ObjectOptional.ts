...
export const test_isStringify_ObjectOptional = _test_isStringify("ObjectOptional", ObjectOptional.generate, (input) => ((input: ObjectOptional): string | null => { const is = (input: any): input is ObjectOptional => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), ObjectOptional.SPOILERS);
