...
export const test_createValidateParse_ArrayRepeatedNullable = _test_validateParse("ArrayRepeatedNullable", ArrayRepeatedNullable.generate, (input: string): typia.IValidation<typia.Primitive<ArrayRepeatedNullable>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedNullable> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayRepeatedNullable.SPOILERS);
