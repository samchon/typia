...
export const test_createAssertPrune_ObjectUndefined = _test_assertPrune("ObjectUndefined", ObjectUndefined.generate, (input: any): ObjectUndefined => { const assert = (input: any): ObjectUndefined => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, ObjectUndefined.SPOILERS);
