...
export const test_functional_validateFunctionAsync_TypeTagType =
  _test_functional_validateFunctionAsync("TypeTagType")(TypeTagType)(
    (p: (input: TypeTagType) => Promise<TypeTagType>) =>
      async (
        input: TypeTagType,
      ): Promise<import("typia").IValidation<TypeTagType>> => {
        const paramResults = [
        ...
        const result = ((input: any): typia.IValidation<TypeTagType> => {
          const errors = [] as any[];
          const __is = (input: any): input is TypeTagType => {
            const $io0 = (input: any): boolean =>
              Array.isArray(input.value) &&
