...
func TestJoinerBranchCoverage(t *testing.T) {
	...
	programmer.disableDeclare = true
	if len(programmer.Declare()) != 0 || len(programmer.DeclareUnions()) != 0 {
		t.Fatal("disabled function programmer should not declare")
	}
	if stringifyJoiner_internal(nativecontext.ITypiaContext{Importer: helperImporter{}}, "jsonStringifyTail") == nil {
		t.Fatal("stringify internal importer branch returned nil")
	}
}

type helperImporter struct{}

func (helperImporter) Internal(name string) *shimast.Node {
