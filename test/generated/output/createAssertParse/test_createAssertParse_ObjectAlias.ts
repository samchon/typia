...
export const test_createAssertParse_ObjectAlias = _test_assertParse("ObjectAlias", ObjectAlias.generate, (input: string): typia.Primitive<ObjectAlias> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ObjectAlias.SPOILERS);
