...
/**
 * Tests that findJsonStart properly skips comments in junk prefix
 * even when those comments contain `{` or `[` characters.
 *
...
export const test_llm_json_parse_lenient_findJsonStart_comment_skip =
  (): void => {
    ...
    if (r1.success)
      TestValidator.equals("sl-brace-data", r1.data, { key: 1 });

    // Single-line comment with [ in junk prefix
    const r2 = LlmJson.parse("// items [0] [1]\n[1, 2, 3]");
    TestValidator.equals("sl-bracket-success", r2.success, true);
    if (r2.success)
      TestValidator.equals("sl-bracket-data", r2.data, [1, 2, 3]);
    ...
    if (r3.success)
      TestValidator.equals("ml-brace-data", r3.data, { key: 1 });

    // Multi-line comment with [ in junk prefix
    const r4 = LlmJson.parse("/* see [0, 1, 2] */ [10, 20]");
    TestValidator.equals("ml-bracket-success", r4.success, true);
    if (r4.success)
      TestValidator.equals("ml-bracket-data", r4.data, [10, 20]);
