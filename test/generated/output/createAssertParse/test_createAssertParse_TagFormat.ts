...
export const test_createAssertParse_TagFormat = _test_assertParse("TagFormat", TagFormat.generate, (input: string): typia.Primitive<TagFormat> => { const assert = (input: any): TagFormat => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TagFormat.SPOILERS);
