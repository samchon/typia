...
export const test_createIsStringify_ObjectPrimitive = _test_isStringify("ObjectPrimitive", ObjectPrimitive.generate, (input: ObjectPrimitive): string | null => { const is = (input: any): input is ObjectPrimitive => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, ObjectPrimitive.SPOILERS);
