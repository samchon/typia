...
export const test_assertPrune_TagMatrix = _test_assertPrune("TagMatrix", TagMatrix.generate, (input) => ((input: any): TagMatrix => { const assert = (input: any): TagMatrix => {
    ...
    if ("object" === typeof input && null !== input)
        $po0(input);
}; assert(input); prune(input); return input; })(input), TagMatrix.SPOILERS);
