...
export const test_assertParse_ArrayRecursiveUnionExplicit = _test_assertParse("ArrayRecursiveUnionExplicit", ArrayRecursiveUnionExplicit.generate, (input) => ((input: string): typia.Primitive<ArrayRecursiveUnionExplicit> => { const assert = (input: any): ArrayRecursiveUnionExplicit => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayRecursiveUnionExplicit.SPOILERS);
