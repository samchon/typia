...
export const test_createValidateParse_TagCustom = _test_validateParse("TagCustom", TagCustom.generate, (input: string): typia.IValidation<typia.Primitive<TagCustom>> => { const validate = (input: any): typia.IValidation<TagCustom> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, TagCustom.SPOILERS);
