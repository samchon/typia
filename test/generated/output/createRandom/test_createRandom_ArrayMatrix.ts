...
export const test_createRandom_ArrayMatrix = _test_random("ArrayMatrix", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<ArrayMatrix> => {
    const $generator = (typia.createRandom as any).generator;
    return (generator?.array ?? $generator.array)(() => (generator?.array ?? $generator.array)(() => (generator?.array ?? $generator.array)(() => (generator?.customs ?? $generator.customs)?.number?.([]) ?? (generator?.number ?? $generator.number)(0, 100))));
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<ArrayMatrix> => {
