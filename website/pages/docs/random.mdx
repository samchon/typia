---
title: Guide Documents > random() function
---
import { Tabs } from "nextra/components";

import RemoteSource from "../../src/components/RemoteSource";

## `random()` function

<Tabs items={[
  <code>typia</code>, 
  <code>IRandomGenerator.ts</code>, 
  <code>Customizable.ts</code>,
  <code>Resolved.ts</code>
]}>
  <Tabs.Tab>
```typescript copy
export function random<T>(g?: IRandomGenerator): Resolved<T>;
```
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/IRandomGenerator.ts" 
      filename="typia/IRandomGenerator.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/typings/Customizable.ts" 
      filename="typia/Customizable.ts"
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/Resolved.ts" 
      filename="typia/Resolved.ts"
      showLineNumbers />
  </Tabs.Tab>
</Tabs>

You can make every random data just by calling `typia.random<T>()` function.

When you call the `typia.random<T>()` function, `typia` will analyze your type `T`, and writes optimal random generation code for the type `T`, in the compilation level. This is called AOT (Ahead of Time) compilation, and you may understand what it is just by reading below example code. 

<Tabs items={['TypeScript Source Code', 'Compiled JavaScript File']}>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/src/random/random.ts" 
      filename="examples/src/random/random.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/bin/random/random.js" 
      filename="examples/bin/random/random.js" 
      showLineNumbers />
  </Tabs.Tab>
</Tabs>

## Reusable function

<Tabs items={[
  <code>typia</code>, 
  <code>IRandomGenerator.ts</code>, 
  <code>Customizable.ts</code>,
  <code>Resolved.ts</code>,
]}>
  <Tabs.Tab>
```typescript copy
export function createRandom<T>(): (g?: IRandomGenerator) => Resolved<T>;
```
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/IRandomGenerator.ts" 
      filename="typia/IRandomGenerator.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/typings/Customizable.ts" 
      filename="typia/Customizable.ts"
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/Resolved.ts" 
      filename="typia/Resolved.ts"
      showLineNumbers />
  </Tabs.Tab>
</Tabs>

## Special Tags

Runtime validators of `typia` provides additional type checking logic through [Type Tags](./validators/tags#type-tags) and [Comment Tags](./validators/tags#comment-tags). `typia.random<T>()` function also like that. `typia.random<T>()` function can utilize those tags to specialize the behavior of random data generation.

For reference, whether you choose [Type Tags](./validators/tags#type-tags) or [Comment Tags](./validators/tags#comment-tags). `typia.random<T>()`, it is not a matter for `typia.random<T>()` function. Below two TypeScript codes are generating exactly same JavaScript code. Therefore, you can choose whatever you want considering your preference.

<Tabs items={[
  'TypeScript (Type Tags)', 
  'TypeScript (Comment Tags)', 
  'Compiled JavaScript File'
]}>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/src/random/random-tag.ts" 
      filename="examples/src/random-tag.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/src/random/random-tag-comment.ts" 
      filename="examples/src/random-tag-comment.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/bin/random/random-tag.js" 
      filename="examples/bin/random-tag.js" 
      showLineNumbers />
  </Tabs.Tab>
</Tabs>

## Customization

<Tabs items={[
  <code>typia</code>, 
  <code>IRandomGenerator.ts</code>, 
  <code>Customizable.ts</code>,
  <code>Resolved.ts</code>,
]}>
  <Tabs.Tab>
```typescript copy
export function random<T>(g?: IRandomGenerator): Resolved<T>;
```
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/IRandomGenerator.ts" 
      filename="typia/IRandomGenerator.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/typings/Customizable.ts" 
      filename="typia/Customizable.ts"
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/src/Resolved.ts" 
      filename="typia/Resolved.ts"
      showLineNumbers />
  </Tabs.Tab>
</Tabs>

You can add custom type tags for random data generation.

As above `IRandomGenerator.CustomMap` has a little bit complicate type, it may hard to understand for newcomers. However, such newcomers may easily understand, how to customize the random generation, just by reading the following example.

Just define custom type tags like below, then everything would be done. 

For reference, when defining custom type tag, `typia` enforces user to define `validate` function literal for type safety. Never forget it when you define custom type tags for random generation. Such validation logic definition may enhance your random data generator logic when combining with [`typia.assert<T>()`](./validators/assert) function.

<Tabs items={['TypeScript Source Code', 'Compiled JavaScript File']}>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/src/random/random-custom.ts" 
      filename="examples/src/random-custom.ts" 
      showLineNumbers />
  </Tabs.Tab>
  <Tabs.Tab>
    <RemoteSource 
      url="https://raw.githubusercontent.com/samchon/typia/refs/heads/master/examples/bin/random/random-custom.js" 
      filename="examples/bin/random-custom.js" 
      showLineNumbers />
  </Tabs.Tab>
</Tabs>