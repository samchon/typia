...
export const test_assertParse_DynamicUnion = _test_assertParse("DynamicUnion", DynamicUnion.generate, (input) => ((input: string): typia.Primitive<DynamicUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), DynamicUnion.SPOILERS);
