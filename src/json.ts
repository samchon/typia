...
/**
 * Generates JSON Schema specifications from TypeScript types.
 *
 * Creates comprehensive JSON Schema collections that include both main schemas 
 * and reusable component definitions. Perfect for API documentation, form 
 * generation, validation libraries, or any system that needs to understand 
 * your TypeScript types at runtime.
 *
 * Supports multiple OpenAPI versions with different feature sets. Version 3.1 
 * includes modern features like tuple types, while 3.0 maintains broader compatibility.
 *
 * @example
 * ```typescript
 ...
 * // Creates JSON Schema collection with both types and shared components
 * ```
 *
 * @template Types Tuple of TypeScript types to generate schemas for
 * @template Version OpenAPI specification version (3.0 or 3.1)
 * @returns JSON Schema collection with schemas and component definitions
 */
...
/**
 * Generates a JSON Schema specification from a single TypeScript type.
 *
 * Creates a focused JSON Schema for one specific type along with any referenced 
 * component definitions. Ideal when you need schema information for a particular 
 * type without the overhead of processing multiple types at once.
 *
 * @example
 * ```typescript
 * interface User { 
 ...
 * // Creates JSON Schema for User type with Address as a referenced component
 * ```
 *
 * @template Type TypeScript type to generate schema for
 * @template Version OpenAPI specification version (3.0 or 3.1)
 * @returns JSON Schema with main schema and component definitions
 */
...
----------------------------------------------------------- */
/**
 * Parses JSON strings with runtime type validation that crashes on invalid data.
 *
 * Combines JSON.parse() with typia's ultra-fast validation. First parses the JSON, 
 * then validates it matches your TypeScript type. If validation fails, throws 
 * detailed error information showing exactly what's wrong with the data.
 *
 * Perfect for API responses, configuration files, or any JSON data where you 
 * need guarantees about the structure and types.
 *
 * @example
 * ```typescript
 * interface User { name: string; age: number; }
 ...
 * const user = typia.json.assertParse<User>(jsonStr);
 * // Returns User object or throws with detailed validation errors
 * ```
 *
 * @template T Expected type of the parsed data
 * @param input JSON string to parse and validate
 * @param errorFactory Custom error generator (optional)
 * @returns Validated and typed data from JSON
 * @throws Detailed validation error if JSON doesn't match expected type
 */
