...
export const test_createRandom_TagMatrix = _test_random("TagMatrix", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<TagMatrix> => {
    const $generator = (typia.createRandom as any).generator;
    const $ro0 = (_recursive: boolean = false, _depth: number = 0): any => ({
        matrix: (generator?.array ?? $generator.array)(() => (generator?.array ?? $generator.array)(() => (generator?.customs ?? $generator.customs)?.string?.([
            {
                name: "items",
                value: "3"
            },
        ...
        ]) ?? (generator?.uuid ?? $generator.uuid)(), 3), 3)
    });
    return $ro0();
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<TagMatrix> => {
