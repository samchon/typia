...
export const test_createAssertParse_TupleRestAtomic = _test_assertParse("TupleRestAtomic", TupleRestAtomic.generate, (input: string): typia.Primitive<TupleRestAtomic> => { const assert = (input: any): TupleRestAtomic => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TupleRestAtomic.SPOILERS);
