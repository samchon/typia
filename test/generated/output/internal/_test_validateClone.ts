...
export function _test_validateClone<T>(name: string, generator: () => T, validator: (input: T) => TSON.IValidation<TSON.Primitive<T>>, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        ...
        if (valid.success === false)
            throw new Error(`Bug on TSON.validateClone(): failed to understand the ${name} type.`);
        if (primitive_equal_to(replica, valid.data) === false) {
            throw new Error(`Bug on TSON.assertStringify(): failed to understand the ${name} type.`);
        }
        ...
        for (const spoil of spoilers || []) {
            ...
            if (valid.success === true)
                throw new Error(`Bug on TSON.validateClone(): failed to detect error on the ${name} type.`);
            TSON.assert(valid);
            expected.sort();
            valid.errors.sort((x, y) => (x.path < y.path ? -1 : 1));
            if (valid.errors.length !== expected.length ||
