...
export const test_assertParse_ObjectSimple = _test_assertParse("ObjectSimple", ObjectSimple.generate, (input) => ((input: string): typia.Primitive<IBox3D> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectSimple.SPOILERS);
