...
func TestRandomProgrammerHelperCoverage(t *testing.T) {
  ...
  if len(randomProgrammer_write_object_functions(struct {
    ...
    t.Fatal("random recursive object function writer returned no functions")
  }
  if len(randomProgrammer_write_array_functions(struct {
    Context    nativecontext.ITypiaContext
    Functor    *nativehelpers.FunctionProgrammer
    Collection *schemametadata.MetadataCollection
  }{Context: context, Functor: functor, Collection: collection})) == 0 {
    t.Fatal("random recursive array function writer returned no functions")
  }
  if len(randomProgrammer_write_tuple_functions(struct {
    Context    nativecontext.ITypiaContext
    Functor    *nativehelpers.FunctionProgrammer
    Collection *schemametadata.MetadataCollection
  }{Context: context, Functor: functor, Collection: collection})) == 0 {
    t.Fatal("random recursive tuple function writer returned no functions")
  }
