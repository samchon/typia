...
export const test_assertParse_DynamicNever = _test_assertParse("DynamicNever", DynamicNever.generate, (input) => ((input: string): typia.Primitive<DynamicNever> => { const assert = (input: any): DynamicNever => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicNever.SPOILERS);
