...
export const test_isParse_ObjectGeneric = _test_isParse("ObjectGeneric", ObjectGeneric.generate, (input) => ((input: any): typia.Primitive<ObjectGeneric> => { const is = (input: any): input is ObjectGeneric => {
    ...
    const $io2 = (input: any) => "number" === typeof input.value && ("object" === typeof input.child && null !== input.child && ("number" === typeof input.child.child_value && "number" === typeof input.child.child_next)) && (Array.isArray(input.elements) && input.elements.every((elem: any) => "object" === typeof elem && null !== elem && $io3(elem)));
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ObjectGeneric.SPOILERS);
