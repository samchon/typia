...
export const test_isParse_TagLength = _test_isParse("TagLength", TagLength.generate, (input) => ((input: any): typia.Primitive<TagLength> => { const is = (input: any): input is TagLength => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), TagLength.SPOILERS);
