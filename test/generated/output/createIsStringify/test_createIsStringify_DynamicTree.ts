...
export const test_createIsStringify_DynamicTree = _test_isStringify("DynamicTree", DynamicTree.generate, (input: DynamicTree): string | null => { const is = (input: any): input is DynamicTree => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, DynamicTree.SPOILERS);
