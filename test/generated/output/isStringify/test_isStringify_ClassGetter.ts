...
export const test_isStringify_ClassGetter = _test_isStringify("ClassGetter", ClassGetter.generate, (input) => ((input: ClassGetter.Person): string | null => { const is = (input: any): input is ClassGetter.Person => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), ClassGetter.SPOILERS);
