...
export const test_assertParse_TupleRestAtomic = _test_assertParse("TupleRestAtomic", TupleRestAtomic.generate, (input) => ((input: string): typia.Primitive<TupleRestAtomic> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TupleRestAtomic.SPOILERS);
