...
func TestRandomProgrammerHelperCoverage(t *testing.T) {
	...
	if randomProgrammer_internal(context, "helper") == nil ||
		randomProgrammer_internal(nativecontext.ITypiaContext{Importer: nativecontext.NewImportProgrammer()}, "helper") == nil {
		t.Fatal("random internal helper returned nil")
	}
	if randomProgrammer_import_type(context, nativecontext.ImportProgrammer_TypeProps{Name: "Alias"}) == nil ||
		randomProgrammer_import_type(context, nativecontext.ImportProgrammer_TypeProps{Name: factory.NewIdentifier("AliasNode")}) == nil ||
		randomProgrammer_import_type(nativecontext.ITypiaContext{Importer: nativecontext.NewImportProgrammer()}, nativecontext.ImportProgrammer_TypeProps{Name: "Alias"}) == nil {
	...
	if randomProgrammer_first(nil, emit).Text() != "undefined" ||
		randomProgrammer_first([]*shimast.Node{factory.NewIdentifier("value")}, emit).Text() != "value" {
		t.Fatal("random first helper failed")
	}
	if randomProgrammer_schema_without_items(nativeiterate.JsonSchema{"type": "array", "items": "skip"}, emit) == nil {
		t.Fatal("random schema copy helper returned nil")
	}
	if randomProgrammer_compose_atomic("string", nativeiterate.JsonSchema{"type": "string", "format": "date-time"}, emit).Internal != "randomFormatDatetime" {
		t.Fatal("date-time format should use the datetime random helper")
	}
	if randomProgrammer_compose_atomic("string", nativeiterate.JsonSchema{"type": "string", "format": "uuid"}, emit).Internal != "randomFormatUuid" ||
		randomProgrammer_compose_atomic("string", nativeiterate.JsonSchema{"type": "string", "pattern": "^[a]$"}, emit).Internal != "randomPattern" {
