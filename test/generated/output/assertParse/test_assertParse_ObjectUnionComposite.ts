...
export const test_assertParse_ObjectUnionComposite = _test_assertParse("ObjectUnionComposite", ObjectUnionComposite.generate, (input) => ((input: string): typia.Primitive<ObjectUnionComposite> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectUnionComposite.SPOILERS);
