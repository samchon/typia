...
export const test_createValidateParse_ObjectAlias = _test_validateParse("ObjectAlias", ObjectAlias.generate, (input: string): typia.IValidation<typia.Primitive<ObjectAlias>> => { const validate = (input: any): typia.IValidation<ObjectAlias> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectAlias.SPOILERS);
