...
export const test_isStringify_NativeSimple = _test_isStringify("NativeSimple", NativeSimple.generate, (input) => ((input: NativeSimple): string | null => { const is = (input: any): input is NativeSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), NativeSimple.SPOILERS);
