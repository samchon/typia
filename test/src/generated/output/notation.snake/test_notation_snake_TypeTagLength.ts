...
export const test_notation_validateSnake_TypeTagLength =
  ...
  >({
    convert: (input) =>
      ((input: any): typia.IValidation<typia.SnakeCase<TypeTagLength>> => {
    ...
    assert: (input: any): typia.SnakeCase<TypeTagLength> => {
      const __is = (input: any): input is typia.SnakeCase<TypeTagLength> => {
        const $io0 = (input: any): boolean =>
          Array.isArray(input.value) &&
          input.value.every(
            (elem: any) =>
      ...
      if (false === __is(input))
        ((
          ...
          _path: string,
          _exceptionable: boolean = true,
        ): input is typia.SnakeCase<TypeTagLength> => {
