...
export const test_createAssertStringify_DynamicUnion = _test_assertStringify("DynamicUnion", DynamicUnion.generate, (input: any): string => { const assert = (input: any): DynamicUnion => {
    ...
    return $so0(input);
}; return stringify(assert(input)); }, DynamicUnion.SPOILERS);
