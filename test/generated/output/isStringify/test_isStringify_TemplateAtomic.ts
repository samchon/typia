...
export const test_isStringify_TemplateAtomic = _test_isStringify("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: TemplateAtomic): string | null => { const is = (input: any): input is TemplateAtomic => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), TemplateAtomic.SPOILERS);
