...
export const test_validateStringify_TemplateAtomic = _test_validateStringify("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: TemplateAtomic): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<TemplateAtomic> => {
    ...
    const $so0 = (input: any) => `{"prefix":${$string(input.prefix)},"postfix":${$string(input.postfix)},"middle_string":${$string(input.middle_string)},"middle_string_empty":${$string(input.middle_string_empty)},"middle_numeric":${$string(input.middle_numeric)},"middle_boolean":${(() => {
