...
export const test_assertParse_ObjectUnionImplicit = _test_assertParse("ObjectUnionImplicit", ObjectUnionImplicit.generate, (input) => ((input: string): typia.Primitive<ObjectUnionImplicit> => { const assert = (input: any): ObjectUnionImplicit => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectUnionImplicit.SPOILERS);
