...
export function _test_validateEquals<T>(name: string, generator: () => T, assert: (input: T) => IValidation<T>, doSpoil: boolean = true): () => void {
    return () => {
        const input: T = generator();
        // EXACT TYPE
        const valid: IValidation<unknown> = assert(input);
        if (valid.success === false)
            throw new Error(`Bug on typia.validateEquals(): failed to understand the ${name} type.`);
        else if (valid.data !== input)
            throw new Error("Bug on typia.validateEquals(): failed to archive the input value.");
        typia.assert(valid);
        if (doSpoil === false)
            return;
        // EXPECTED
        ...
        if (expected.length !== solution.length ||
            ...
            throw new Error(`Bug on typia.validateEquals(): failed to detect surplus property on the ${name} type.`);
        }
    };
}
function spoil(accessors: string[], path: string, input: any): void {
    if (Array.isArray(input))
        spoil_array(accessors, path, input);
...
function spoil_object(accessors: string[], path: string, obj: any): void {
    ...
    for (const [key, value] of Object.entries(obj))
        spoil(accessors, Escaper.variable(key)
            ? `${path}.${key}`
