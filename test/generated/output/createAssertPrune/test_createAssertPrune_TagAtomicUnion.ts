...
export const test_createAssertPrune_TagAtomicUnion = _test_assertPrune("TagAtomicUnion", TagAtomicUnion.generate, (input: any): TagAtomicUnion => { const assert = (input: any): TagAtomicUnion => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, TagAtomicUnion.SPOILERS);
