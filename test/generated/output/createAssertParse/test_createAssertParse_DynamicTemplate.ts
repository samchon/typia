...
export const test_createAssertParse_DynamicTemplate = _test_assertParse("DynamicTemplate", DynamicTemplate.generate, (input: string): typia.Primitive<DynamicTemplate> => { const assert = (input: any): DynamicTemplate => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, DynamicTemplate.SPOILERS);
