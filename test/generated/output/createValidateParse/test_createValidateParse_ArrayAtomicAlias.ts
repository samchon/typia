...
export const test_createValidateParse_ArrayAtomicAlias = _test_validateParse("ArrayAtomicAlias", ArrayAtomicAlias.generate, (input: string): typia.IValidation<typia.Primitive<ArrayAtomicAlias>> => { const validate = (input: any): typia.IValidation<ArrayAtomicAlias> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayAtomicAlias.SPOILERS);
