...
export const test_assertClone_TemplateUnion = _test_assertClone("TemplateUnion", TemplateUnion.generate, (input) => ((input: any): typia.Primitive<Array<TemplateUnion.Type>> => { const assert = (input: any): Array<TemplateUnion.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TemplateUnion.SPOILERS);
