...
// returns `x` typed as `T` or throws. Phase 0 uses a plain `Error`; the full
// TypeGuardError with path threading lands in the next iteration.
...
// a drop-in for typia v12's `typia.validate<T>` result shape. Phase 0 emits
// a single top-level error rather than per-property diagnostics; a proper
// path-threaded implementation follows in the next iteration.
