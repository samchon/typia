...
export const test_isStringify_DynamicComposite = _test_isStringify("DynamicComposite", DynamicComposite.generate, (input) => ((input: DynamicComposite): string | null => { const is = (input: any): input is DynamicComposite => {
    ...
    const $io0 = (input: any) => "string" === typeof input.id && "string" === typeof input.name && Object.keys(input).every(key => {
        ...
        if (RegExp(/^(value_-?\d+\.?\d*)$/).test(key))
            return "string" === typeof value || "number" === typeof value && !Number.isNaN(value) || "boolean" === typeof value;
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), DynamicComposite.SPOILERS);
