...
export const test_assertParse_ObjectUndefined = _test_assertParse("ObjectUndefined", ObjectUndefined.generate, (input) => ((input: string): typia.Primitive<ObjectUndefined> => { const assert = (input: any): ObjectUndefined => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectUndefined.SPOILERS);
