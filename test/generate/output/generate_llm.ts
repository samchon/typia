...
export const application = typia.llm.application<IApplication, "llama">({
    separate: (schema) => LlamaTypeChecker.isString(schema) && schema.format === "date-time",
});
typia.llm.application<IApplication, "llama">({
    separate: (schema) => LlamaTypeChecker.isString(schema) && schema.format === "date-time",
} satisfies Partial<Pick<ILlmApplication.IOptions<"llama">, "separate">>);
export const controller = typia.llm.controller<IApplication, "deepseek">("company", {
    establishCompany: (props: {
        company: ICompany;
    }) => props.company,
