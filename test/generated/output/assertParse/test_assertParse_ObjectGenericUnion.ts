...
export const test_assertParse_ObjectGenericUnion = _test_assertParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input) => ((input: string): typia.Primitive<ObjectGenericUnion> => { const assert = (input: any): ObjectGenericUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectGenericUnion.SPOILERS);
