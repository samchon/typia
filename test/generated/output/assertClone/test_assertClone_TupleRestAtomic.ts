...
export const test_assertClone_TupleRestAtomic = _test_assertClone("TupleRestAtomic", TupleRestAtomic.generate, (input) => ((input: any): typia.Primitive<[boolean, number, ...string[]]> => { const assert = (input: any): [boolean, number, ...string[]] => {
...
}; assert(input); const output = clone(input); return output; })(input), TupleRestAtomic.SPOILERS);
