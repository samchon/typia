...
export const test_validateParse_TagLength = _test_validateParse("TagLength", TagLength.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TagLength>> => { const validate = (input: any): typia.IValidation<TagLength> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), TagLength.SPOILERS);
