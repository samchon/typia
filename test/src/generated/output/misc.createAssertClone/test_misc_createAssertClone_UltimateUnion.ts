...
export const test_misc_createAssertClone_UltimateUnion = _test_misc_assertClone(
  "UltimateUnion",
)<UltimateUnion>(UltimateUnion)((input: any): typia.Resolved<UltimateUnion> => {
  ...
  const clone = (input: UltimateUnion): typia.Resolved<UltimateUnion> => {
    ...
    const $cp7 = (input: any) =>
      input.map((elem: any) =>
        "object" === typeof elem && null !== elem ? $co7(elem) : (elem as any),
      );
    ...
    const $co2 = (input: any): any => ({
      ...
      text: Array.isArray(input.text) ? $cp4(input.text) : (input.text as any),
    });
