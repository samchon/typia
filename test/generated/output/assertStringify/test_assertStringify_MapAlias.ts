...
export const test_assertStringify_MapAlias = _test_assertStringify("MapAlias", MapAlias.generate, (input) => ((input: any): string => { const assert = (input: any): MapAlias => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), MapAlias.SPOILERS);
