...
export const test_createAssertParse_ObjectGenericArray = _test_assertParse("ObjectGenericArray", ObjectGenericArray.generate, (input: string): typia.Primitive<ObjectGenericArray> => { const assert = (input: any): ObjectGenericArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectGenericArray.SPOILERS);
