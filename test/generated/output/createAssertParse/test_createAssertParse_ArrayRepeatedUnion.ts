...
export const test_createAssertParse_ArrayRepeatedUnion = _test_assertParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: string): typia.Primitive<ArrayRepeatedUnion> => { const assert = (input: any): ArrayRepeatedUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ArrayRepeatedUnion.SPOILERS);
