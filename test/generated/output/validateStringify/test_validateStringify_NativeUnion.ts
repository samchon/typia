...
export const test_validateStringify_NativeUnion = _test_validateStringify("NativeUnion", NativeUnion.generate, (input) => ((input: Array<NativeUnion.Union>): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<Array<NativeUnion.Union>> => {
...
}; const stringify = (input: Array<NativeUnion.Union>): string => {
    ...
    const $so0 = (input: any): any => `{"date":${null !== input.date ? (() => {
        if ("object" === typeof input.date && "function" === typeof input.date.toJSON)
        ...
        $throws({
            expected: "(ArrayBuffer | DataView | SharedArrayBuffer | __type | unknown)",
            value: input.buffer
        });
    })()},"weak":${(() => {
