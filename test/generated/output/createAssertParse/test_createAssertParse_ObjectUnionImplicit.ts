...
export const test_createAssertParse_ObjectUnionImplicit = _test_assertParse("ObjectUnionImplicit", ObjectUnionImplicit.generate, (input: string): typia.Primitive<ObjectUnionImplicit> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ObjectUnionImplicit.SPOILERS);
