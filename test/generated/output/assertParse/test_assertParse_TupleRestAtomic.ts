...
export const test_assertParse_TupleRestAtomic = _test_assertParse("TupleRestAtomic", TupleRestAtomic.generate, (input) => ((input: string): typia.Primitive<TupleRestAtomic> => { const assert = (input: any): TupleRestAtomic => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TupleRestAtomic.SPOILERS);
