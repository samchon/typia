...
export const test_isStringify_TagTuple = _test_isStringify("TagTuple", TagTuple.generate, (input) => ((input: TagTuple): string | null => { const is = (input: any): input is TagTuple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), TagTuple.SPOILERS);
