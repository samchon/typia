...
export const test_assertStringify_ObjectPrimitive = _test_assertStringify("ObjectPrimitive", ObjectPrimitive.generate, (input) => ((input: any): string => { const assert = (input: any): ObjectPrimitive.IArticle => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), ObjectPrimitive.SPOILERS);
