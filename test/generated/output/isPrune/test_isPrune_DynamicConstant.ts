...
export const test_isPrune_DynamicConstant = _test_isPrune("DynamicConstant", DynamicConstant.generate, (input) => ((input: any): input is {    a: number;    b: number;    c: number;    d: number;} => { const is = (input: any): input is {    a: number;    b: number;    c: number;    d: number;} => {
