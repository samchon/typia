...
export const test_assertParse_ObjectGenericArray = _test_assertParse("ObjectGenericArray", ObjectGenericArray.generate, (input) => ((input: string): typia.Primitive<ObjectGenericArray> => { const assert = (input: any): ObjectGenericArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectGenericArray.SPOILERS);
