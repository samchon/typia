...
export async function test_ttsx_blocks_entry_type_error(): Promise<void> {
  const fixture = path.join(TestGlobal.ROOT, "fixtures", "runner-entry-error");
  ...
  assert.equal(result.stdout.includes("TTSX_ENTRY_SHOULD_NOT_RUN"), false);
