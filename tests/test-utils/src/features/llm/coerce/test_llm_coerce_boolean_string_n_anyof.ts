...
export const test_llm_coerce_boolean_string_n_anyof = (): void => {
  ...
  const boolResult = LlmJson.coerce<IBooleanUnion>(boolParams, {
    value: "n" as unknown,
  });
  ...
  const nullResult = LlmJson.coerce<INullUnion>(nullParams, {
    value: "n" as unknown,
  });
  ...
  const bothResult = LlmJson.coerce<IBooleanNullUnion>(bothParams, {
    value: "n" as unknown,
  });
