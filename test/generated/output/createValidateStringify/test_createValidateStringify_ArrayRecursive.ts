...
export const test_createValidateStringify_ArrayRecursive = _test_validateStringify("ArrayRecursive", ArrayRecursive.generate, (input: ArrayRecursive): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<ArrayRecursive> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is ArrayRecursive => {
    ...
    const $so0 = (input: any): any => `{"children":${`[${input.children.map((elem: any) => $so0(elem)).join(",")}]`},"id":${$number(input.id)},"code":${$string(input.code)},"sequence":${$number(input.sequence)},"created_at":${`{"time":${$number(input.created_at.time)},"zone":${$number(input.created_at.zone)}}`}}`;
    return $so0(input);
}; const output = validate(input) as any; if (output.success)
    output.data = stringify(input); return output; }, ArrayRecursive.SPOILERS);
