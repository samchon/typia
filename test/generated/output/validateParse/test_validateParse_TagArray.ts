...
export const test_validateParse_TagArray = _test_validateParse("TagArray", TagArray.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<TagArray>> => { const validate = (input: any): typia.IValidation<TagArray> => {
    const errors = [] as any[];
    ...
    ((input: any, _path: string, _exceptionable: boolean): input is TagArray => {
        const $vo0 = (input: any, _path: string, _exceptionable: boolean) => [(Array.isArray(input.items) && 3 === input.items.length || $report(_exceptionable, {
            ...
            })) && input.minItems.map((elem: any, _index3: number) => "number" === typeof elem && 3 <= elem || $report(_exceptionable, {
            ...
            })) && input.maxItems.map((elem: any, _index4: number) => "string" === typeof elem && 7 >= elem.length || "number" === typeof elem && 7 >= elem || $report(_exceptionable, {
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), TagArray.SPOILERS);
