...
func TestProtobufProgrammerHelperCoverage(t *testing.T) {
	...
	for _, meta := range []*schemametadata.MetadataSchema{
		...
		if protobufDecodeProgrammer_write_property_default_value(meta) == nil {
	...
	if protobufDecodeProgrammer_decode_number(schemaprotobuf.IProtobufSchema_INumber{Type: "number", Name: "int64"}) == nil ||
		protobufDecodeProgrammer_decode_number(&schemaprotobuf.IProtobufPropertyType_INumber{IProtobufSchema_INumber: schemaprotobuf.IProtobufSchema_INumber{Type: "number", Name: "uint64"}, Index: &first}) == nil {
	...
	for _, schema := range []schemaprotobuf.IProtobufSchema{
		...
		if protobufDecodeProgrammer_decode_array_value(schema) == nil {
	...
	if protobufDecodeProgrammer_decode_array(protobufDecodeProgrammer_decodeArrayProps{
		...
		Required: false,
	}) == nil || protobufDecodeProgrammer_decode_array(protobufDecodeProgrammer_decodeArrayProps{
		Accessor: input,
		Schema: schemaprotobuf.IProtobufSchema_IArray{
			Type:  "array",
	...
	func() {
		...
		_ = protobufDecodeProgrammer_decode_array_value(nil)
	}()
	for _, schema := range []schemaprotobuf.IProtobufSchema{
		&schemaprotobuf.IProtobufPropertyType_IBoolean{IProtobufSchema_IBoolean: schemaprotobuf.IProtobufSchema_IBoolean{Type: "bool"}, Index: &first},
		schemaprotobuf.IProtobufSchema_IBoolean{Type: "bool"},
	...
	if protobufEncodeProgrammer_decode_bigint(struct {
		...
		Input      *shimast.Node
		Index      *int
	}{Candidates: []string{"int64"}, Type: "int64", Input: input, Index: &first}).Is() == nil {
	...
	if protobufEncodeProgrammer_decode_number(struct {
		...
		Input      *shimast.Node
		Index      *int
	}{Candidates: []string{"int32", "uint32"}, Type: "uint32", Input: input, Index: &first}).Is() == nil {
	...
	if protobufEncodeProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: "StringType"}) == nil ||
		protobufEncodeProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: nodeName}) != nodeName ||
	...
	importContext := nativecontext.ITypiaContext{Importer: protobufProgrammerImporter{}}
	if protobufDecodeProgrammer_import_type(importContext, nativeprogrammers.ImportProgrammer_TypeProps{Name: "Imported"}) == nil ||
		protobufDecodeProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: "StringType"}) == nil ||
		protobufDecodeProgrammer_import_type(context, nativeprogrammers.ImportProgrammer_TypeProps{Name: nodeName}) != nodeName ||
...
type protobufProgrammerImporter struct{}

func (protobufProgrammerImporter) Type(props nativeprogrammers.ImportProgrammer_TypeProps) *shimast.Node {
	return protobufDecodeProgrammer_factory.NewTypeReferenceNode(protobufDecodeProgrammer_factory.NewIdentifier("Imported"), nil)
}

func (protobufProgrammerImporter) Internal(name string) *shimast.Node {
	return protobufDecodeProgrammer_factory.NewIdentifier("Imported" + name)
}
