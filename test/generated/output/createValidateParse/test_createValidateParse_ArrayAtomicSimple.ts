...
export const test_createValidateParse_ArrayAtomicSimple = _test_validateParse("ArrayAtomicSimple", ArrayAtomicSimple.generate, (input: string): typia.IValidation<typia.Primitive<ArrayAtomicSimple>> => { const validate = (input: any): typia.IValidation<ArrayAtomicSimple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayAtomicSimple.SPOILERS);
