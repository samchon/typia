...
export const test_assertParse_DefinitionArrayRecursiveNullable = _test_assertParse("DefinitionArrayRecursiveNullable", DefinitionArrayRecursiveNullable.generate, (input) => ((input: string): typia.Primitive<DefinitionArrayRecursiveNullable> => { const assert = (input: any): DefinitionArrayRecursiveNullable => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DefinitionArrayRecursiveNullable.SPOILERS);
