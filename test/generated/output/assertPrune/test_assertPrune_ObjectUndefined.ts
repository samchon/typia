...
export const test_assertPrune_ObjectUndefined = _test_assertPrune("ObjectUndefined", ObjectUndefined.generate, (input) => ((input: any): Array<ObjectUndefined.ILecture> => { const assert = (input: any): Array<ObjectUndefined.ILecture> => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; })(input), ObjectUndefined.SPOILERS);
