...
export const test_assertClone_TemplateConstant = _test_assertClone("TemplateConstant", TemplateConstant.generate, (input) => ((input: any): typia.Primitive<Array<TemplateConstant.Type>> => { const assert = (input: any): Array<TemplateConstant.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TemplateConstant.SPOILERS);
