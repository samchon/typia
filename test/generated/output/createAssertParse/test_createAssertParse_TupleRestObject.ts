...
export const test_createAssertParse_TupleRestObject = _test_assertParse("TupleRestObject", TupleRestObject.generate, (input: string): typia.Primitive<TupleRestObject> => { const assert = (input: any): TupleRestObject => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TupleRestObject.SPOILERS);
