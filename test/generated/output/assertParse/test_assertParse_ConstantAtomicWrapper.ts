...
export const test_assertParse_ConstantAtomicWrapper = _test_assertParse("ConstantAtomicWrapper", ConstantAtomicWrapper.generate, (input) => ((input: string): typia.Primitive<ConstantAtomicWrapper> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ConstantAtomicWrapper.SPOILERS);
