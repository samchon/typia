...
export const test_createIsParse_ArrayRepeatedUnionWithTuple = _test_isParse("ArrayRepeatedUnionWithTuple", ArrayRepeatedUnionWithTuple.generate, (input: any): typia.Primitive<ArrayRepeatedUnionWithTuple> => { const is = (input: any): input is ArrayRepeatedUnionWithTuple => {
    const $ip0 = (input: any) => {
        const array = input;
...
}; input = JSON.parse(input); return is(input) ? input as any : null; }, ArrayRepeatedUnionWithTuple.SPOILERS);
