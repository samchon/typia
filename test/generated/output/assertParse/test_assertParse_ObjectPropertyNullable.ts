...
export const test_assertParse_ObjectPropertyNullable = _test_assertParse("ObjectPropertyNullable", ObjectPropertyNullable.generate, (input) => ((input: string): typia.Primitive<ObjectPropertyNullable> => { const assert = (input: any): ObjectPropertyNullable => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectPropertyNullable.SPOILERS);
