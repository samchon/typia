...
export const test_validateParse_ObjectGenericUnion = _test_validateParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ISaleEntireArticle>> => { const validate = (input: any): typia.IValidation<ISaleEntireArticle> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; })(input), ObjectGenericUnion.SPOILERS);
