    ...
    const task = (replacer: string) => (callback: (input: T) => [T, T]) => {
      // SUCCESS
      {
        ...
        if (validate(() => y)(x).success === false)
          throw new Error(
            `Bug on typia.functional.validateEqualsReturn(): failed to understand the ${name} type.`,
          );
      ...
      if (
        ...
        console.log(actual);
        throw new Error(
          `Bug on typia.functional.validateEqualsReturn(): failed to detect surplus property on the ${name} type.`,
        );
      }
    };
