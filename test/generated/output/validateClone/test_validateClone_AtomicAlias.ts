...
export const test_validateClone_AtomicAlias = _test_validateClone("AtomicAlias", AtomicAlias.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<[boolean, number, string]>> => { const validate = (input: any): typia.IValidation<[boolean, number, string]> => {
    const errors = [] as any[];
    ...
    output.data = clone(input); return output; })(input), AtomicAlias.SPOILERS);
