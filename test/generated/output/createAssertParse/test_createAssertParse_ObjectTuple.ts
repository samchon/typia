...
export const test_createAssertParse_ObjectTuple = _test_assertParse("ObjectTuple", ObjectTuple.generate, (input: string): typia.Primitive<ObjectTuple> => { const assert = (input: any): ObjectTuple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectTuple.SPOILERS);
