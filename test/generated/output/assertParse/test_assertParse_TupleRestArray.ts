...
export const test_assertParse_TupleRestArray = _test_assertParse("TupleRestArray", TupleRestArray.generate, (input) => ((input: string): typia.Primitive<TupleRestArray> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TupleRestArray.SPOILERS);
