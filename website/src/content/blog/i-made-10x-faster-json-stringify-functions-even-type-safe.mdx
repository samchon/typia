...
As you can see from the above code, the new JSON string conversion functions throw `TypeGuardError` or return `null` value when `input` value is different with target type `T`. One thing amazing is, those JSON string conversion functions are much faster than the native `JSON.stringify()` function despite of type validation process.
