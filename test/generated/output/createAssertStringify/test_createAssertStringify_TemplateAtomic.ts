...
export const test_createAssertStringify_TemplateAtomic = _test_assertStringify("TemplateAtomic", TemplateAtomic.generate, (input: any): string => { const assert = (input: any): TemplateAtomic => {
    ...
    return $so0(input);
}; return stringify(assert(input)); }, TemplateAtomic.SPOILERS);
