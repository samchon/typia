...
`is` rejects `{ "not-a-uuid": "x" }` for `IAccount` and `{ 70000: "x" }` for `IPort`. `validate` and `assert` name the declared key type as `expected` and point at the key: `expected` is `(string & Format<"uuid">)` and the description reads _The key `not-a-uuid` does not satisfy the type its index signature declares_. A declared property beside the signature keeps its own name, so `id` in `{ id: string; [key: string & tags.MinLength<3>]: string }` is never measured against `MinLength<3>`.

A key that falls outside the signature's declared _type_ is a different case: nothing declares it, so it is a surplus property. `is` accepts it as it accepts any surplus property, and only the `equals` variants report it — as an extra property, with `expected` of `"undefined"`. For ``interface IPrefixed { [key: `prefix_${string}`]: string }``, the key `wrong` is surplus, while for `[key: string & tags.Pattern<"^prefix_">]` the same key is a declared key that broke its tag. The two spellings accept the same keys and differ only in what they call a rejection.
...
| Tag                                         | What it does                                                                                              |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `tags.Default<V>`                           | Records a primitive or readonly tuple default in JSON Schema output                                       |
...
| `tags.Probability<N>`                       | Evaluation threshold or option minimum ([evaluation docs](/docs/llm/evaluation#probability-requirements)) |
...
| `@probability 0.8`      | `tags.Probability<0.8>`, read only by [`llm.evaluation`](/docs/llm/evaluation#probability-requirements), and also valid on an enum member |
...
> - On a `bigint`, a range or `@multipleOf` value must be a 64-bit integer that a `number` represents exactly. `@minimum 9007199254740993` is a compile error there, because the bound would be enforced as 9007199254740992. A `number` property reads the same text as `Number()` does.
>
...
- [`llm.evaluation`](/docs/llm/evaluation) — `tags.Probability<N>` sets decision thresholds and option minimums; validators and schemas ignore it
