...
export const test_validateClone_TupleRestObject = _test_validateClone("TupleRestObject", TupleRestObject.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<[boolean, number, ...TupleRestObject.IObject[]]>> => { const validate = (input: any): typia.IValidation<[boolean, number, ...TupleRestObject.IObject[]]> => {
    const errors = [] as any[];
    ...
    return Array.isArray(input) && ("boolean" === typeof input[0] && "number" === typeof input[1] && (Array.isArray(input.slice(2)) && input.slice(2).every((elem: any) => "object" === typeof elem && null !== elem && $io0(elem)))) ? [
        input[0] as any,
        input[1] as any,
        ...Array.isArray(input.slice(2)) ? $cp0(input.slice(2)) : input.slice(2) as any
    ] as any : input as any;
}; const output = validate(input) as any; if (output.success)
    output.data = clone(input); return output; })(input), TupleRestObject.SPOILERS);
