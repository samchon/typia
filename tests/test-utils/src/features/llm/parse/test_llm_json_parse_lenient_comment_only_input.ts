...
export const test_llm_json_parse_lenient_comment_only_input = (): void => {
  ...
  if (!r1.success)
    TestValidator.equals("single-comment-errors", [{ expected: "JSON value" }], r1.errors);
  ...
  if (!r2.success)
    TestValidator.equals("multi-comment-errors", [{ expected: "JSON value" }], r2.errors);
  ...
  if (!r3.success)
    TestValidator.equals("multi-comments-errors", [{ expected: "JSON value" }], r3.errors);
  ...
  if (!r6.success)
    TestValidator.equals("unclosed-comment-errors", [{ expected: "JSON value" }], r6.errors);
  ...
  if (!r10.success)
    TestValidator.equals("ws-comments-ws-errors", [{ expected: "JSON value" }], r10.errors);
  ...
  if (!r13.success)
    TestValidator.equals("unclosed-junk-comment-errors", [{ expected: "JSON value" }], r13.errors);

  const r14 = LlmJson.parse("/* unclosed [1, 2]");
  TestValidator.equals("unclosed-junk-comment-arr-success", r14.success, false);
  if (!r14.success)
    TestValidator.equals("unclosed-junk-comment-arr-errors", [{ expected: "JSON value" }], r14.errors);

  const r15 = LlmJson.parse("/* just a comment");
  TestValidator.equals("unclosed-comment-just-success", r15.success, false);
  if (!r15.success)
    TestValidator.equals("unclosed-comment-just-errors", [{ expected: "JSON value" }], r15.errors);
};
