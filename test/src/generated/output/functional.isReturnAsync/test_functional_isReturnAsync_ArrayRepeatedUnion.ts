...
export const test_functional_isReturnAsync_ArrayRepeatedUnion =
  _test_functional_isReturnAsync("ArrayRepeatedUnion")(ArrayRepeatedUnion)(
    (p: (input: ArrayRepeatedUnion) => Promise<ArrayRepeatedUnion>) =>
      async (input: ArrayRepeatedUnion): Promise<ArrayRepeatedUnion | null> => {
        const result = await p(input);
        return ((input: any): input is ArrayRepeatedUnion => {
          const $ip0 = (input: any) => {
            const array = input;
            const top = input[0];
