...
export const test_assertClone_MapAlias = _test_assertClone("MapAlias", MapAlias.generate, (input) => ((input: any): typia.Primitive<MapAlias> => { const assert = (input: any): MapAlias => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is MapAlias => {
...
}; assert(input); const output = clone(input); return output; })(input), MapAlias.SPOILERS);
