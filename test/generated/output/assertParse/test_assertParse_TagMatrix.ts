...
export const test_assertParse_TagMatrix = _test_assertParse("TagMatrix", TagMatrix.generate, (input) => ((input: string): typia.Primitive<TagMatrix> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TagMatrix.SPOILERS);
