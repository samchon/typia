...
export const test_createAssertPrune_TagRange = _test_assertPrune("TagRange", TagRange.generate, (input: any): TagRange => { const assert = (input: any): TagRange => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, TagRange.SPOILERS);
