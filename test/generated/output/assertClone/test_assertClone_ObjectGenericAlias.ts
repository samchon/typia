...
export const test_assertClone_ObjectGenericAlias = _test_assertClone("ObjectGenericAlias", ObjectGenericAlias.generate, (input) => ((input: any): typia.Primitive<ObjectGenericAlias.ISomething<string>> => { const assert = (input: any): ObjectGenericAlias.ISomething<string> => {
...
}; assert(input); const output = clone(input); return output; })(input), ObjectGenericAlias.SPOILERS);
