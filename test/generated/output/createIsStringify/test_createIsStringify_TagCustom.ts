...
export const test_createIsStringify_TagCustom = _test_isStringify("TagCustom", TagCustom.generate, (input: TagCustom): string | null => { const is = (input: any): input is TagCustom => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, TagCustom.SPOILERS);
