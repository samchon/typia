...
export const test_assertClone_ObjectLiteralType = _test_assertClone("ObjectLiteralType", ObjectLiteralType.generate, (input) => ((input: any): typia.Primitive<{ id: string; name: string; age: number; }> => { const assert = (input: any) => {
...
}; assert(input); const output = clone(input); /* __object */; return output as { id: string; name: string; age: number; }; })(input), ObjectLiteralType.SPOILERS);
