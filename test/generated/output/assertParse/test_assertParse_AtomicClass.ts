...
export const test_assertParse_AtomicClass = _test_assertParse("AtomicClass", AtomicClass.generate, (input) => ((input: string): typia.Primitive<AtomicClass> => { const assert = (input: any): AtomicClass => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), AtomicClass.SPOILERS);
