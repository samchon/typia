...
빌드 통합 박스(맨 위)가 tsc API에 강하게 묶여 있다. tsgo 7.0이 in-process API를 포기하면, 이 박스의 모양이 근본적으로 바뀐다(IPC, async). 그러나 **나머지 3개 박스는 그대로**다 — IR과 코드 생성기는 ts.Type 모양과 무관하게 작동할 수 있다. 이게 typia가 tsgo를 견딜 수 있는 구조적 이유다.
