...
`HttpLlm` lives in `@typia/utils`. Install it alongside the framework adapter you want to wire it into (Vercel, LangChain).
...
When validation fails inside a framework adapter (Vercel / LangChain), the tool returns annotated error JSON the LLM can read:
...
- Framework adapters → [Vercel](/docs/utilization/vercel) · [LangChain](/docs/utilization/langchain)
