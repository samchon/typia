...
export const test_assertClone_TagFormat = _test_assertClone("TagFormat", TagFormat.generate, (input) => ((input: any): typia.Primitive<TagFormat> => { const assert = (input: any): TagFormat => {
...
}; assert(input); const output = clone(input); return output; })(input), TagFormat.SPOILERS);
