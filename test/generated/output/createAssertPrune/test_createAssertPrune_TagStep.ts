...
export const test_createAssertPrune_TagStep = _test_assertPrune("TagStep", TagStep.generate, (input: any): TagStep => { const assert = (input: any): TagStep => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, TagStep.SPOILERS);
