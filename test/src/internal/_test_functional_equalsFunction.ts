...
export const _test_functional_equalsFunction =
  (name: string) =>
    ...
    const task = (callback: (input: T) => [T, T]) => {
      {
        ...
        if (validate(() => y)(x) === null)
          throw new Error(
            `Bug on typia.functional.equalsFunction(): failed to understand the ${name} type.`,
          );
      }
      ...
      if (validate(() => y)(x) !== null)
        throw new Error(
          `Bug on typia.functional.equalsFunction(): failed to detect surplus property on the ${name} type.`,
        );
    };
