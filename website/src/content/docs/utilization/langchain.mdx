...
`toLangChainTools()` embeds the complete **function calling harness** in every tool — lenient JSON parsing, type coercion, and validation feedback all work automatically. When validation fails, the error is returned as text content with inline `// ❌` comments at each invalid property:
