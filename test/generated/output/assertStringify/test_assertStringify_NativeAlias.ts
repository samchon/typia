...
export const test_assertStringify_NativeAlias = _test_assertStringify("NativeAlias", NativeAlias.generate, (input) => ((input: NativeSimple): string => { const assert = (input: any) => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), NativeAlias.SPOILERS);
