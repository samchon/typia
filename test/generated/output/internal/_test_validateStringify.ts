...
export function _test_validateStringify<T>(name: string, generator: () => T, validator: (input: T) => typia.IValidation<string>, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        const input: T = generator();
        const valid: typia.IValidation<string> = validator(input);
        if (valid.success === false)
            throw new Error(`Bug on typia.validateStringify(): failed to understand the ${name} type.`);
        ...
        for (const spoil of spoilers ?? []) {
            ...
            if (valid.success === true)
                throw new Error(`Bug on typia.validateStringify(): failed to detect error on the ${name} type.`);
            typia.assert(valid);
            expected.sort();
