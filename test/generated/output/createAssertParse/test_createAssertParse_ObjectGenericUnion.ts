...
export const test_createAssertParse_ObjectGenericUnion = _test_assertParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input: string): typia.Primitive<ObjectGenericUnion> => { const assert = (input: any): ObjectGenericUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectGenericUnion.SPOILERS);
