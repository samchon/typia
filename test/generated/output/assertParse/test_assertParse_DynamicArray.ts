...
export const test_assertParse_DynamicArray = _test_assertParse("DynamicArray", DynamicArray.generate, (input) => ((input: string): typia.Primitive<DynamicArray> => { const assert = (input: any): DynamicArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicArray.SPOILERS);
