...
export const test_isStringify_NativeAlias = _test_isStringify("NativeAlias", NativeAlias.generate, (input) => ((input: NativeSimple): string | null => { const is = (input: any): input is NativeSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), NativeAlias.SPOILERS);
