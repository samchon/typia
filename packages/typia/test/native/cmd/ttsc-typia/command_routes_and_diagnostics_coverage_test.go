...
func TestCommandRoutesAndDiagnosticsCoverage(t *testing.T) {
	...
	if defaultTransformOutput() != "js" {
		t.Fatal("empty environment default output should be js")
	}
	...
	blank := newTransformCompilerDiagnostic("", 0, 0, "typia.blank", "blank")
	if blank.File != nil {
		t.Fatalf("blank diagnostic should not keep a file pointer: %+v", blank)
	}

	if text, ok := sourceFileText(commandRoutesSourceText{text: "source"}); !ok || text != "source" {
		t.Fatalf("sourceFileText should read helper source text, got text=%q ok=%v", text, ok)
	}
	if _, ok := sourceFileText(struct{}{}); ok {
		t.Fatal("sourceFileText should report false for objects without Text")
	}
	if _, err := applySourceRewrites("abc", []transformSourceRewrite{{start: 2, end: 1, replacement: "x"}}); err == nil {
		t.Fatal("invalid rewrite range should fail")
	}
	if code := runTransformTypeScript(nil, primitives, "src/main.ts", struct{}{}, "none", "tsconfig.json", ""); code != 3 {
		t.Fatalf("unavailable source text should fail with code 3, got %d", code)
	}
}

type commandRoutesSourceText struct {
	text string
}

func (s commandRoutesSourceText) Text() string {
	return s.text
}
