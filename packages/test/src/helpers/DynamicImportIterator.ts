...
export namespace DynamicImportIterator {
  ...
  async function iterate<Arguments extends any[]>(
    ...
    for (const file of directory) {
      ...
      if (stats.isDirectory() === true) {
        ...
        continue;
      } else if (file.substring(file.length - 3) !== `.${EXTENSION}`) continue;
  ...
  async function execute<Arguments extends any[]>(
    ...
    for (const key in external) {
      if (command.exclude && key.indexOf(command.exclude) !== -1) continue;
      else if (command.include && key.indexOf(command.include) === -1) continue;
      ...
      const func = async () => {
        if (options.wrapper !== undefined) await options.wrapper(key, closure);
      ...
      try {
        ...
        if (options.showElapsedTime === false) {
          ...
          console.log(
            `  - ${label}: ${chalk.yellowBright(time.toLocaleString())} ms`,
          );
        }
      } catch (exp) {
