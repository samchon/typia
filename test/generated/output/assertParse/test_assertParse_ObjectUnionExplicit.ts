...
export const test_assertParse_ObjectUnionExplicit = _test_assertParse("ObjectUnionExplicit", ObjectUnionExplicit.generate, (input) => ((input: string): typia.Primitive<ObjectUnionExplicit> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ObjectUnionExplicit.SPOILERS);
