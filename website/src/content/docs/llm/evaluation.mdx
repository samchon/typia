...
`@probability` belongs to the property where it is written. An indexed-access type such as `Source["threshold"]` extracts the property's type, not its JSDoc; put another `@probability` on the resulting evaluation property if it needs the same requirement. Passing `Source` itself to `evaluation<Source>()` does read the original property's comment.
