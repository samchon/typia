...
A blank numeric value is absent rather than `0`. In a string-typed property, the text `null` decodes to `null` only where the type admits `null`; elsewhere it stays the string `"null"`.
...
An absent or empty optional array header is omitted from the decoded object, while an absent required one decodes to `[]`. Other array headers are split on `, ` (`; ` for `cookie`), but `set-cookie` never is, because cookie attributes such as `Expires` contain `, `: its values are kept as given, and a single string becomes a one-element array. A blank numeric header is absent rather than `0`.
...
Absent fields follow the `query` rules: a required array decodes to `[]`, a nullable one to `null`. A blank numeric field is absent rather than `0`, and a string-typed field keeps the text `null` unless its type admits `null`.
