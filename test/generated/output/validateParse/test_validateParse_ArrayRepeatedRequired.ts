...
export const test_validateParse_ArrayRepeatedRequired = _test_validateParse("ArrayRepeatedRequired", ArrayRepeatedRequired.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArrayRepeatedRequired>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedRequired> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArrayRepeatedRequired.SPOILERS);
