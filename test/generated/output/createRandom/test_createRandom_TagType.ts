...
export const test_createRandom_TagType = _test_random("TagType", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<TagType> => {
    const $generator = (typia.createRandom as any).generator;
    const $ro0 = (_recursive: boolean = false, _depth: number = 0): any => ({
        int: (generator?.customs ?? $generator.customs)?.number?.([
            {
                name: "type",
                value: "int"
            }
        ]) ?? (generator?.integer ?? $generator.integer)(0, 100),
        uint: (generator?.customs ?? $generator.customs)?.number?.([
            {
                name: "type",
                value: "uint"
            }
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<TagType> => {
