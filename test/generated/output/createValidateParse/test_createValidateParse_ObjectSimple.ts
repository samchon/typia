...
export const test_createValidateParse_ObjectSimple = _test_validateParse("ObjectSimple", ObjectSimple.generate, (input: string): typia.IValidation<typia.Primitive<ObjectSimple>> => { const validate = (input: any): typia.IValidation<ObjectSimple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectSimple.SPOILERS);
