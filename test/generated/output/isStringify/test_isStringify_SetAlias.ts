...
export const test_isStringify_SetAlias = _test_isStringify("SetAlias", SetAlias.generate, (input) => ((input: SetSimple): string | null => { const is = (input: any): input is SetSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), SetAlias.SPOILERS);
