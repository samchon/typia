...
2. **[Namespace inheritance](#namespace-inheritance)** — a child type used as the parameters inherits its namespace parent's description
...
CRUD-style controllers tend to have parent types (`IBbsArticle`) and child variants (`IBbsArticle.ICreate`, `IBbsArticle.IUpdate`). Repeating the same long description on every variant is tedious. When a function takes a named type as its whole parameters object, typia cascades the descriptions into the parameters `description`: the type's own description, the quoted description of the type itself, then that of each described namespace parent. An undescribed type still names itself as `Current Type: {@link Name}`. The cascade applies only to that root parameters object; a property typed with the same child keeps its own property description. A parent contributes only when its schema is part of the same output; here `create` returns `IBbsArticle`, which puts it there.
