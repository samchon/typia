...
export const test_createAssertClone_NativeUnion = _test_assertClone("NativeUnion", NativeUnion.generate, (input: any): typia.Primitive<NativeUnion> => { const assert = (input: any): NativeUnion => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is NativeUnion => {
...
}; assert(input); const output = clone(input); return output; }, NativeUnion.SPOILERS);
