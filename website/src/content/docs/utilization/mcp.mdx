...
if (!result.success) return errorResult(JSON.stringify(result.errors, null, 2));
const output = await method.call(execute, result.data);
```

If you already depend on [`@typia/utils`](/docs/llm/json), `LlmJson.validateArguments(func, args)` bundles the same step into one call and `LlmJson.stringify(result)` formats the failure for the model. See the [`@typia/mcp` README](https://github.com/samchon/typia/tree/master/packages/mcp#inlining-registration-without-typiamcp) for the full `tools/list` + `tools/call` example.
