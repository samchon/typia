...
For AI SDK 7's `experimental_evaluate()`, use [`typia.llm.evaluation<T>()`](/docs/llm/evaluation) directly. Its questions already have the AI SDK shape, and its `decode(result.answers, result.rounding)` converts the answers to `T`; `@typia/vercel` is not involved.
