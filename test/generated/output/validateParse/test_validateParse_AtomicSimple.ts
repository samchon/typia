...
export const test_validateParse_AtomicSimple = _test_validateParse("AtomicSimple", AtomicSimple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<AtomicSimple>> => { const validate = (input: any): typia.IValidation<AtomicSimple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), AtomicSimple.SPOILERS);
