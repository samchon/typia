...
export const test_assertParse_TagAtomicUnion = _test_assertParse("TagAtomicUnion", TagAtomicUnion.generate, (input) => ((input: string): typia.Primitive<TagAtomicUnion> => { const assert = (input: any): TagAtomicUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagAtomicUnion.SPOILERS);
