...
export const test_json_validateParse_TagDefault = _test_json_validateParse(
    "TagDefault",
)<TagDefault>(TagDefault)((input) =>
    ((input: string): typia.IValidation<typia.Primitive<TagDefault>> => {
        const validate = (input: any): typia.IValidation<TagDefault> => {
            const errors = [] as any[];
            const __is = (input: any): input is TagDefault => {
                const $is_custom = (typia.json.validateParse as any).is_custom;
