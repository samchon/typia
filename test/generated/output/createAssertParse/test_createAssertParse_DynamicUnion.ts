...
export const test_createAssertParse_DynamicUnion = _test_assertParse("DynamicUnion", DynamicUnion.generate, (input: string): typia.Primitive<DynamicUnion> => { const assert = (input: any): DynamicUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, DynamicUnion.SPOILERS);
