...
export const test_createAssertParse_ObjectGenericUnion = _test_assertParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input: string): typia.Primitive<ISaleEntireArticle> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ObjectGenericUnion.SPOILERS);
