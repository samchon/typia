...
export const test_assertClone_TagStep = _test_assertClone("TagStep", TagStep.generate, (input) => ((input: any): typia.Primitive<Array<TagStep.Type>> => { const assert = (input: any): Array<TagStep.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagStep.SPOILERS);
