...
Introducing such package renaming from `typescript-json` to `typia`, I will tell you which changes had been occurred during six months. Lots of interesting features and improvements have been happened.
...
>
> Also, as `typia` performs AOT (Ahead of Time) compilation skill, its performance is much faster than other competitive libraries. For an example, when comparing validate function `is()` with other competitive libraries, `typia` is maximum **15,000x times faster** than `class-validator`.
...
For reference, `@nestia/sdk` also can generate SDK (Software Development Kit) library for client developers, by analyzing backend server codes in the compilation level. If I write next article in here `dev.to`, subject of the new article would be this `nestia` and SDK library.
