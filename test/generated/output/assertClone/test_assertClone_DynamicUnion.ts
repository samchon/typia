...
export const test_assertClone_DynamicUnion = _test_assertClone("DynamicUnion", DynamicUnion.generate, (input) => ((input: any): typia.Primitive<DynamicUnion> => { const assert = (input: any): DynamicUnion => {
...
}; assert(input); const output = clone(input); return output; })(input), DynamicUnion.SPOILERS);
