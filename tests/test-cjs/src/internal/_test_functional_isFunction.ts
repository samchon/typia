...
export const _test_functional_isFunction =
  (name: string) =>
  ...
  (validate: (p: (input: T) => T) => (input: T) => T | null): void => {
    const task = (callback: (input: T) => [T, T]) => {
      ...
      if (validate(() => y)(x) === null)
        throw new Error(
          `Bug on typia.functional.isFunction(): failed to understand the ${name} type.`,
        );
      for (const spoil of factory.SPOILERS ?? []) {
        ...
        if (validate(() => y)(x) !== null)
          throw new Error(
            `Bug on typia.functional.isFunction(): failed to detect error on the ${name} type.`,
          );
      }
    };
