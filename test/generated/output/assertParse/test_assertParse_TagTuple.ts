...
export const test_assertParse_TagTuple = _test_assertParse("TagTuple", TagTuple.generate, (input) => ((input: string): typia.Primitive<TagTuple> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TagTuple.SPOILERS);
