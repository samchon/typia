...
export const test_assertParse_ObjectLiteralType = _test_assertParse("ObjectLiteralType", ObjectLiteralType.generate, (input) => ((input: string): typia.Primitive<ObjectLiteralType> => { const assert = (input: any): ObjectLiteralType => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectLiteralType.SPOILERS);
