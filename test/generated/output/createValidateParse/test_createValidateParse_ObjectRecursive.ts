...
export const test_createValidateParse_ObjectRecursive = _test_validateParse("ObjectRecursive", ObjectRecursive.generate, (input: string): typia.IValidation<typia.Primitive<IDepartment>> => { const validate = (input: any): typia.IValidation<IDepartment> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; }, ObjectRecursive.SPOILERS);
