...
export const test_assertStringify_DynamicTemplate = _test_assertStringify("DynamicTemplate", DynamicTemplate.generate, (input) => ((input: any): string => { const assert = (input: any): DynamicTemplate => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), DynamicTemplate.SPOILERS);
