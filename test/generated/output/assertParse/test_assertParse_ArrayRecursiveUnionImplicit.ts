...
export const test_assertParse_ArrayRecursiveUnionImplicit = _test_assertParse("ArrayRecursiveUnionImplicit", ArrayRecursiveUnionImplicit.generate, (input) => ((input: string): typia.Primitive<ArrayRecursiveUnionImplicit> => { const assert = (input: any): ArrayRecursiveUnionImplicit => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayRecursiveUnionImplicit.SPOILERS);
