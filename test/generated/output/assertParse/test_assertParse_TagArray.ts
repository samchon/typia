...
export const test_assertParse_TagArray = _test_assertParse("TagArray", TagArray.generate, (input) => ((input: string): typia.Primitive<TagArray> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TagArray.SPOILERS);
