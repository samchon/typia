...
export const test_createAssertParse_DynamicConstant = _test_assertParse("DynamicConstant", DynamicConstant.generate, (input: string): typia.Primitive<DynamicConstant> => { const assert = (input: any): DynamicConstant => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, DynamicConstant.SPOILERS);
