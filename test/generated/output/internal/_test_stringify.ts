...
export function _test_stringify<T>(name: string, generator: () => T, converter: (input: T) => string): () => void {
    return () => {
        const data: T = generator();
        const optimized: string = converter(data);
        if (predicate(data, optimized) === false) {
            throw new Error(`Bug on typia.stringify(): failed to understand the ${name} type.`);
        }
    };
}
