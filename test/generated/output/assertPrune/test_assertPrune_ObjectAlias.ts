...
export const test_assertPrune_ObjectAlias = _test_assertPrune("ObjectAlias", ObjectAlias.generate, (input) => ((input: any): Array<ObjectAlias.IMember> => { const assert = (input: any): Array<ObjectAlias.IMember> => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; })(input), ObjectAlias.SPOILERS);
