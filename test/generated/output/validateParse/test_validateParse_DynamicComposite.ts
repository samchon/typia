...
export const test_validateParse_DynamicComposite = _test_validateParse("DynamicComposite", DynamicComposite.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicComposite>> => { const validate = (input: any): typia.IValidation<DynamicComposite> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), DynamicComposite.SPOILERS);
