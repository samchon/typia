...
export const test_assertClone_TagObjectUnion = _test_assertClone("TagObjectUnion", TagObjectUnion.generate, (input) => ((input: any): typia.Primitive<Array<TagObjectUnion.Type>> => { const assert = (input: any): Array<TagObjectUnion.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagObjectUnion.SPOILERS);
