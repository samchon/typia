...
export namespace FunctionFactory {
    ...
    export function generate(
        ...
        if (file !== LIB_PATH && file !== SRC_PATH) return null;

        // FIND FUNCTION
        const name: string = project.checker.getTypeAtLocation(
            signature.declaration,
        ).symbol.name;
        const generator: IGenerator | undefined = FUNCTORS[name];

        if (generator === undefined) return null;
        else if (expression.arguments.length !== generator.count) return null;
    ...
    function argue_stringify(
        ...
        const application = SchemaFactory.application(
            app,
    ...
    function argue_application(
        ...
        return function (
            ...
            if (type === null) {
                ...
                throw new Error(
                    `Error on TSON.${method}(): the generic argument must be specified - ${
                        file.fileName
                    }:${line + 1}:${character + 1}.`,
                );
            }
