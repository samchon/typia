...
export const test_assertStringify_DynamicSimple = _test_assertStringify("DynamicSimple", DynamicSimple.generate, (input) => ((input: any): string => { const assert = (input: any): DynamicSimple => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), DynamicSimple.SPOILERS);
