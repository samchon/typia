...
export const test_assertPrune_ObjectIntersection = _test_assertPrune("ObjectIntersection", ObjectIntersection.generate, (input) => ((input: any): ObjectIntersection.IEmail & ObjectIntersection.IName => { const assert = (input: any): ObjectIntersection.IEmail & ObjectIntersection.IName => {
    ...
    if ("object" === typeof input && null !== input)
        $po0(input);
}; assert(input); prune(input); return input; })(input), ObjectIntersection.SPOILERS);
