...
export const test_createAssertParse_AtomicClass = _test_assertParse("AtomicClass", AtomicClass.generate, (input: string): typia.Primitive<AtomicClass> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, AtomicClass.SPOILERS);
