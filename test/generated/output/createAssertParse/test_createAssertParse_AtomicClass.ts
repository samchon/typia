...
export const test_createAssertParse_AtomicClass = _test_assertParse("AtomicClass", AtomicClass.generate, (input: string): typia.Primitive<AtomicClass> => { const assert = (input: any): AtomicClass => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, AtomicClass.SPOILERS);
