...
핵심 차이: AOT(Ahead of Time) 컴파일 — 타입을 분석해 컴파일 타임에 검증/직렬화 코드 emit. 현재 공개 setup 문서의 기본 경로는 `typia setup` → `@typescript/native-preview` + `@typia/ttsc` + `typia/lib/transform` 이다.
