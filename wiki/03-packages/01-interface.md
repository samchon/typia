...
- 실측 **~21개 export** (`packages/interface/src/tags/index.ts`): Constant, ContentMediaType, Default, Example, Examples, ExclusiveMinimum/Maximum, Format, JsonSchemaPlugin, Maximum, MaxItems, MaxLength, Minimum, MinItems, MinLength, MultipleOf, Pattern, Sequence, Type, UniqueItems, TagBase. wiki가 자주 쓰는 대표 11개: Format, Pattern, Type, Min/Max, MinLength/MaxLength, UniqueItems, Default, Example.
...
규모 ~8,508 LOC / 60+ 인터페이스. typia의 **계약 표면**이 이 한 곳에 모여 있다. 강점이자 동시에 **변경 비용이 가장 큰 곳** — semver-major 한 번이 생태계 전체 bump. LLM 스키마 표준이 아직 미성숙(OpenAI strict/Anthropic/Google 매년 변동)하니 `IConfig` 안정성이 typia LLM 채택의 핵심 변수.
