...
export const test_validateParse_DynamicSimple = _test_validateParse("DynamicSimple", DynamicSimple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicSimple>> => { const validate = (input: any): typia.IValidation<DynamicSimple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), DynamicSimple.SPOILERS);
