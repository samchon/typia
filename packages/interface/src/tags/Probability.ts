 ...
 *   threshold is `0.5`.
 * - On one member of a literal union, it is that member's acceptance minimum:
 *   `validate()` fails when the model selects the member with a probability
 *   below `N`, instead of falling back to a less likely member.
 * - On one member of a literal union used as an array element, where every member
 *   is its own yes/no decision, it is that member's inclusion threshold, as on
 *   a `boolean`.
 *
 ...
 *
 *     // "escalate" is accepted only when its probability is >= 0.9
 *     action:
 *       | (Constant<"escalate", { description: "Page the on-call" }> &
 *           Probability<0.9>)
 *       | Constant<"reply", { description: "Answer the customer" }>;
 *   }
 *
