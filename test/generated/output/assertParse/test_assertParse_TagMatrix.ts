...
export const test_assertParse_TagMatrix = _test_assertParse("TagMatrix", TagMatrix.generate, (input) => ((input: string): typia.Primitive<TagMatrix> => { const assert = (input: any): TagMatrix => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagMatrix.SPOILERS);
