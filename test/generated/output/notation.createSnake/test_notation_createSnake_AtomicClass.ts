...
export const test_notation_createValidateSnake_AtomicClass =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.SnakeCase<AtomicClass>> => {
      ...
      const general = (input: AtomicClass): typia.SnakeCase<AtomicClass> => {
    ...
    assert: (input: any): typia.SnakeCase<AtomicClass> => {
      const __is = (input: any): input is typia.SnakeCase<AtomicClass> => {
        return (
          Array.isArray(input) &&
          input.length === 9 &&
          "boolean" === typeof input[0] &&
