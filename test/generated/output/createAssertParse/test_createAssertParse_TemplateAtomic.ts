...
export const test_createAssertParse_TemplateAtomic = _test_assertParse("TemplateAtomic", TemplateAtomic.generate, (input: string): typia.Primitive<TemplateAtomic> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, TemplateAtomic.SPOILERS);
