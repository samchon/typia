...
export const test_llm_json_parse_lenient_object_syntax_error = (): void => {
  ...
  if (!r1.success)
    TestValidator.equals("double-colon-errors", [{ expected: "JSON value" }, { expected: "':'" }], r1.errors);
  ...
  if (!r2.success)
    TestValidator.equals("colon-no-key-errors", [{ expected: "string key" }], r2.errors);
  ...
  if (!r4.success)
    TestValidator.equals("key-no-val-then-kv-errors", [{ expected: "':'" }], r4.errors);
  ...
  if (!r5.success)
    TestValidator.equals("arr-colons-errors", [{ expected: "JSON value" }, { expected: "JSON value" }], r5.errors);
  ...
  if (!r6.success)
    TestValidator.equals("semicolons-errors", [{ expected: "string key" }], r6.errors);
  ...
  if (!r7.success)
    TestValidator.equals("equals-error-errors", [{ expected: "':'" }], r7.errors);
};
