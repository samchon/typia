...
export const test_assertParse_ObjectUnionDouble = _test_assertParse("ObjectUnionDouble", ObjectUnionDouble.generate, (input) => ((input: string): typia.Primitive<ObjectUnionDouble> => { const assert = (input: any): ObjectUnionDouble => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectUnionDouble.SPOILERS);
