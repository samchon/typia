...
export const test_validateEquals_TagArray = _test_validateEquals("TagArray", TagArray.generate, (input) => ((input: any): typia.IValidation<Array<TagArray.Type>> => {
    const __is = (input: any, _exceptionable: boolean = true): input is Array<TagArray.Type> => {
        const $io0 = (input: any, _exceptionable: boolean = true): boolean => Array.isArray(input.items) && 3 === input.items.length && input.items.every((elem: any, _index2: number) => "string" === typeof elem && $is_uuid(elem)) && (Array.isArray(input.minItems) && 3 <= input.minItems.length && input.minItems.every((elem: any, _index3: number) => "number" === typeof elem && Number.isFinite(elem) && 3 <= elem)) && (Array.isArray(input.maxItems) && 7 >= input.maxItems.length && input.maxItems.every((elem: any, _index4: number) => "string" === typeof elem && 7 >= elem.length || "number" === typeof elem && Number.isFinite(elem) && 7 >= elem)) && (Array.isArray(input.both) && 3 <= input.both.length && 7 >= input.both.length && input.both.every((elem: any, _index5: number) => "string" === typeof elem && $is_uuid(elem))) && (4 === Object.keys(input).length || Object.keys(input).every(key => {
            if (["items", "minItems", "maxItems", "both"].some(prop => key === prop))
                return true;
