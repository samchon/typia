...
export const test_createValidateParse_ObjectGenericUnion = _test_validateParse("ObjectGenericUnion", ObjectGenericUnion.generate, (input: string): typia.IValidation<typia.Primitive<ISaleEntireArticle>> => { const validate = (input: any): typia.IValidation<ISaleEntireArticle> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; }, ObjectGenericUnion.SPOILERS);
