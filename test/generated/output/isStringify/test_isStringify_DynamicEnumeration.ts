...
export const test_isStringify_DynamicEnumeration = _test_isStringify("DynamicEnumeration", DynamicEnumeration.generate, (input) => ((input: DynamicEnumeration): string | null => { const is = (input: any): input is DynamicEnumeration => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), DynamicEnumeration.SPOILERS);
