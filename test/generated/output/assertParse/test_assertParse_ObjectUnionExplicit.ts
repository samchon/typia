...
export const test_assertParse_ObjectUnionExplicit = _test_assertParse("ObjectUnionExplicit", ObjectUnionExplicit.generate, (input) => ((input: string): typia.Primitive<ObjectUnionExplicit> => { const assert = (input: any): ObjectUnionExplicit => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectUnionExplicit.SPOILERS);
