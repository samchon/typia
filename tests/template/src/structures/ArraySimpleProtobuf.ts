...
export interface ArraySimpleProtobuf {
  boolean: boolean[];
  ...
  string: string[];
  bytes: Uint8Array[];
  object: ArraySimpleProtobuf[];
}
export namespace ArraySimpleProtobuf {
  export const ADDABLE = false;
  export const JSONABLE = false;

  export function generate(_level: number = 0): ArraySimpleProtobuf {
    return {
  ...
  export const SPOILERS: Spoiler<ArraySimpleProtobuf>[] = [
    (input) => {
