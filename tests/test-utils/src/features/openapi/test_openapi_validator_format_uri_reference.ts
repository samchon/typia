...
/**
 * Verifies uri-reference validation matches the RFC 3986 character repertoire.
 *
 * RFC 3986 builds every component of a URI-reference from unreserved
 * characters, sub-delims (`!$&'()*+,;=`), and pct-encoded octets, so a raw
 * double quote is legal in none of them and `%22` is its only spelling. The
 * same expression is copied into the typia runtime helper, the native code
 * generator, and the @typia/utils validator, so the matrix runs through every
 * owner rather than trusting one of them. It also pins the coverage relation
 * OpenApiTypeChecker declares between iri-reference and uri-reference, which is
 * what makes an over-permissive uri-reference observable from inside typia.
 *
...
export const test_openapi_validator_format_uri_reference = (): void => {
  ...
  for (const value of [...valids, ...rawQuotes, ...excluded])
