...
Pass a [`typia.llm.controller<Class>()`](/docs/llm/application). The controller's `name` becomes the server name, and its class JSDoc becomes the handshake instructions — the class _is_ the server. The returned server connects to any MCP transport:
...
MCP `instructions` tell the client's LLM how to use the server. `typia.llm.controller` reflects the class or interface JSDoc onto `ILlmApplication.description`, and `createMcpServer` ships that reflected description as the handshake instructions — write the usage contract once, as documentation on the class, and it reaches the client automatically. Because the controller is constructed by the caller, the executor can defer expensive work until the first tool call while the handshake and instructions answer immediately:
