...
export const test_assertClone_DynamicComposite = _test_assertClone("DynamicComposite", DynamicComposite.generate, (input) => ((input: any): typia.Primitive<DynamicComposite> => { const assert = (input: any): DynamicComposite => {
...
}; assert(input); const output = clone(input); return output; })(input), DynamicComposite.SPOILERS);
