...
export const test_validateParse_ArrayRecursive = _test_validateParse("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArrayRecursive>> => { const validate = (input: any): typia.IValidation<ArrayRecursive> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArrayRecursive.SPOILERS);
