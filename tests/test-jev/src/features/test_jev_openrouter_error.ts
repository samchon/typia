 ...
 * answer map is a broken contract and must throw too, rather than reach
 * `validate()` as if it were an empty answer.
 *
 ...
 * 3. Answer 200 without `answers` and assert the thrown contract error.
 */
export const test_jev_openrouter_error = async (): Promise<void> => {
  ...
  const broken = { model: "m", usage: {} };
  TestEquality.equals("200 without answers", summary(await run(200, broken)), {
    calls: 1,
    status: 200,
    message:
      "Jev request failed with status 200: the response carries no answer map",
    body: broken,
  });
};
