...
export const test_createIsStringify_ArrayAny = _test_isStringify("ArrayAny", ArrayAny.generate, (input: ArrayAny): string | null => { const is = (input: any): input is ArrayAny => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, ArrayAny.SPOILERS);
