...
func TestFactoryUtilityCoverage(t *testing.T) {
	...
	if ExpressionFactory.SelfCall(call) != call {
		t.Fatal("self-call should return call expressions unchanged")
	}
	if ExpressionFactory.SelfCall(StatementFactory.Block(input), TypeFactory.Keyword("string")) == nil {
	...
	assignment := expressionFactory_factory.NewPropertyAssignment(nil, IdentifierFactory.Identifier("assigned"), nil, nil, arrow)
	declaration := expressionFactory_factory.NewPropertyDeclaration(nil, IdentifierFactory.Identifier("declared"), nil, nil, arrow)
	nonFunctionAssignment := expressionFactory_factory.NewPropertyAssignment(nil, IdentifierFactory.Identifier("value"), nil, nil, input)
	if typeFactory_get_function(&shimast.Symbol{Declarations: []*shimast.Node{assignment}}) == nil ||
		typeFactory_get_function(&shimast.Symbol{Declarations: []*shimast.Node{declaration}}) == nil ||
		typeFactory_get_function(&shimast.Symbol{Declarations: []*shimast.Node{nonFunctionAssignment}}) != nil ||
		typeFactory_get_function(&shimast.Symbol{Declarations: []*shimast.Node{input}}) != nil {
		t.Fatal("synthetic function declaration detection failed")
	}
	if TypeFactory.GetFullName(TypeFactory_GetFullNameProps{}) != "" {
		t.Fatal("nil full name lookup should be empty")
	}
	if typeFactory_get_type_arguments(nil, nil) != nil || typeFactory_get_name(nil) != "__type" {
