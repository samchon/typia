...
export const test_assertClone_NativeUnion = _test_assertClone("NativeUnion", NativeUnion.generate, (input) => ((input: any): typia.Primitive<Array<NativeUnion.Union>> => { const assert = (input: any): Array<NativeUnion.Union> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is Array<NativeUnion.Union> => {
            ...
            const $ao0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => (null === input.date || input.date instanceof Date || $guard(_exceptionable, {
                path: _path + ".date",
                expected: "(Date | null)",
                value: input.date
                ...
                value: input.float
            })) && (input.buffer instanceof ArrayBuffer || input.buffer instanceof SharedArrayBuffer || input.buffer instanceof DataView || input.buffer instanceof Buffer || $guard(_exceptionable, {
                path: _path + ".buffer",
...
}; assert(input); const output = clone(input); return output; })(input), NativeUnion.SPOILERS);
