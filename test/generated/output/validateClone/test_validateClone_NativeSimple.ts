...
export const test_validateClone_NativeSimple = _test_validateClone("NativeSimple", NativeSimple.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<NativeSimple>> => { const validate = (input: any): typia.IValidation<NativeSimple> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is NativeSimple => {
            const $vo0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => [input.date instanceof Date || $report(_exceptionable, {
                    path: _path + ".date",
                    expected: "Date",
                    value: input.date
    ...
    output.data = clone(input); return output; })(input), NativeSimple.SPOILERS);
