...
export const test_validateParse_ArrayRepeatedUnion = _test_validateParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArrayRepeatedUnion>> => { const validate = (input: any): typia.IValidation<ArrayRepeatedUnion> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArrayRepeatedUnion.SPOILERS);
