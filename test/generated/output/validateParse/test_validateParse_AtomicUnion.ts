...
export const test_validateParse_AtomicUnion = _test_validateParse("AtomicUnion", AtomicUnion.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<AtomicUnion>> => { const validate = (input: any): typia.IValidation<AtomicUnion> => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), AtomicUnion.SPOILERS);
