...
export const test_createAssertPrune_TagType = _test_assertPrune("TagType", TagType.generate, (input: any): TagType => { const assert = (input: any): TagType => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, TagType.SPOILERS);
