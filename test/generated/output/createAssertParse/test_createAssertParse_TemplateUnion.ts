...
export const test_createAssertParse_TemplateUnion = _test_assertParse("TemplateUnion", TemplateUnion.generate, (input: string): typia.Primitive<TemplateUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, TemplateUnion.SPOILERS);
