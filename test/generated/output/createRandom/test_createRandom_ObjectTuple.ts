...
export const test_createRandom_ObjectTuple = _test_random("ObjectTuple", (generator?: Partial<typia.IRandomGenerator>): typia.Primitive<ObjectTuple> => {
    ...
    const $ro1 = (_recursive: boolean = false, _depth: number = 0): any => ({
        id: (generator?.customs ?? $generator.customs)?.string?.([]) ?? (generator?.string ?? $generator.string)(),
        mobile: (generator?.customs ?? $generator.customs)?.string?.([]) ?? (generator?.string ?? $generator.string)(),
        name: (generator?.customs ?? $generator.customs)?.string?.([]) ?? (generator?.string ?? $generator.string)()
    });
    return [
        $ro0(),
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is typia.Primitive<ObjectTuple> => {
