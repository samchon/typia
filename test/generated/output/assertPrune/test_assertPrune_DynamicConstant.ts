...
export const test_assertPrune_DynamicConstant = _test_assertPrune("DynamicConstant", DynamicConstant.generate, (input) => ((input: any): {    a: number;    b: number;    c: number;    d: number;} => { const assert = (input: any): {    a: number;    b: number;    c: number;    d: number;} => {
    ...
    return input;
}; const prune = (input: {    a: number;    b: number;    c: number;    d: number;}): void => {
    ...
    if ("object" === typeof input && null !== input)
        $po0(input);
}; assert(input); prune(input); return input; })(input), DynamicConstant.SPOILERS);
