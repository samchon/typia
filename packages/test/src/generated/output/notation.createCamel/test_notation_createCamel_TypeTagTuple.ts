...
export const test_notation_createValidateCamel_TypeTagTuple =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.CamelCase<TypeTagTuple>> => {
      ...
      const general = (input: TypeTagTuple): typia.CamelCase<TypeTagTuple> => {
    ...
    assert: (input: any): typia.CamelCase<TypeTagTuple> => {
      const __is = (input: any): input is typia.CamelCase<TypeTagTuple> => {
        const $io0 = (input: any): boolean =>
          Array.isArray(input.tuple) &&
          input.tuple.length === 4 &&
          "string" === typeof input.tuple[0] &&
