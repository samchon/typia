...
export const test_validateParse_DynamicUnion = _test_validateParse("DynamicUnion", DynamicUnion.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicUnion>> => { const validate = (input: any): typia.IValidation<DynamicUnion> => {
    const errors = [] as any[];
    ...
    } as typia.IValidation<DynamicUnion>;
}; input = JSON.parse(input); const output = validate(input); return output; })(input), DynamicUnion.SPOILERS);
