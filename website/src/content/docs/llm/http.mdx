...
`HttpLlm` first upgrades any OpenAPI version to a normalized v3.1 format, then converts each operation into an `IHttpLlmFunction` with parameter schemas, descriptions, and HTTP metadata. The resulting `IHttpLlmController` can be passed to [MCP](./mcp), [Vercel AI SDK](./vercel), or [Agentica](./chat).
</Callout>
