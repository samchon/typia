...
export const test_stringify_ObjectUnionDouble = _test_stringify("ObjectUnionDouble", ObjectUnionDouble.generate, (input) => ((input: ObjectUnionDouble): string => {
    ...
    const $so0 = (input: any) => `{"value":${`{"x":${$number(input.value.x)}}`},"child":${$su0(input.child)}}`;
    const $so2 = (input: any) => `{"value":${`{"y":${input.value.y}}`}}`;
    const $so4 = (input: any) => `{"value":${`{"y":${$number(input.value.y)}}`}}`;
    ...
    const $so11 = (input: any) => `{"y":${`[${input.y.map((elem: any) => $number(elem)).join(",")}]`}}`;
    const $su0 = (input: any) => (() => {
        if ($io2(input))
            return $so2(input);
        if ($io4(input))
