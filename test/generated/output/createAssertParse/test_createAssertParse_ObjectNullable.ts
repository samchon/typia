...
export const test_createAssertParse_ObjectNullable = _test_assertParse("ObjectNullable", ObjectNullable.generate, (input: string): typia.Primitive<ObjectNullable> => { const assert = (input: any): ObjectNullable => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectNullable.SPOILERS);
