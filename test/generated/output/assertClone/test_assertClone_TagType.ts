...
export const test_assertClone_TagType = _test_assertClone("TagType", TagType.generate, (input) => ((input: any): typia.Primitive<Array<TagType.Type>> => { const assert = (input: any): Array<TagType.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagType.SPOILERS);
