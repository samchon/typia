...
export const test_assertParse_ConstantAtomicUnion = _test_assertParse("ConstantAtomicUnion", ConstantAtomicUnion.generate, (input) => ((input: string): typia.Primitive<ConstantAtomicUnion> => { const assert = (input: any): ConstantAtomicUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ConstantAtomicUnion.SPOILERS);
