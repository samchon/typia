...
export const test_createAssertParse_TagLength = _test_assertParse("TagLength", TagLength.generate, (input: string): typia.Primitive<TagLength> => { const assert = (input: any): TagLength => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TagLength.SPOILERS);
