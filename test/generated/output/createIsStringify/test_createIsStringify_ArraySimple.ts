...
export const test_createIsStringify_ArraySimple = _test_isStringify("ArraySimple", ArraySimple.generate, (input: ArraySimple): string | null => { const is = (input: any): input is ArraySimple => {
    ...
    const $io1 = (input: any) => "string" === typeof input.name && "string" === typeof input.body && ("number" === typeof input.rank && !Number.isNaN(input.rank));
...
}; return is(input) ? stringify(input) : null; }, ArraySimple.SPOILERS);
