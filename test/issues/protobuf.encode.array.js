...
const encode = (input) => {
    ...
    const encoder = (writer) => {
        ...
        const $io0 = (input) =>
            ...
            input.boolean.every((elem) => "boolean" === typeof elem) &&
            ...
            input.int64.every((elem) => "bigint" === typeof elem) &&
            Array.isArray(input.uint64) &&
            input.uint64.every(
                (elem) => "bigint" === typeof elem && BigInt(0) <= elem,
            ) &&
            ...
            input.double.every((elem) => "number" === typeof elem) &&
            Array.isArray(input.string) &&
            input.string.every((elem) => "string" === typeof elem) &&
            Array.isArray(input.bytes) &&
            input.bytes.every((elem) => elem instanceof Uint8Array) &&
...
function ArraySimpleProtobuf$encode(m, w) {
    ...
    if (m.uint64 != null && m.uint64.length) {
        w.uint32(42).fork();
        for (var i = 0; i < m.uint64.length; ++i) w.uint64(m.uint64[i]);
        w.ldelim();
    }
