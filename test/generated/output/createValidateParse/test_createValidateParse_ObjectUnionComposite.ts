...
export const test_createValidateParse_ObjectUnionComposite = _test_validateParse("ObjectUnionComposite", ObjectUnionComposite.generate, (input: string): typia.IValidation<typia.Primitive<ObjectUnionComposite>> => { const validate = (input: any): typia.IValidation<ObjectUnionComposite> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectUnionComposite.SPOILERS);
