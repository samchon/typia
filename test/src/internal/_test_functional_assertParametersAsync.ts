...
export const _test_functional_assertParametersAsync =
  (ErrorClass: Function) =>
  (name: string) =>
      ...
      (replacer: string) => async (callback: (input: T) => [T, T]) => {
        try {
          ...
          if ((exp as Function).constructor?.name === ErrorClass.name)
            throw new Error(
              `Bug on await typia.functional.assertParameters(): failed to understand the ${name} type.`,
            );
          else throw exp;
        }
        for (const spoil of factory.SPOILERS ?? []) {
          ...
          throw new Error(
            `Bug on await typia.functional.assertParameters(): failed to detect error on the ${name} type - ${expected}.`,
          );
        }
      };
