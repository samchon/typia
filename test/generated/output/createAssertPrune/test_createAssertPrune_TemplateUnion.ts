...
export const test_createAssertPrune_TemplateUnion = _test_assertPrune("TemplateUnion", TemplateUnion.generate, (input: any): TemplateUnion => { const assert = (input: any): TemplateUnion => {
    ...
    if (Array.isArray(input))
        $pp0(input);
}; assert(input); prune(input); return input; }, TemplateUnion.SPOILERS);
