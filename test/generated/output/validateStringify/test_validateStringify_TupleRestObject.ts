...
export const test_validateStringify_TupleRestObject = _test_validateStringify("TupleRestObject", TupleRestObject.generate, (input) => ((input: [boolean, number, ...TupleRestObject.IObject[]]): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<[boolean, number, ...TupleRestObject.IObject[]]> => {
    const errors = [] as any[];
    ...
    return `[${input[0]},${$number(input[1])}${$rest(`[${input.slice(2).map((elem: any) => `{"value":${$string((elem as any).value)}}`).join(",")}]`)}]`;
}; const output = validate(input) as any; if (output.success)
    output.data = stringify(input); return output; })(input), TupleRestObject.SPOILERS);
