...
export const test_notation_validateCamel_ObjectNullable =
  _test_notation_validateGeneral("ObjectNullable")<ObjectNullable>(
    ...
    convert: (input) =>
      ((input: any): typia.IValidation<typia.CamelCase<ObjectNullable>> => {
    ...
    assert: (input: any): typia.CamelCase<ObjectNullable> => {
      const __is = (input: any): input is typia.CamelCase<ObjectNullable> => {
        const $io0 = (input: any): boolean =>
          Array.isArray(input.value) &&
          input.value.every(
            (elem: any) =>
