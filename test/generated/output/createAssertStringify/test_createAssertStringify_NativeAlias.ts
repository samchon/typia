...
export const test_createAssertStringify_NativeAlias = _test_assertStringify("NativeAlias", NativeAlias.generate, (input: NativeAlias): string => { const assert = (input: any) => {
    ...
    return $so0(input);
}; return stringify(assert(input)); }, NativeAlias.SPOILERS);
