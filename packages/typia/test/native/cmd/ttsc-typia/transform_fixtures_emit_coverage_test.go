...
// 3. Transform representative projects to JavaScript output in memory.
...
func TestTransformSyntheticEmitCoverage(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name+"_js", func(t *testing.T) {
			...
			if !strings.Contains(out, "exports.") && !strings.Contains(out, "module.exports") {
				t.Fatalf("transform js output for %s looks empty:\n%s", tc.name, out)
			}
		})
	}
	core := projects["core"]
	...
	invalid := transformCoverageProject(t, "invalid-intersection", transformCoverageInvalidIntersectionSource)
	if _, errText, code := transformCoverageCapture(func() int {
		return runTransform([]string{
			"--cwd", invalid,
			"--tsconfig", "tsconfig.json",
		...
		t.Fatalf("invalid intersection transform should fail")
	} else if !strings.Contains(errText, "intersection") && !strings.Contains(errText, "typia.tag") {
		t.Fatalf("invalid intersection diagnostics should mention intersection or tag errors:\n%s", errText)
	}
}
