...
export const test_assertClone_ArrayAny = _test_assertClone("ArrayAny", ArrayAny.generate, (input) => ((input: any): typia.Primitive<ArrayAny> => { const assert = (input: any): ArrayAny => {
...
}; assert(input); const output = clone(input); return output; })(input), ArrayAny.SPOILERS);
