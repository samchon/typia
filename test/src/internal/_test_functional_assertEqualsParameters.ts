...
export const _test_functional_assertEqualsParameters =
  (ErrorClass: Function) =>
  (name: string) =>
    ...
    const validate = (replacer: string) => (callback: (input: T) => [T, T]) => {
      ...
      try {
        ...
        if ((exp as Function).constructor?.name === ErrorClass.name)
          throw new Error(
            `Bug on typia.functional.assertEqualsParameters(): failed to understand the ${name} type.`,
          );
        else throw exp;
      }
      ...
      for (const { path, value } of accessors) {
        ...
        try {
          assertEquals(() => y)(x);
          throw new Error(
            `Bug on typia.functional.assertEqualsParameters(): failed to detect surplus property on the ${name} type.`,
          );
        } catch (exp) {
          if (
