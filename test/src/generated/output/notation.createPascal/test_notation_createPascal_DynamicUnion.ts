...
export const test_notation_createValidatePascal_DynamicUnion =
  ...
  >({
    convert: (
      input: any,
    ): typia.IValidation<typia.PascalCase<DynamicUnion>> => {
      ...
      const general = (input: DynamicUnion): typia.PascalCase<DynamicUnion> => {
    ...
    assert: (input: any): typia.PascalCase<DynamicUnion> => {
      const __is = (input: any): input is typia.PascalCase<DynamicUnion> => {
        const $io0 = (input: any): boolean =>
          Object.keys(input).every((key: any) => {
            const value = input[key];
            if (undefined === value) return true;
