...
export const test_validateStringify_TagFormat = _test_validateStringify("TagFormat", TagFormat.generate, (input) => ((input: TagFormat): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<TagFormat> => {
    ...
    const $is_datetime = (typia.validateStringify as any).is_datetime;
    if (false === __is(input))
    ...
    output.data = stringify(input); return output; })(input), TagFormat.SPOILERS);
