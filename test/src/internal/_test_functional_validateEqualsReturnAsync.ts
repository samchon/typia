...
export const _test_functional_validateEqualsReturnAsync =
  (name: string) =>
      ...
      (replacer: string) => async (callback: (input: T) => [T, T]) => {
        // SUCCESS
        {
          ...
          if ((await validate(async () => y)(x)).success === false)
            throw new Error(
              `Bug on await typia.functional.validateEqualsReturn(): failed to understand the ${name} type.`,
            );
        ...
        if (
          ...
          console.log(actual);
          throw new Error(
            `Bug on await typia.functional.validateEqualsReturn(): failed to detect surplus property on the ${name} type.`,
          );
        }
      };
