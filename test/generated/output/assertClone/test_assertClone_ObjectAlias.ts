...
export const test_assertClone_ObjectAlias = _test_assertClone("ObjectAlias", ObjectAlias.generate, (input) => ((input: any): typia.Primitive<Array<ObjectAlias.IMember>> => { const assert = (input: any): Array<ObjectAlias.IMember> => {
...
}; assert(input); const output = clone(input); return output; })(input), ObjectAlias.SPOILERS);
