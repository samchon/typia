...
export const test_validateStringify_ObjectPrimitive = _test_validateStringify("ObjectPrimitive", ObjectPrimitive.generate, (input) => ((input: ObjectPrimitive.IArticle): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<ObjectPrimitive.IArticle> => {
    ...
    const $io1 = (input: any): boolean => "string" === typeof input.id && "string" === typeof input.name && "string" === typeof input.extension && "string" === typeof input.url && "string" === typeof input.created_at;
    const $so0 = (input: any): any => `{"id":${$string(input.id)},"extension":${(() => {
    ...
    })()},"title":${$string(input.title)},"body":${$string(input.body)},"files":${`[${input.files.map((elem: any) => $so1(elem)).join(",")}]`},"secret":${input.secret},"created_at":${$string(input.created_at)}}`;
    const $so1 = (input: any): any => `{"id":${$string(input.id)},"name":${$string(input.name)},"extension":${$string(input.extension)},"url":${$string(input.url)},"created_at":${$string(input.created_at)}}`;
    return $so0(input);
}; const output = validate(input) as any; if (output.success)
    output.data = stringify(input); return output; })(input), ObjectPrimitive.SPOILERS);
