...
export const test_notation_createValidatePascal_AtomicSimple =
  ...
  >({
    convert: (
      input: any,
    ): typia.IValidation<typia.PascalCase<AtomicSimple>> => {
      ...
      const general = (input: AtomicSimple): typia.PascalCase<AtomicSimple> => {
    ...
    assert: (input: any): typia.PascalCase<AtomicSimple> => {
      const __is = (input: any): input is typia.PascalCase<AtomicSimple> => {
        return (
          Array.isArray(input) &&
          input.length === 3 &&
          "boolean" === typeof input[0] &&
      ...
      if (false === __is(input))
        ((
          ...
          _path: string,
          _exceptionable: boolean = true,
        ): input is typia.PascalCase<AtomicSimple> => {
