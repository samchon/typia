...
export const test_assertParse_DynamicTemplate = _test_assertParse("DynamicTemplate", DynamicTemplate.generate, (input) => ((input: string): typia.Primitive<DynamicTemplate> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), DynamicTemplate.SPOILERS);
