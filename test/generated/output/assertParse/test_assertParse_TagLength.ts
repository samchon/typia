...
export const test_assertParse_TagLength = _test_assertParse("TagLength", TagLength.generate, (input) => ((input: string): typia.Primitive<TagLength> => { const assert = (input: any): TagLength => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagLength.SPOILERS);
