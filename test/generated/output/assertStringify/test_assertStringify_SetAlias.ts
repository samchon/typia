...
export const test_assertStringify_SetAlias = _test_assertStringify("SetAlias", SetAlias.generate, (input) => ((input: any): string => { const assert = (input: any): SetAlias => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), SetAlias.SPOILERS);
