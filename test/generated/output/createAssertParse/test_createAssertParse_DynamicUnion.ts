...
export const test_createAssertParse_DynamicUnion = _test_assertParse("DynamicUnion", DynamicUnion.generate, (input: string): typia.Primitive<DynamicUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, DynamicUnion.SPOILERS);
