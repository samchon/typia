...
export const test_createIsStringify_TagFormat = _test_isStringify("TagFormat", TagFormat.generate, (input: TagFormat): string | null => { const is = (input: any): input is TagFormat => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, TagFormat.SPOILERS);
