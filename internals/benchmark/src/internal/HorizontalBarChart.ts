    ...
    (data: IMeasure[]) => {
      ...
      const calc = {
        ...
        height: (category: string) => (column: string) =>
          categories(category)! + components(column)! + style.margin.top,
      };
      ...
      const values: d3.ScaleLinear<number, number, never> = d3
        .scaleLinear()
        .domain([0, maximum])
        .range([0, style.width - style.margin.left - style.margin.right]);
      ...
      svg
        ...
        .attr(
          "transform",
          `translate(${style.margin.left - 1},${height - style.margin.bottom})`,
        )
        .call(
          d3
            .axisBottom(values)
      ...
      svg
        ...
        .attr(
          "transform",
          `translate(${style.width / 2}, ${(2 * style.margin.top) / 3})`,
        )
      ...
      const mapped = svg
        ...
        .data((d) =>
          columns.map((column) => ({
            ...
            value:
              d.result[column]! !== 0 &&
              Object.values(d.result).filter((val) => val !== 0).length === 1
