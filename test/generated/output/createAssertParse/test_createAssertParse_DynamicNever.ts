...
export const test_createAssertParse_DynamicNever = _test_assertParse("DynamicNever", DynamicNever.generate, (input: string): typia.Primitive<DynamicNever> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, DynamicNever.SPOILERS);
