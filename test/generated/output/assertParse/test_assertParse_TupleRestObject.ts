...
export const test_assertParse_TupleRestObject = _test_assertParse("TupleRestObject", TupleRestObject.generate, (input) => ((input: string): typia.Primitive<TupleRestObject> => { const assert = (input: any): TupleRestObject => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TupleRestObject.SPOILERS);
