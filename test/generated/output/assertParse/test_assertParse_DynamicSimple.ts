...
export const test_assertParse_DynamicSimple = _test_assertParse("DynamicSimple", DynamicSimple.generate, (input) => ((input: string): typia.Primitive<DynamicSimple> => { const assert = (input: any): DynamicSimple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), DynamicSimple.SPOILERS);
