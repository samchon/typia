...
export const test_assertParse_ConstantAtomicSimple = _test_assertParse("ConstantAtomicSimple", ConstantAtomicSimple.generate, (input) => ((input: string): typia.Primitive<ConstantAtomicSimple> => { const assert = (input: any): ConstantAtomicSimple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ConstantAtomicSimple.SPOILERS);
