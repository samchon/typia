...
export const test_functional_validateFunctionAsync_MapAlias =
  _test_functional_validateFunctionAsync("MapAlias")(MapAlias)(
    (p: (input: MapAlias) => Promise<MapAlias>) =>
      async (
        input: MapAlias,
      ): Promise<import("typia").IValidation<MapAlias>> => {
        const paramResults = [
        ...
        const result = ((input: any): typia.IValidation<MapAlias> => {
          const errors = [] as any[];
          const __is = (input: any): input is MapAlias => {
            const $io0 = (input: any): boolean =>
              input.boolean instanceof Map &&
