...
export const test_validateParse_ArraySimple = _test_validateParse("ArraySimple", ArraySimple.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ArraySimple>> => { const validate = (input: any): typia.IValidation<ArraySimple> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ArraySimple.SPOILERS);
