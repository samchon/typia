...
export const test_createValidateParse_ObjectLiteralProperty = _test_validateParse("ObjectLiteralProperty", ObjectLiteralProperty.generate, (input: string): typia.IValidation<typia.Primitive<ObjectLiteralProperty>> => { const validate = (input: any): typia.IValidation<ObjectLiteralProperty> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectLiteralProperty.SPOILERS);
