...
export const test_notation_validatePascal_ObjectDynamic =
  ...
  >({
    convert: (input) =>
      ((input: any): typia.IValidation<typia.PascalCase<ObjectDynamic>> => {
    ...
    assert: (input: any): typia.PascalCase<ObjectDynamic> => {
      const __is = (input: any): input is typia.PascalCase<ObjectDynamic> => {
        const $io0 = (input: any): boolean =>
          Object.keys(input).every((key: any) => {
            const value = input[key];
            if (undefined === value) return true;
      ...
      if (false === __is(input))
        ((
          ...
          _path: string,
          _exceptionable: boolean = true,
        ): input is typia.PascalCase<ObjectDynamic> => {
