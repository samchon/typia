...
export const _test_functional_assertEqualsReturnAsync =
  (ErrorClass: Function) =>
  (name: string) =>
    ...
    const validate =
      (replacer: string) => async (callback: (input: T) => [T, T]) => {
        ...
        try {
          ...
          await assertEquals(async () => y)(x);
        } catch (exp) {
          if ((exp as Function).constructor?.name === ErrorClass.name)
            throw new Error(
              `Bug on await typia.functional.assertEqualsReturn(): failed to understand the ${name} type.`,
            );
          else throw exp;
        }
        ...
        for (const { path, value } of accessors) {
          ...
          try {
            await assertEquals(async () => y)(x);
            throw new Error(
              `Bug on await typia.functional.assertEqualsReturn(): failed to detect surplus property on the ${name} type.`,
            );
          } catch (exp) {
            if (
              (exp as Function).constructor?.name === ErrorClass.name &&
              typia.is<TypeGuardError.IProps>(exp) &&
              exp.method === "typia.functional.assertEqualsReturn" &&
              exp.path === fullPath &&
              ...
              throw new Error(
                `Bug on typia.functional.assertEqualsReturn(): failed to detect surplus property on the ${name} type.`,
              );
            } else throw exp;
          }
        }
