...
export const test_validateClone_AtomicSimple = _test_validateClone("AtomicSimple", AtomicSimple.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<[boolean, number, string]>> => { const validate = (input: any): typia.IValidation<[boolean, number, string]> => {
    const errors = [] as any[];
    ...
    output.data = clone(input); return output; })(input), AtomicSimple.SPOILERS);
