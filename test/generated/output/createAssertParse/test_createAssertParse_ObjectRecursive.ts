...
export const test_createAssertParse_ObjectRecursive = _test_assertParse("ObjectRecursive", ObjectRecursive.generate, (input: string): typia.Primitive<IDepartment> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ObjectRecursive.SPOILERS);
