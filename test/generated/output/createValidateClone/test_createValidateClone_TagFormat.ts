...
export const test_createValidateClone_TagFormat = _test_validateClone("TagFormat", TagFormat.generate, (input: any): typia.IValidation<typia.Primitive<TagFormat>> => { const validate = (input: any): typia.IValidation<TagFormat> => {
    ...
    const $is_datetime = (typia.createValidateClone as any).is_datetime;
    if (false === __is(input))
    ...
    output.data = clone(input); return output; }, TagFormat.SPOILERS);
