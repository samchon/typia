...
export const test_validateStringify_TagCustom = _test_validateStringify("TagCustom", TagCustom.generate, (input) => ((input: TagCustom): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<TagCustom> => {
    const errors = [] as any[];
    ...
    const $is_uuid = (typia.validateStringify as any).is_uuid;
    const $is_custom = (typia.validateStringify as any).is_custom;
    const $so0 = (input: any): any => `{"id":${$string(input.id)},"dollar":${$string(input.dollar)},"postfix":${$string(input.postfix)},"log":${$number(input.log)}}`;
    return $so0(input);
}; const output = validate(input) as any; if (output.success)
    output.data = stringify(input); return output; })(input), TagCustom.SPOILERS);
