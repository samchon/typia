...
export const test_assertParse_ArraySimple = _test_assertParse("ArraySimple", ArraySimple.generate, (input) => ((input: string): typia.Primitive<ArraySimple> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), ArraySimple.SPOILERS);
