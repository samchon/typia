...
export const test_assertClone_NativeSimple = _test_assertClone("NativeSimple", NativeSimple.generate, (input) => ((input: any): typia.Primitive<NativeSimple> => { const assert = (input: any): NativeSimple => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is NativeSimple => {
            const $ao0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => (input.date instanceof Date || $guard(_exceptionable, {
                path: _path + ".date",
                expected: "Date",
                value: input.date
...
}; assert(input); const output = clone(input); return output; })(input), NativeSimple.SPOILERS);
