...
export const test_createAssertParse_ObjectPrimitive = _test_assertParse("ObjectPrimitive", ObjectPrimitive.generate, (input: string): typia.Primitive<ObjectPrimitive> => { const assert = (input: any): ObjectPrimitive => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectPrimitive.SPOILERS);
