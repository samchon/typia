...
export const test_notation_createValidateCamel_AtomicClass =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.CamelCase<AtomicClass>> => {
      ...
      const general = (input: AtomicClass): typia.CamelCase<AtomicClass> => {
    ...
    assert: (input: any): typia.CamelCase<AtomicClass> => {
      const __is = (input: any): input is typia.CamelCase<AtomicClass> => {
        return (
          Array.isArray(input) &&
          input.length === 9 &&
          "boolean" === typeof input[0] &&
      ...
      if (false === __is(input))
        ((
          ...
          _path: string,
          _exceptionable: boolean = true,
        ): input is typia.CamelCase<AtomicClass> => {
