...
export const test_assertParse_TagObjectUnion = _test_assertParse("TagObjectUnion", TagObjectUnion.generate, (input) => ((input: string): typia.Primitive<TagObjectUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TagObjectUnion.SPOILERS);
