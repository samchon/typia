...
export const test_assertParse_DynamicComposite = _test_assertParse("DynamicComposite", DynamicComposite.generate, (input) => ((input: string): typia.Primitive<DynamicComposite> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), DynamicComposite.SPOILERS);
