...
export const test_assertParse_ObjectInternal = _test_assertParse("ObjectInternal", ObjectInternal.generate, (input) => ((input: string): typia.Primitive<ObjectInternal> => { const assert = (input: any): ObjectInternal => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectInternal.SPOILERS);
