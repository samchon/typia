...
export const test_assertParse_TemplateAtomic = _test_assertParse("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: string): typia.Primitive<TemplateAtomic> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); })(input), TemplateAtomic.SPOILERS);
