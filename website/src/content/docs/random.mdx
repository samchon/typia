...
Recursion terminates only where the depth cutoff has an escape to take: an array, set, or map that may be empty, a nullable edge that may become `null`, an optional or index-signature property that may be dropped, or a union with at least one finite variant. A recursive type whose every cycle is a required, non-nullable, non-container value — `interface INode { next: INode }`, a mutual `A → B → A` object cycle, or a recursive tuple rest element — has no finite value to generate, so `typia.random` rejects it at compile time instead of overflowing the stack. Give the cycle any one escape (`next: INode | null`, `next?: INode`, or `next: INode[]`) to make it satisfiable.
