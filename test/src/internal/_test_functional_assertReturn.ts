...
export const _test_functional_assertReturn =
  (ErrorClass: Function) =>
  (name: string) =>
    ...
    const validate = (replacer: string) => (callback: (input: T) => [T, T]) => {
      try {
        ...
        if ((exp as Function).constructor?.name === ErrorClass.name)
          throw new Error(
            `Bug on typia.functional.assertReturn(): failed to understand the ${name} type.`,
          );
        else throw exp;
      }
      for (const spoil of factory.SPOILERS ?? []) {
        ...
        throw new Error(
          `Bug on typia.functional.assertReturn(): failed to detect error on the ${name} type - ${expected}.`,
        );
      }
    };
