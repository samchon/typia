...
export const test_validateParse_DynamicConstant = _test_validateParse("DynamicConstant", DynamicConstant.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<DynamicConstant>> => { const validate = (input: any): typia.IValidation<DynamicConstant> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), DynamicConstant.SPOILERS);
