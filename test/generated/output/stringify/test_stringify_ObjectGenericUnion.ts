...
export const test_stringify_ObjectGenericUnion = _test_stringify("ObjectGenericUnion", ObjectGenericUnion.generate, (input) => ((input: ISaleEntireArticle): string => {
    ...
    const $so0 = (input: any) => `{"writer":${$string(input.writer)},"answer":${$so1(input.answer)},"id":${$string(input.id)},"hit":${$number(input.hit)},"contents":${`[${input.contents.map((elem: any) => $so2(elem)).join(",")}]`},"created_at":${$string(input.created_at)}}`;
    const $so1 = (input: any) => `{"id":${$string(input.id)},"hit":${$number(input.hit)},"contents":${`[${input.contents.map((elem: any) => $so2(elem)).join(",")}]`},"created_at":${$string(input.created_at)}}`;
    ...
    const $so4 = (input: any) => `{"writer":${$string(input.writer)},"answer":${$so1(input.answer)},"id":${$string(input.id)},"hit":${$number(input.hit)},"contents":${`[${input.contents.map((elem: any) => $so5(elem)).join(",")}]`},"created_at":${$string(input.created_at)}}`;
    const $so5 = (input: any) => `{"score":${$number(input.score)},"id":${$string(input.id)},"created_at":${$string(input.created_at)},"title":${$string(input.title)},"body":${$string(input.body)},"files":${`[${input.files.map((elem: any) => `{"name":${$string(elem.name)},"extension":${$string(elem.extension)},"url":${$string(elem.url)}}`).join(",")}]`}}`;
    const $su0 = (input: any) => (() => {
        if ($io0(input))
            return $so0(input);
        if ($io4(input))
