    ...
    () => {
        try {
            ...
            if (primitive_equal_to(data, decoded as T) === false)
                throw new Error(
                    `Bug on typia.protobuf.decode(): failed to understand ${factory.constructor.name} type.`,
                );
        } catch {
            throw new Error(
                `Bug on typia.protobuf.decode(): failed to decode ${factory.constructor.name} type.`,
            );
        }
    };
