...
export const test_llm_stringify_bracket_notation_keys = (): void => {
  ...
  TestEquality.equals("hyphen-success", result1.success, false);
  if (!result1.success) {
    const output1: string = LlmJson.stringify(result1);
    TestEquality.equals("hyphen-code-block", output1.includes("```json"), true);
    TestEquality.equals("hyphen-error-marker", output1.includes("// ❌"), true);
    TestEquality.equals("hyphen-key", output1.includes("my-key"), true);
  }
  ...
  TestEquality.equals("dot-success", result2.success, false);
  if (!result2.success) {
    const output2: string = LlmJson.stringify(result2);
    TestEquality.equals("dot-code-block", output2.includes("```json"), true);
    TestEquality.equals("dot-error-marker", output2.includes("// ❌"), true);
    TestEquality.equals("dot-key", output2.includes("another.key"), true);
  }
  ...
  TestEquality.equals("string-hyphen-success", result3.success, false);
  if (!result3.success) {
