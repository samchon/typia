...
export const test_createAssertParse_TagAtomicUnion = _test_assertParse("TagAtomicUnion", TagAtomicUnion.generate, (input: string): typia.Primitive<TagAtomicUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, TagAtomicUnion.SPOILERS);
