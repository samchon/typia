...
export const test_createAssertParse_ArrayUnion = _test_assertParse("ArrayUnion", ArrayUnion.generate, (input: string): typia.Primitive<ArrayUnion> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ArrayUnion.SPOILERS);
