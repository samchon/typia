...
export const test_createValidateParse_ArrayRecursive = _test_validateParse("ArrayRecursive", ArrayRecursive.generate, (input: string): typia.IValidation<typia.Primitive<ICategory>> => { const validate = (input: any): typia.IValidation<ICategory> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output; }, ArrayRecursive.SPOILERS);
