...
export const test_assertClone_ArrayRecursive = _test_assertClone("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: any): typia.Primitive<ArrayRecursive.ICategory> => { const assert = (input: any): ArrayRecursive.ICategory => {
    ...
    return input;
}; const clone = (input: ArrayRecursive.ICategory): typia.Primitive<ArrayRecursive.ICategory> => {
    ...
    const $co0 = (input: any): any => ({
        children: Array.isArray(input.children) ? $cp0(input.children) : input.children as any,
        id: input.id as any,
...
}; assert(input); const output = clone(input); return output; })(input), ArrayRecursive.SPOILERS);
