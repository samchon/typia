...
export namespace TestReflectMetadataGenerator {
    export const generate = async (
        ...
        const path: string = `${__dirname}/../../test/features/reflect.metadata`;
    ...
    export const schemas = async (): Promise<void> => {
        const path: string = `${__dirname}/../../test/schemas/reflect/metadata`;
        await fs.promises.mkdir(path, { recursive: true });

        const directory: string[] = await fs.promises.readdir(
            `${__dirname}/../../test/features/reflect.metadata`,
        );
        for (const file of directory) {
            ...
            const location: string = `${__dirname}/../../bin/test/features/reflect.metadata/${file.slice(
                0,
                -3,
            )}.js`;
