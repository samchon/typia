...
export const test_isParse_ArrayRepeatedUnion = _test_isParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input) => ((input: any): typia.Primitive<ArrayRepeatedUnion> => { const is = (input: any): input is ArrayRepeatedUnion => {
    const $ip0 = (input: any) => {
        const array = input;
        const top = input[0];
        if (0 === input.length)
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), ArrayRepeatedUnion.SPOILERS);
