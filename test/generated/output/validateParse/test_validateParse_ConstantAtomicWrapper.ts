...
export const test_validateParse_ConstantAtomicWrapper = _test_validateParse("ConstantAtomicWrapper", ConstantAtomicWrapper.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ConstantAtomicWrapper>> => { const validate = (input: any): typia.IValidation<ConstantAtomicWrapper> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ConstantAtomicWrapper.SPOILERS);
