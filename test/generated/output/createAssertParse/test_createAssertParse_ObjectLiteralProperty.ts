...
export const test_createAssertParse_ObjectLiteralProperty = _test_assertParse("ObjectLiteralProperty", ObjectLiteralProperty.generate, (input: string): typia.Primitive<ObjectLiteralProperty> => { const assert = (input: any): ObjectLiteralProperty => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, ObjectLiteralProperty.SPOILERS);
