...
export const test_isStringify_DynamicArray = _test_isStringify("DynamicArray", DynamicArray.generate, (input) => ((input: DynamicArray): string | null => { const is = (input: any): input is DynamicArray => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), DynamicArray.SPOILERS);
