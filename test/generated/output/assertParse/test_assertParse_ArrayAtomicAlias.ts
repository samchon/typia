...
export const test_assertParse_ArrayAtomicAlias = _test_assertParse("ArrayAtomicAlias", ArrayAtomicAlias.generate, (input) => ((input: string): typia.Primitive<ArrayAtomicAlias> => { const assert = (input: any): ArrayAtomicAlias => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayAtomicAlias.SPOILERS);
