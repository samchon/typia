...
- `tests/test-*`: feature-test workspaces:
  ...
  - `test-feature-identity`: repository check that every suite's committed `src/features` file exports exactly one `test_*` function named after the file, and that one suite never exports a name twice.
