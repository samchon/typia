...
export const test_assertPrune_TagFormat = _test_assertPrune("TagFormat", TagFormat.generate, (input) => ((input: any): TagFormat => { const assert = (input: any): TagFormat => {
    ...
    if ("object" === typeof input && null !== input)
        $po0(input);
}; assert(input); prune(input); return input; })(input), TagFormat.SPOILERS);
