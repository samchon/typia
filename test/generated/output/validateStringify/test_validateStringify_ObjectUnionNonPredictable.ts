...
export const test_validateStringify_ObjectUnionNonPredictable = _test_validateStringify("ObjectUnionNonPredictable", ObjectUnionNonPredictable.generate, (input) => ((input: Array<ObjectUnionNonPredictable.IWrapper<ObjectUnionNonPredictable.IUnion>>): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<Array<ObjectUnionNonPredictable.IWrapper<ObjectUnionNonPredictable.IUnion>>> => {
    const errors = [] as any[];
    const $report = (typia.validateStringify as any).report(errors);
    const __is = (input: any): input is Array<ObjectUnionNonPredictable.IWrapper<ObjectUnionNonPredictable.IUnion>> => {
...
}; const stringify = (input: Array<ObjectUnionNonPredictable.IWrapper<ObjectUnionNonPredictable.IUnion>>): string => {
    const $io1 = (input: any): boolean => "object" === typeof input.value && null !== input.value && $iu0(input.value);
    ...
    const $so2 = (input: any): any => `{"value":${`{"value":${(input.value as any).value}}`}}`;
    const $so4 = (input: any): any => `{"value":${`{"value":${$number((input.value as any).value)}}`}}`;
    const $so6 = (input: any): any => `{"value":${`{"value":${$string((input.value as any).value)}}`}}`;
    ...
    output.data = stringify(input); return output; })(input), ObjectUnionNonPredictable.SPOILERS);
