...
export const test_isStringify_ArrayRecursive = _test_isStringify("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: ArrayRecursive.ICategory): string | null => { const is = (input: any): input is ArrayRecursive.ICategory => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; })(input), ArrayRecursive.SPOILERS);
