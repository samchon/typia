...
export const test_assertParse_ObjectIntersection = _test_assertParse("ObjectIntersection", ObjectIntersection.generate, (input) => ((input: string): typia.Primitive<ObjectIntersection> => { const assert = (input: any): ObjectIntersection => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectIntersection.SPOILERS);
