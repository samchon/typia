...
const unpluginFactory = (rawOptions = {}) => {
     ...
     */
    function generateCodeWithMap({ source, code, id }) {
    ...
    async function transformCodeWithTypiaTransform({ id, source, ctx, options }) {
        /** get cache */
    ...
    return {
        ...
        async transform(_source, _id) {
            ...
            const id = wrap(removeRslibPrefixId);
            /** skip if source does not include typia */
            if (!source.includes('typia')) {
                return;
            }
            const _transform = async ({ source, id }) => transformCodeWithTypiaTransform({ id, source, ctx: this, options });
            /** transform code */
            let code;
