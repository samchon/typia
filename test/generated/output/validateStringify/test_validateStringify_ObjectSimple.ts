...
export const test_validateStringify_ObjectSimple = _test_validateStringify("ObjectSimple", ObjectSimple.generate, (input) => ((input: ObjectSimple.IBox3D): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<ObjectSimple.IBox3D> => {
    ...
    const $so0 = (input: any): any => `{"scale":${`{"x":${$number(input.scale.x)},"y":${$number(input.scale.y)},"z":${$number(input.scale.z)}}`},"position":${`{"x":${$number(input.position.x)},"y":${$number(input.position.y)},"z":${$number(input.position.z)}}`},"rotate":${`{"x":${$number(input.rotate.x)},"y":${$number(input.rotate.y)},"z":${$number(input.rotate.z)}}`},"pivot":${`{"x":${$number(input.pivot.x)},"y":${$number(input.pivot.y)},"z":${$number(input.pivot.z)}}`}}`;
    return $so0(input);
}; const output = validate(input) as any; if (output.success)
    output.data = stringify(input); return output; })(input), ObjectSimple.SPOILERS);
