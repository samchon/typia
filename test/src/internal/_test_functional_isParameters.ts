...
export const _test_functional_isParameters =
  (name: string) =>
  ...
  () => {
    const task = (callback: (input: T) => [T, T]) => {
      ...
      if (validate(() => y)(x) === null)
        throw new Error(
          `Bug on typia.functional.isParameters(): failed to understand the ${name} type.`,
        );
      for (const spoil of factory.SPOILERS ?? []) {
        ...
        if (validate(() => y)(x) !== null)
          throw new Error(
            `Bug on typia.functional.isParameters(): failed to detect error on the ${name} type.`,
          );
      }
    };
