...
export const test_assertParse_TagArray = _test_assertParse("TagArray", TagArray.generate, (input) => ((input: string): typia.Primitive<TagArray> => { const assert = (input: any): TagArray => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), TagArray.SPOILERS);
