...
export const test_validateParse_ObjectPrimitive = _test_validateParse("ObjectPrimitive", ObjectPrimitive.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ObjectPrimitive>> => { const validate = (input: any): typia.IValidation<ObjectPrimitive> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ObjectPrimitive.SPOILERS);
