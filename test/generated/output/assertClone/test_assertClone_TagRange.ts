...
export const test_assertClone_TagRange = _test_assertClone("TagRange", TagRange.generate, (input) => ((input: any): typia.Primitive<Array<TagRange.Type>> => { const assert = (input: any): Array<TagRange.Type> => {
...
}; assert(input); const output = clone(input); return output; })(input), TagRange.SPOILERS);
