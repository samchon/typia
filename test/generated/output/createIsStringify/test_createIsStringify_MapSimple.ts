...
export const test_createIsStringify_MapSimple = _test_isStringify("MapSimple", MapSimple.generate, (input: MapSimple): string | null => { const is = (input: any): input is MapSimple => {
    ...
    return $so0(input);
}; return is(input) ? stringify(input) : null; }, MapSimple.SPOILERS);
