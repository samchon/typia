...
> The right-hand "Compiled JavaScript" tab is produced by typia's compile-time transform — **stock `tsc` will not produce it**. To enable the transform, install the TypeScript 7 toolchain in [Setup](/docs/setup) and build with `ttsc`, `ttsx`, or `@ttsc/unplugin`. Without setup, your `typia.is<T>()` call sites throw at runtime instead.
