...
export function _test_isPrune<T>(name: string, generator: () => T, pruner: (input: T) => boolean, spoilers?: Spoiler<T>[]): () => void {
    return () => {
        ...
        if (pruner(input) === false)
            throw new Error(`Bug on typia.isPrune(): failed to understand the ${name} type.`);
        else if (pruner.toString().indexOf("RegExp(/(.*)/).test") === -1)
            iterate((obj: any) => {
                if (Object.keys(obj).some((key) => key.indexOf("__non_regular_type__") === 0))
                    throw new Error(`Bug on typia.isPrune(): failed to prune the ${name} type.`);
            })(input);
        // SPOIL
        for (const spoil of spoilers ?? []) {
            ...
            spoil(elem);
            if (pruner(elem) === true)
                throw new Error(`Bug on typia.isPrune(): failed to detect error on the ${name} type.`);
        }
    };
}
