...
export const test_createAssertParse_ObjectOptional = _test_assertParse("ObjectOptional", ObjectOptional.generate, (input: string): typia.Primitive<ObjectOptional> => { const assert = (input: any) => {
...
}; input = JSON.parse(input); return assert(input); }, ObjectOptional.SPOILERS);
