...
export const _test_assertParse = <T>(name: string, generator: () => T, parser: (input: string) => Primitive<T>, spoilers?: Spoiler<T>[]) => () => {
    ...
    if (primitive_equal_to(expected, parsed) === false) {
        throw new Error(`Bug on typia.assertParse(): failed to understand the ${name} type.`);
    }
    for (const spoil of spoilers ?? []) {
        ...
        throw new Error(`Bug on typia.assertParse(): failed to detect error on the ${name} type.`);
    }
};
