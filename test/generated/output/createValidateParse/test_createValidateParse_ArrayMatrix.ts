...
export const test_createValidateParse_ArrayMatrix = _test_validateParse("ArrayMatrix", ArrayMatrix.generate, (input: string): typia.IValidation<typia.Primitive<ArrayMatrix>> => { const validate = (input: any): typia.IValidation<ArrayMatrix> => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ArrayMatrix.SPOILERS);
