...
export const test_assertParse_ObjectSimple = _test_assertParse("ObjectSimple", ObjectSimple.generate, (input) => ((input: string): typia.Primitive<ObjectSimple> => { const assert = (input: any): ObjectSimple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ObjectSimple.SPOILERS);
