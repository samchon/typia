...
export const test_createValidateParse_ObjectUnionExplicit = _test_validateParse("ObjectUnionExplicit", ObjectUnionExplicit.generate, (input: string): typia.IValidation<typia.Primitive<ObjectUnionExplicit>> => { const validate = (input: any): typia.IValidation<ObjectUnionExplicit> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; }, ObjectUnionExplicit.SPOILERS);
