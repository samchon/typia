...
// TestPlainPruneProgrammerBranchMatrix covers prune-specific recursive branches.
//
// The broad plain helper test drives shared clone/prune helpers, but recursive
...
func TestPlainPruneProgrammerBranchMatrix(t *testing.T) {
  ...
  functor := nativehelpers.NewFunctionProgrammer("plain.prune")
  objectMeta := plainObjectMeta("PruneBranchObject")
  ...
  if len(plainPruneProgrammer_write_array_functions(struct {
    Context    nativecontext.ITypiaContext
  ...
  if len(plainPruneProgrammer_write_tuple_functions(struct {
    Context    nativecontext.ITypiaContext
    Config     nativeinternal.FeatureProgrammer_IConfig
    Functor    *nativehelpers.FunctionProgrammer
    Collection *schemametadata.MetadataCollection
  ...
  if plainPruneProgrammer_decode_array(plainPruneProgrammer_decodeArrayProps{
    Config:  config,
    Functor: functor,
    Input:   input,
  ...
  if plainPruneProgrammer_to_statement(nil, emit) != nil ||
    plainPruneProgrammer_to_statement(factory.NewBlock(factory.NewNodeList(nil), false), emit).Kind != shimast.KindBlock ||
    plainPruneProgrammer_to_statement(factory.NewReturnStatement(input), emit).Kind != shimast.KindReturnStatement {
    t.Fatal("prune statement normalizer should preserve nil, blocks, and returns")
  }
  if plainPruneProgrammer_top_level_body(factory.NewBlock(factory.NewNodeList([]*shimast.Node{
    factory.NewExpressionStatement(input),
  }), true), emit) == nil {
    t.Fatal("top-level prune body should unwrap block statements")
  }
  if plainPruneProgrammer_explore_objects(plainPruneProgrammer_exploreObjectsProps{
    Config:   config,
    Functor:  functor,
    Input:    input,
    Metadata: objectMeta,
