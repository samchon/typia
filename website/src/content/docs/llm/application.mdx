...
**Why this and not "validate after the call":** because of the harness. If you wait until after `ILlmFunction.execute(args)` to apply business rules, the failure path is your problem — you have to format errors, decide whether to retry, choose what to send back. Wiring the validator into `config.validate` makes the rules part of the harness loop: the model gets one consistent error format, the retry logic is already there, and `ILlmFunction.validate(args)` reports the *combined* type + business-rule result so callers don't need to know there's an override.

> [!TIP]
>
> Combine `config.validate` with `{ equals: true }` if you want both strict-shape and business-rule checks. The strict shape runs first (inside the per-method validator you call as `typia.validateEquals<ArgumentType>(input)`); your business rules layer on top.
