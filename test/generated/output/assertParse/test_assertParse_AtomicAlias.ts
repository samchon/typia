...
export const test_assertParse_AtomicAlias = _test_assertParse("AtomicAlias", AtomicAlias.generate, (input) => ((input: string): typia.Primitive<AtomicAlias> => { const assert = (input: any): AtomicAlias => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), AtomicAlias.SPOILERS);
