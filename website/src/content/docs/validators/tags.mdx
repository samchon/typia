...
On a `bigint`, `Type<"int64">` accepts `-(2n ** 63n)` through `2n ** 63n - 1n` and `Type<"uint64">` accepts `0n` through `2n ** 64n - 1n`, both inclusive. Those are exactly the values `typia.protobuf.encode` can write into a 64-bit varint without truncating them, so a certified value always survives the round trip. On a `number` the same two tags bound their range at the neighbouring power of two, because no `number` can represent either maximum.
...
`MinLength` and `MaxLength` count characters, not UTF-16 code units, which is what the `minLength` and `maxLength` keywords they emit mean. One emoji is one character even though its `.length` is `2`, and a flag or a ZWJ sequence counts each of its code points.
