    ...
    const task = (callback: (input: T) => [T, T]) => {
      {
        ...
        if (validate(() => y)(x) === null)
          throw new Error(
            `Bug on typia.functional.equalsReturn(): failed to understand the ${name} type.`,
          );
      }
      ...
      if (validate(() => y)(x) !== null)
        throw new Error(
          `Bug on typia.functional.equalsReturn(): failed to detect surplus property on the ${name} type.`,
        );
    };
