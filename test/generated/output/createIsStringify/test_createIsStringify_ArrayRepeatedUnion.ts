...
export const test_createIsStringify_ArrayRepeatedUnion = _test_isStringify("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: ArrayRepeatedUnion): string | null => { const is = (input: any): input is ArrayRepeatedUnion => {
    const $ip0 = (input: any) => {
        const array = input;
        const top = input[0];
        if (0 === input.length)
...
}; return is(input) ? stringify(input) : null; }, ArrayRepeatedUnion.SPOILERS);
