...
export const test_validateParse_ObjectLiteralProperty = _test_validateParse("ObjectLiteralProperty", ObjectLiteralProperty.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<ObjectLiteralProperty>> => { const validate = (input: any): typia.IValidation<ObjectLiteralProperty> => {
    const errors = [] as any[];
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), ObjectLiteralProperty.SPOILERS);
