...
On a `bigint`, `Type<"int64">` accepts `-(2n ** 63n)` through `2n ** 63n - 1n` and `Type<"uint64">` accepts `0n` through `2n ** 64n - 1n`, both inclusive. Those are exactly the values `typia.protobuf.encode` can write into a 64-bit varint without truncating them, so a `bigint` typia certifies always survives the round trip. The `number` form cannot be that exact: neither maximum is representable, so each rounds up to the neighbouring power of two and is accepted — `2 ** 63` passes as an `int64` — and that one value is not round-trip safe. Use the `bigint` form when the boundary matters.
