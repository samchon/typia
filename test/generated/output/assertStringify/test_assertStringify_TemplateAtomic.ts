...
export const test_assertStringify_TemplateAtomic = _test_assertStringify("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: any): string => { const assert = (input: any): TemplateAtomic => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), TemplateAtomic.SPOILERS);
