...
`typia.llm.application` and `typia.llm.controller` report their own config the same way, so pass `application.config` there too. Only a bare schema from `typia.llm.parameters` or `typia.llm.structuredOutput` carries no config of its own, which is why the first example above restates the generic.
