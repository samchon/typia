...
export const test_createIsParse_ArrayRepeatedUnion = _test_isParse("ArrayRepeatedUnion", ArrayRepeatedUnion.generate, (input: any): typia.Primitive<ArrayRepeatedUnion> => { const is = (input: any): input is ArrayRepeatedUnion => {
    const $ip0 = (input: any) => {
        const array = input;
        const top = input[0];
        if (0 === input.length)
...
}; input = JSON.parse(input); return is(input) ? input as any : null; }, ArrayRepeatedUnion.SPOILERS);
