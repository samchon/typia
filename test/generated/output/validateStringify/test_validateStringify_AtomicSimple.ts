...
export const test_validateStringify_AtomicSimple = _test_validateStringify("AtomicSimple", AtomicSimple.generate, (input) => ((input: [boolean, number, string]): typia.IValidation<string> => { const validate = (input: any): typia.IValidation<[boolean, number, string]> => {
    const errors = [] as any[];
    ...
    output.data = stringify(input); return output; })(input), AtomicSimple.SPOILERS);
