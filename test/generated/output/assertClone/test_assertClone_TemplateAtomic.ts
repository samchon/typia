...
export const test_assertClone_TemplateAtomic = _test_assertClone("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: any): typia.Primitive<TemplateAtomic> => { const assert = (input: any): TemplateAtomic => {
...
}; assert(input); const output = clone(input); return output; })(input), TemplateAtomic.SPOILERS);
