...
export const test_assertParse_ArrayAtomicSimple = _test_assertParse("ArrayAtomicSimple", ArrayAtomicSimple.generate, (input) => ((input: string): typia.Primitive<ArrayAtomicSimple> => { const assert = (input: any): ArrayAtomicSimple => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayAtomicSimple.SPOILERS);
