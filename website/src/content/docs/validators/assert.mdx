...
>
> `Array#map`, `forEach`, and `filter` pass the element index as their second argument, which lands in `errorFactory`. TypeScript rejects `rows.map(assertUser)` for that reason; write `rows.map((row) => assertUser(row))` instead. A build compiled against typia 13.2.0 or earlier still type-checks the direct form and throws `TypeGuardError` for the failing element rather than the configured error.
