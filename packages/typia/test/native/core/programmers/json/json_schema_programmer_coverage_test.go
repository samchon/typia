...
func TestJsonSchemaProgrammerCoverage(t *testing.T) {
  ...
  if messages := JsonApplicationProgrammer.Validate(struct {
    Metadata *schemametadata.MetadataSchema
    Explore  nativefactories.MetadataFactory_IExplore
  }{Metadata: valid, Explore: nativefactories.MetadataFactory_IExplore{Top: false}}); len(messages) != 0 {
    t.Fatalf("valid atomic schema should produce no validation messages, got %v", messages)
  }
