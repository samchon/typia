...
export const test_validateParse_AtomicClass = _test_validateParse("AtomicClass", AtomicClass.generate, (input) => ((input: string): typia.IValidation<typia.Primitive<AtomicClass>> => { const validate = (input: any): typia.IValidation<AtomicClass> => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is AtomicClass => {
...
}; input = JSON.parse(input); const output = validate(input); return output as any; })(input), AtomicClass.SPOILERS);
