...
`), 0o644); err != nil {
	...
	removedWd := filepath.Join(root, "removed-wd")
	if err := os.MkdirAll(removedWd, 0o755); err != nil {
		t.Fatalf("mkdir removed cwd: %v", err)
	}
	if err := os.Chdir(removedWd); err != nil {
		t.Fatalf("chdir removed cwd: %v", err)
	}
	...
	if err := TypiaGenerator.Build(TypiaGenerator_ILocation{
		...
		t.Fatal("relative input should fail when cwd cannot be resolved")
	}
	if err := TypiaGenerator.Build(TypiaGenerator_ILocation{
		Input:   input,
		...
		t.Fatal("relative output should fail when cwd cannot be resolved")
	}
	if err := TypiaGenerator.Build(TypiaGenerator_ILocation{
		Input:   input,
		Output:  output,
		...
		t.Fatal("relative project should fail when cwd cannot be resolved")
	}
