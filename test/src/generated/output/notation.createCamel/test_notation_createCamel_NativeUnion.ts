...
export const test_notation_createValidateCamel_NativeUnion =
  ...
  >({
    convert: (input: any): typia.IValidation<typia.CamelCase<NativeUnion>> => {
      ...
      const general = (input: NativeUnion): typia.CamelCase<NativeUnion> => {
        const $cp0 = (input: any) =>
          input.map((elem: any) =>
            "object" === typeof elem && null !== elem
              ? $co0(elem)
    ...
    assert: (input: any): typia.CamelCase<NativeUnion> => {
      const __is = (input: any): input is typia.CamelCase<NativeUnion> => {
        const $io0 = (input: any): boolean =>
          (null === input.date || input.date instanceof Date) &&
          (input.unsigned instanceof Uint8Array ||
            input.unsigned instanceof Uint8ClampedArray ||
      ...
      if (false === __is(input))
        ((
          ...
          _path: string,
          _exceptionable: boolean = true,
        ): input is typia.CamelCase<NativeUnion> => {
