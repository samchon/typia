...
export const _test_functional_assertEqualsParametersAsync =
  (ErrorClass: Function) =>
  (name: string) =>
  ...
  async () => {
      ...
      (replacer: string) => async (callback: (input: T) => [T, T]) => {
        ...
        try {
          ...
          if ((exp as Function).constructor?.name === ErrorClass.name)
            throw new Error(
              `Bug on await typia.functional.assertEqualsParameters(): failed to understand the ${name} type.`,
            );
          else throw exp;
        }
        ...
        for (const { path, value } of accessors) {
          ...
          try {
            await assertEquals(async () => y)(x);
            throw new Error(
              `Bug on await typia.functional.assertEqualsParameters(): failed to detect surplus property on the ${name} type.`,
            );
          } catch (exp) {
            if (
              (exp as Function).constructor?.name === ErrorClass.name &&
              typia.is<TypeGuardError.IProps>(exp) &&
              exp.method === "typia.functional.assertEqualsParameters" &&
              exp.path === fullPath &&
    ...
    await validate("$input.parameters[0]")((input) => [
      input,
      factory.generate(),
    ]);
