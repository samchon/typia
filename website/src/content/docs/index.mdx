...
![Write a TypeScript type and typia compiles it into a dedicated validator](/images/concepts/transform.svg)

No reflection at runtime, no schema file to maintain — just the exact comparisons your `IMember` type implies, inlined into the build. This is the core trick that every other typia feature builds on; see [Pure TypeScript](/docs/pure) for the long story.

> The right-hand "Compiled Output" is produced by typia's compile-time transform — **stock `tsc` will not produce it**. To enable the transform, install the TypeScript 7 toolchain in [Setup](/docs/setup) and build with `ttsc`, `ttsx`, or `@ttsc/unplugin`. Without setup, your `typia.is<T>()` call sites throw at runtime instead.
...
typia.llm.evaluation<T>(); // evaluation-model questions + checked answer decoder
...
<Callout type="default">
  Runtime validator up to **20,000× faster** than `class-validator`
  ([benchmark](https://github.com/samchon/typia/tree/master/benchmark/results/AMD%20Ryzen%209%207940HS%20w%20Radeon%20780M%20Graphics))
</Callout>

<Callout type="info">
  JSON serialization up to **200× faster** than `class-transformer`
  ([benchmark](https://github.com/samchon/typia/tree/master/benchmark/results/AMD%20Ryzen%209%207940HS%20w%20Radeon%20780M%20Graphics#stringify))
</Callout>

<Callout type="default">
  LLM function calling success rate: **6.75% → 100%** with the typia harness
  ([story](https://github.com/wrtnlabs/autobe))
</Callout>
...
<br />
<iframe
  src="https://www.youtube.com/embed/WM6LGy2UU6s?si=lZrqbILxHI22QwEE"
...
The transformation above happens at compile time, so typia needs to be wired into the TypeScript build. Use [`ttsc`](/docs/setup) for TypeScript 7 builds, [`ttsx`](/docs/setup) when running a TypeScript entrypoint directly, and [`@ttsc/unplugin`](/docs/setup#bundlers) for bundlers such as Vite, Next.js, Webpack, esbuild, Rollup, and Bun.
...
- **Need to install it now?** See [Setup](/docs/setup) for the TypeScript 7 `ttsc` toolchain.
...
- **Adopting on NestJS / Hono / tRPC?** See the [Utilization Cases](/docs/utilization/nestjs) — drop-in recipes for the most common backend frameworks.
