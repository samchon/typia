...
func EmitLlmStructuredOutputExpression(schema *metadata.Schema) (string, error) {
	...
	return fmt.Sprintf(
		`(() => { const __utils = require("@typia/utils"); const __schema = %s; const __result = __utils.LlmSchemaConverter.parameters({ components: __schema.components, schema: __schema.schema }); if (!__result.success) throw new Error("typia.llm.structuredOutput: failed to convert JSON schema to LLM schema"); const __parameters = __result.value; const __validate = %s; return { parameters: __parameters, parse: (input) => __utils.LlmJson.parse(input, __parameters), coerce: (input) => __utils.LlmJson.coerce(input, __parameters), validate: __validate }; })()`,
		schemaExpr,
		validateExpr,
	), nil
}
...
func emitLlmFunctionEntry(name string, fn *metadata.Function) (string, error) {
	...
	parts := []string{
		...
		fmt.Sprintf(`validate: (input) => { const __hook = __hooks && __hooks[%s]; return typeof __hook === "function" ? __hook(input) : (%s)(input); }`, jsonQuote(name), validateExpr),
	}
	if hasOutput {
...
func emitLlmParameterArtifacts(name string, fn *metadata.Function) (string, string, error) {
	...
	if len(fn.Parameters) > 1 {
		return "", "", fmt.Errorf("llm.controller.%s: only zero or one parameter is supported in Phase 0", name)
	}
