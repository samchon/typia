...
export const test_assertStringify_MapSimple = _test_assertStringify("MapSimple", MapSimple.generate, (input) => ((input: any): string => { const assert = (input: any): MapSimple => {
    ...
    return $so0(input);
}; return stringify(assert(input)); })(input), MapSimple.SPOILERS);
