...
export const test_createAssertParse_TagObjectUnion = _test_assertParse("TagObjectUnion", TagObjectUnion.generate, (input: string): typia.Primitive<TagObjectUnion> => { const assert = (input: any): TagObjectUnion => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, TagObjectUnion.SPOILERS);
