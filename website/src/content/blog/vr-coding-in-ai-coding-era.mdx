...
AI coding has a strange new kind of idle time. An agent starts editing, pauses to think, runs a test, waits on a build, or gets stuck halfway through a plan. Sitting there watching one task crawl is boring, so people naturally fill that time by launching another agent on another ticket. Around me, this is already normal: one developer, several agents, several tasks moving at once.

Launching them is easy. Watching them is the annoying part.

And by "watching," I do not mean staring at five agent terminals. A CLI transcript tells me what the agent claims it is doing. The editor tells me what it is actually changing. For this workflow to be safe, I need the code, the diff, and the agent log visible together.
...
In practice, I usually split each VSCode window into two parts: one side has a terminal running the Codex or Claude Code CLI, and the other side has the source code or diff it is changing. The exact agent does not matter much. What matters is that the agent's words and the agent's code changes sit next to each other.
...
I mainly bounce between two apps.

- **[Immersed](https://immersed.com/)** — The fast path. It gives me a repeatable five-screen workspace with very little setup.
- **[Overay desk](https://desk.overay.com/en)** — It really is called _Overay_. In my setup, it is the manual-control path: more work to arrange, but more freedom to shape the monitoring layout.
...
The result: when I'm in VR, I average 3–4 hours in one seat coding. The agents stay in front of me the whole time, so I keep checking them instead of drifting away and coming back to a giant mystery diff.
...
The setup is not smooth. It is heavier, slower to start, and more awkward than opening a laptop. But once I am inside it, the friction works in my favor. I stay seated, keep the agents in view, and catch suspicious diffs earlier.

> **For me, VR coding is not about escaping the desk. It is about keeping the agent, the code, and the diff visible before the bad change turns into a rewrite.**
...
Right now I'm on **Meta Quest 3**. Both Overay and Immersed run smoothly on it, and it has handled my five-screen workspace reliably enough for daily use. Price, weight, passthrough quality, app compatibility, and the ability to glance at my real keyboard all matter here. If retail is steep, used works fine — I picked mine up for $300 secondhand.
