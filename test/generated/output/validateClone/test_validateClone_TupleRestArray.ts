...
export const test_validateClone_TupleRestArray = _test_validateClone("TupleRestArray", TupleRestArray.generate, (input) => ((input: any): typia.IValidation<typia.Primitive<[boolean, number, ...Array<string>[]]>> => { const validate = (input: any): typia.IValidation<[boolean, number, ...Array<string>[]]> => {
    const __is = (input: any): input is [boolean, number, ...Array<string>[]] => {
    ...
    if (false === __is(input))
        ((input: any, _path: string, _exceptionable: boolean = true): input is [boolean, number, ...Array<string>[]] => {
...
}; const clone = (input: [boolean, number, ...Array<string>[]]): typia.Primitive<[boolean, number, ...Array<string>[]]> => {
    return Array.isArray(input) && ("boolean" === typeof input[0] && "number" === typeof input[1] && (Array.isArray(input.slice(2)) && input.slice(2).every((elem: any) => Array.isArray(elem) && elem.every((elem: any) => "string" === typeof elem)))) ? [
        input[0] as any,
        input[1] as any,
        ...Array.isArray(input.slice(2)) ? input.slice(2).map((elem: any) => Array.isArray(elem) ? elem.map((elem: any) => elem as any) : elem as any) : input.slice(2) as any
    ] as any : input as any;
}; const output = validate(input) as any; if (output.success)
    output.data = clone(input); return output; })(input), TupleRestArray.SPOILERS);
