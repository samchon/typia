...
export const test_functional_validateEqualsFunctionAsync_ObjectUnionImplicit =
  _test_functional_validateEqualsFunctionAsync("ObjectUnionImplicit")(
    ObjectUnionImplicit,
  )(
    (p: (input: ObjectUnionImplicit) => Promise<ObjectUnionImplicit>) =>
      async (
        input: ObjectUnionImplicit,
      ): Promise<import("typia").IValidation<ObjectUnionImplicit>> => {
        const paramResults = [
          ...
          const __is = (
            ...
            const $io5 = (
              ...
              (1 === Object.keys(input).length ||
                Object.keys(input).every((key: any) => {
                  if (
                    ["outer", "inner", "area"].some((prop: any) => key === prop)
                  )
                    return true;
                  const value = input[key];
                  if (undefined === value) return true;
                ...
                [
                  ...
                  2 === Object.keys(input).length ||
                    ...
                    Object.keys(input)
                      .map((key: any) => {
                        if (
                          ["x", "y", "slope"].some((prop: any) => key === prop)
                        )
                          return true;
                        const value = input[key];
                        if (undefined === value) return true;
                ...
                [
                  ...
                  1 === Object.keys(input).length ||
                    ...
                    Object.keys(input)
                      .map((key: any) => {
                        if (
                          ["points", "length"].some((prop: any) => key === prop)
                        )
                          return true;
                        const value = input[key];
                        if (undefined === value) return true;
