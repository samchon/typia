...
export const test_createIsPrune_ArrayUnion = _test_isPrune("ArrayUnion", ArrayUnion.generate, (input: any): input is ArrayUnion => { const is = (input: any): input is ArrayUnion => {
    const $ip0 = (input: any) => {
        const array = input;
        const top = input[0];
        if (0 === input.length)
    ...
    return false; prune(input); return true; }, ArrayUnion.SPOILERS);
