...
export const test_createAssertParse_DynamicUndefined = _test_assertParse("DynamicUndefined", DynamicUndefined.generate, (input: string): typia.Primitive<DynamicUndefined> => { const assert = (input: any): DynamicUndefined => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; }, DynamicUndefined.SPOILERS);
