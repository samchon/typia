...
export const test_isParse_TemplateAtomic = _test_isParse("TemplateAtomic", TemplateAtomic.generate, (input) => ((input: any): typia.Primitive<TemplateAtomic> => { const is = (input: any): input is TemplateAtomic => {
...
}; input = JSON.parse(input); return is(input) ? input as any : null; })(input), TemplateAtomic.SPOILERS);
