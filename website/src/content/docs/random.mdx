...
It honors satisfiable [type tags](/docs/validators/tags) you put on the type: `tags.Format<"email">` will produce an email, `tags.Minimum<0> & tags.Maximum<100>` will pick a number in that range, `tags.MaxLength<8>` will cap a string's length.

Numeric `tags.MultipleOf<N>` constraints choose an integer quotient and construct the result with finite-decimal arithmetic. One-sided fallback ranges stay in value units before that snapping step, and a bounded range with no valid multiple throws instead of retrying indefinitely.

`MultipleOf` is one constraint where a generated value can fail its own validator. The generator picks an integer quotient over exact decimals, while `tags.MultipleOf<N>` validates with `value % N === 0`, which divides the binary doubles actually stored — so the two part company wherever an operand does not equal the decimal it prints back. `typia.random<number & tags.MultipleOf<0.1>>()` can return `0.3`, and `0.3 % 0.1` is `0.09999999999999998`. Past `Number.MAX_SAFE_INTEGER` the generated value stops printing back exactly, so an integer divisor can diverge there too.

Length tags combine with a `Format` or `Pattern` on the same string: `tags.Format<"email"> & tags.MinLength<40>` produces a 40-character email, and `tags.Pattern<"^[0-9]+$"> & tags.MaxLength<3>` produces a short digit string. A combination no string can satisfy — `tags.Format<"email"> & tags.MaxLength<3>`, or a pattern whose fixed length conflicts with the bound — throws instead of emitting a value its own validator would reject.
