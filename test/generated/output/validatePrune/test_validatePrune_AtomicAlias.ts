...
export const test_validatePrune_AtomicAlias = _test_validatePrune("AtomicAlias", AtomicAlias.generate, (input) => ((input: any): typia.IValidation<[boolean, number, string]> => { const validate = (input: any): typia.IValidation<[boolean, number, string]> => {
    const errors = [] as any[];
    ...
    prune(input); return output; })(input), AtomicAlias.SPOILERS);
