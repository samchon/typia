...
export const test_assertParse_ArrayRecursive = _test_assertParse("ArrayRecursive", ArrayRecursive.generate, (input) => ((input: string): typia.Primitive<ArrayRecursive> => { const assert = (input: any): ArrayRecursive => {
    ...
    return input;
}; input = JSON.parse(input); return assert(input) as any; })(input), ArrayRecursive.SPOILERS);
