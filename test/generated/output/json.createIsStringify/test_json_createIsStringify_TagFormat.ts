...
export const test_json_isStringify_TagFormat = _test_json_isStringify(
    "TagFormat",
)<TagFormat>(TagFormat)((input: TagFormat): string | null => {
    const is = (input: any): input is TagFormat => {
        const $is_uuid = (typia.json.createIsStringify as any).is_uuid;
        const $is_email = (typia.json.createIsStringify as any).is_email;
        ...
        const $is_datetime = (typia.json.createIsStringify as any).is_datetime;
        return (
            "object" === typeof input &&
            null !== input &&
